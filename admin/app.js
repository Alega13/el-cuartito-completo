...
const app = {
    ...
    async syncWithDiscogs() {
        ...
        try {
            ...
            if (result.success) {
                this.showToast(`✅ Sincronizado: ${result.synced} productos (${result.created} nuevos, ${result.updated} actualizados)`);
                // Reload inventory
                await this.loadData();
                this.refreshCurrentView();
            } else {
...
`;
            ...
            if (publishDiscogs) {
                ...
                if (existingProduct && existingProduct.discogs_listing_id) {
                    ...
                    try {
                        ...
                        this.showToast('⚠️ Error actualizando Discogs (producto guardado localmente)', 'warning');
                    }
                } else if (releaseId) {
                    // Create new listing
                    try {
                        ...
                        if (result.success && result.listingId) {
                            ...
                            this.showToast('💿 Publicado en Discogs!');
                        }
                    } catch (error) {
                        console.error('Error creating Discogs listing:', error);
                        this.showToast('⚠️ Error publicando en Discogs (producto guardado localmente)', 'warning');
                    }
                } else {
    ...
    handleDiscogsSelection(release) {
        ...
        if (form) {
            ...
            // Save Discogs Release ID for publishing
    ...
    renderDiscogsSales(container) {
        ...
        const totalRevenue = discogsSales.reduce((sum, s) => sum + (parseFloat(s.total_amount || s.total) || 0), 0);
        const totalProfit = discogsSales.reduce((sum, s) => {
            const total = parseFloat(s.total_amount || s.total) || 0;
            let saleCost = 0;
            if (s.items && Array.isArray(s.items)) {
                saleCost = s.items.reduce((c, i) => {
        ...
        <div class="p-6">
            ...
            <div class="flex items-center justify-between mb-8">
                ...
                <div class="bg-gradient-to-br from-purple-500 to-indigo-600 text-white px-6 py-4 rounded-2xl shadow-xl">
                    <div class="text-sm font-medium opacity-90">Ingresos Totales</div>
                    <div class="text-3xl font-bold">DKK ${totalRevenue.toFixed(2)}</div>
                    <div class="text-xs opacity-75">${discogsSales.length} ventas</div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div class="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="text-2xl font-bold text-brand-dark">DKK ${totalRevenue.toFixed(2)}</div>
                ...
                <div class="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="text-2xl font-bold text-brand-dark">DKK ${totalProfit.toFixed(2)}</div>
            ...
            const price = item?.priceAtSale || (sale.total_amount / quantity) || 0;
            ...
            return `
                                        <tr class="border-b border-slate-50 hover:bg-purple-50/30 transition-colors">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="font-bold text-brand-dark text-sm">${item?.album || 'Producto'}</div>
                                                <div class="text-xs text-slate-500">${item?.artist || '-'}</div>
                                            </td>
                                            <td class="px-6 py-4 text-sm font-medium text-slate-700">${quantity}</td>
                                            <td class="px-6 py-4 text-sm font-bold text-brand-dark">DKK ${price.toFixed(2)}</td>
                                            <td class="px-6 py-4 text-sm font-bold text-purple-600">DKK ${sale.total_amount.toFixed(2)}</td>
                                            <td class="px-6 py-4 text-sm font-bold ${profit > 0 ? 'text-green-600' : 'text-slate-400'}">DKK ${profit.toFixed(2)}</td>
                                        </tr>
                                    `;
        }).join('')}
                            </tbody>
