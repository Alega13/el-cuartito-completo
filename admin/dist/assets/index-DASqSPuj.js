...
Por favor, recarga la página.`)}else{const l=I.collection("metadata").doc("vinylCounter"),i=await l.get(),d=i.exists&&i.data().current||0;e>d&&(await l.set({current:e},{merge:!0}),console.log(`🆙 Contador actualizado a ${e}`)),console.log("✅ No hay conflictos de SKU.")}}catch(t){console.error("❌ Error en script histórico:",t)}}},state:{inventory:[],sales:[],expenses:[],consignors:[],cart:[],viewMode:"list",selectedItems:new Set,currentView:"dashboard",filterMonths:[new Date().getMonth()],filterYear:new Date().getFullYear(),inventorySearch:"",salesHistorySearch:"",expensesSearch:"",events:[],selectedDate:new Date,vatActive:!1,manualSaleSearch:"",posCondition:"Used",posSelectedItemSku:null,orderFeedFilter:"all",filterGenre:"all",filterOwner:"all",filterLabel:"all",filterStorage:"all",filterDiscogs:"all",filterStockTime:[],privacyMode:!1,rsdExtraDiscount:!1,dashboardAnalysisMode:"genre"},getEffectivePrice(t){return t.is_rsd_discount?Math.round(t.price*.9):t.price},async init(){this.runSkuFix_v3(),!this._initialized&&(this._initialized=!0,re.onAuthStateChanged(async t=>{if(t)try{document.getElementById("login-view").classList.add("hidden"),document.getElementById("main-app").classList.remove("hidden"),document.getElementById("mobile-nav").classList.remove("hidden"),await this.loadData(),this._pollInterval&&clearInterval(this._pollInterval),this._pollInterval=setInterval(()=>this.loadData(),6e4),this.setupListeners(),this.setupMobileMenu(),this.setupNavigation()}catch(e){console.error("Auth token error:",e),this.logout()}else{document.getElementById("login-view").classList.remove("hidden"),document.getElementById("main-app").classList.add("hidden"),document.getElementById("mobile-nav").classList.add("hidden");const e=document.getElementById("login-btn");e&&(e.disabled=!1,e.innerHTML="<span>Entrar</span>")}}),document.addEventListener("click",t=>{const e=document.getElementById("discogs-results"),s=document.getElementById("discogs-search-input");e&&!e.contains(t.target)&&t.target!==s&&e.classList.add("hidden");const o=document.getElementById("sku-results"),r=document.getElementById("sku-search");o&&!o.contains(t.target)&&t.target!==r&&o.classList.add("hidden")}))},async handleLogin(t){t.preventDefault();const e=t.target.email.value,s=t.target.password.value,o=document.getElementById("login-error"),r=document.getElementById("login-btn");o.classList.add("hidden"),r.disabled=!0,r.innerHTML="<span>Cargando...</span>";try{await re.signInWithEmailAndPassword(e,s)}catch(a){console.error("Login error:",a),o.innerText="Error: "+a.message,o.classList.remove("hidden"),r.disabled=!1,r.innerHTML='<span>Ingresar</span><i class="ph-bold ph-arrow-right"></i>'}},async updateFulfillmentStatus(t,e,s){var o,r,a;try{const n=((o=t==null?void 0:t.target)==null?void 0:o.closest("button"))||((a=(r=window.event)==null?void 0:r.target)==null?void 0:a.closest("button"));if(n){n.disabled=!0;const l=n.innerHTML;n.innerHTML='<i class="ph ph-circle-notch animate-spin"></i>'}await I.collection("sales").doc(e).update({fulfillment_status:s}),await this.loadData(),document.getElementById("modal-overlay")&&(document.getElementById("modal-overlay").remove(),this.openOnlineSaleDetailModal(e)),this.showToast("Estado de envío actualizado")}catch(n){console.error("Fulfillment update error:",n),this.showToast("Error al actualizar estado: "+n.message,"error")}},async manualShipOrder(t){var e,s,o,r,a,n;try{const l=prompt("Introduce el número de seguimiento:");if(!l)return;const i=((e=event==null?void 0:event.target)==null?void 0:e.closest("button"))||((o=(s=window.event)==null?void 0:s.target)==null?void 0:o.closest("button"));i&&(i.disabled=!0,i.innerHTML='<i class="ph ph-circle-notch animate-spin"></i> Guardando...');const d=await fetch(`${R}/api/manual-ship`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({orderId:t,trackingNumber:l})}),p=await d.json();if(d.ok&&p.success){if(this.showToast("✅ Pedido marcado como enviado"),p.emailSent)this.showToast("📧 Cliente notificado por email","success");else{const c=typeof p.emailError=="object"?JSON.stringify(p.emailError):p.emailError;this.showToast("⚠️ Pedido marcado pero EL EMAIL FALLÓ: "+c,"warning")}await this.loadData();const b=document.getElementById("sale-detail-modal");b&&(b.remove(),this.openUnifiedOrderDetailModal(t))}else throw new Error(p.error||p.message||"Error desconocido")}catch(l){console.error("Error shipping manually:",l),this.showToast("❌ Error: "+(l.message||"No se pudo procesar el envío"),"error");const i=((r=event==null?void 0:event.target)==null?void 0:r.closest("button"))||((n=(a=window.event)==null?void 0:a.target)==null?void 0:n.closest("button"));i&&(i.disabled=!1,i.innerHTML='<i class="ph-bold ph-truck"></i> Ingresar Tracking y Cerrar')}},async logout(){try{await re.signOut(),location.reload()}catch(t){console.error("Sign out error:",t),location.reload()}},setupListeners(){this._unsubscribeProducts&&this._unsubscribeProducts(),this._unsubscribeProducts=I.collection("products").onSnapshot(t=>{this.state.inventory=t.docs.map(e=>{const s=e.data();return{id:e.id,...s,condition:s.condition||"VG",owner:s.owner||"El Cuartito",label:s.label||"Desconocido",storageLocation:s.storageLocation||"Tienda",cover_image:s.cover_image||s.coverImage||null}}),(this.state.currentTab==="inventory"||this.state.currentTab==="dashboard")&&this.renderCurrentTab()},t=>{console.error("Inventory listener error:",t)})},async loadData(){try{const[t,e,s,o,r,a]=await Promise.all([I.collection("products").get(),I.collection("sales").get(),I.collection("expenses").get(),I.collection("events").orderBy("date","desc").get(),I.collection("consignors").get(),I.collection("extra_income").get()]);this.state.inventory=t.docs.map(n=>{const l=n.data();return{id:n.id,...l,condition:l.condition||"VG",owner:l.owner||"El Cuartito",label:l.label||"Desconocido",storageLocation:l.storageLocation||"Tienda",cover_image:l.cover_image||l.coverImage||null}}),this.state.sales=e.docs.map(n=>{var d,p;const l=n.data(),i={id:n.id,...l,date:l.date||((d=l.timestamp)!=null&&d.toDate?l.timestamp.toDate().toISOString().split("T")[0]:(p=l.created_at)!=null&&p.toDate?l.created_at.toDate().toISOString().split("T")[0]:new Date().toISOString().split("T")[0])};return l.total_amount!==void 0&&l.total===void 0&&(i.total=l.total_amount),l.payment_method&&!l.paymentMethod&&(i.paymentMethod=l.payment_method),i.items&&Array.isArray(i.items)&&(i.items=i.items.map(b=>({...b,priceAtSale:b.priceAtSale!==void 0?b.priceAtSale:b.unitPrice||0,qty:b.qty!==void 0?b.qty:b.quantity||1,costAtSale:b.costAtSale!==void 0?b.costAtSale:b.cost||0}))),i}).filter(n=>n.status!=="PENDING"&&n.status!=="failed").sort((n,l)=>{const i=new Date(n.date);return new Date(l.date)-i}),this.state.expenses=s.docs.map(n=>{var i;const l=n.data();return{id:n.id,...l,date:l.fecha_factura||l.date||((i=l.timestamp)==null?void 0:i.split("T")[0])||new Date().toISOString().split("T")[0]}}).sort((n,l)=>new Date(l.date)-new Date(n.date)),this.state.events=o.docs.map(n=>({id:n.id,...n.data()})),this.state.consignors=r.docs.map(n=>{const l=n.data();return{id:n.id,...l,agreementSplit:l.split||l.agreementSplit||(l.percentage?Math.round(l.percentage*100):70)}}),await this.loadInvestments(),this.state.extraIncome=a.docs.map(n=>({id:n.id,...n.data()})).sort((n,l)=>new Date(l.date)-new Date(n.date)),this.initFuse(),this.refreshCurrentView()}catch(t){console.error("Failed to load data:",t),this.showToast("❌ Error de conexión: "+t.message,"error")}},refreshCurrentView(){const t=document.getElementById("app-content");if(t)switch(this.state.currentView){case"dashboard":this.renderDashboard(t);break;case"inventory":this.renderInventory(t);break;case"sales":this.renderSales(t);break;case"onlineSales":this.renderOnlineSales(t);break;case"discogsSales":this.renderDiscogsSales(t);break;case"expenses":this.renderExpenses(t);break;case"consignments":this.renderConsignments(t);break;case"backup":this.renderBackup(t);break;case"settings":this.renderSettings(t);break;case"calendar":this.renderCalendar(t);break;case"shipping":this.renderShipping(t);break;case"pickups":this.renderPickups(t);break;case"investments":this.renderInvestments(t);break;case"vatReport":this.renderVATReport(t);break;case"datosLegales":this.renderDatosLegales(t);break;case"contabilidad":this.renderContabilidad(t);break;case"facturasManual":this.renderFacturasManual(t);break;case"extraIncome":this.renderExtraIncome(t);break}},renderDatosLegales(t){const e=`
        ...
        `;t.innerHTML=e},renderContabilidad(t){const e=new Date().getFullYear(),s=Math.floor(new Date().getMonth()/3)+1;this.state.contabilidadYear||(this.state.contabilidadYear=e),this.state.contabilidadQuarter||(this.state.contabilidadQuarter=s),this.state.contabilidadInvoices||(this.state.contabilidadInvoices=[]),this.state.contabilidadLoading||(this.state.contabilidadLoading=!1);const o=this.state.contabilidadYear,r=this.state.contabilidadQuarter,a=this.state.contabilidadInvoices,n=this.state.contabilidadLoading,l=d=>{const p={local:"bg-emerald-100 text-emerald-700",online:"bg-blue-100 text-blue-700",discogs:"bg-purple-100 text-purple-700"},b={local:"Tienda",online:"Webshop",discogs:"Discogs"};return`<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold ${p[d]||"bg-slate-100 text-slate-600"}">${b[d]||d}</span>`},i=`
            <div class="max-w-6xl mx-auto px-4 md:px-8 pb-24 md:pb-8 pt-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div class="kpi-card">
                        ...
                        <div class="text-2xl font-bold text-brand-orange">${this.formatCurrency(a.reduce((d,p)=>d+(p.totalAmount||0),0))}</div>
                    </div>
                    <div class="kpi-card">
                ...
                <div class="flex flex-col lg:flex-row gap-8 h-[calc(100vh-140px)]">
                    ...
                    <div class="flex-1 bg-white rounded-2xl shadow-sm border border-orange-100 p-6 flex flex-col">
                        ...
                        <div class="grid grid-cols-7 gap-2 flex-1 auto-rows-fr">
                            ...
                            ${Array.from({length:n},(b,c)=>{const v=c+1,m=`${s}-${String(o+1).padStart(2,"0")}-${String(v).padStart(2,"0")}`,g=e.getDate()===v,w=d(v),u=new Date().toDateString()===new Date(s,o,v).toDateString();return`
...
¿Estás seguro?`))return;if(prompt("Para confirmar, ingresa la contraseña de administrador:")!=="alejo13"){alert("Contraseña incorrecta. Operación cancelada.");return}this.showToast("Borrando todas las ventas..."),I.collection("sales").get().then(e=>{const s=I.batch();return e.docs.forEach(o=>{s.delete(o.ref)}),s.commit()}).then(()=>{this.showToast("✅ Todas las ventas han sido eliminadas"),setTimeout(()=>location.reload(),1500)}).catch(e=>{console.error(e),alert("Error al borrar ventas: "+e.message)})},async findProductBySku(t){try{const e=await I.collection("products").where("sku","==",t).get();if(e.empty)return null;const s=e.docs[0];return{id:s.id,ref:s.ref,data:s.data()}}catch(e){return console.error("Error finding product by SKU:",e),null}},logInventoryMovement(t,e){let s="";t==="EDIT"?s="Producto actualizado":t==="ADD"?s="Ingreso de inventario":t==="DELETE"?s="Egreso manual":t==="SOLD"&&(s="Venta registrada"),I.collection("inventory_logs").add({type:t,sku:e.sku||"Unknown",album:e.album||"Unknown",artist:e.artist||"Unknown",timestamp:firebase.firestore.FieldValue.serverTimestamp(),details:s}).catch(o=>console.error("Error logging movement:",o))},openInventoryLogModal(){I.collection("inventory_logs").orderBy("timestamp","desc").limit(50).get().then(t=>{const s=`
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-left">
                                    <tbody class="divide-y divide-slate-50">
                                        ${i.map(h=>{const k=h.type==="sale",K=k?h.album||"Venta de Items":h.proveedor||h.description||"Gasto registrado",B=k?h.channel||"Tienda Local":h.categoria||"Operativo";let L="ph-receipt";if(k){const A=(h.channel||"").toLowerCase();A.includes("web")&&(L="ph-globe-simple"),A.includes("discogs")&&(L="ph-vinyl-record")}else L="ph-credit-card";return`
        ...
        `;t.innerHTML=Be,this.renderDashboardCharts(n,d,p);const Ie=(e=document.getElementById("genreDonutChart"))==null?void 0:e.getContext("2d");if(Ie&&ne.length>0){this.genreDonutChartInstance&&this.genreDonutChartInstance.destroy();const h=ne.map(B=>B[0]),k=ne.map(B=>B[1]),K=ne.map((B,L)=>le[L%le.length]);this.genreDonutChartInstance=new Chart(Ie,{type:"doughnut",data:{labels:h,datasets:[{data:k,backgroundColor:K,borderWidth:0,hoverOffset:8}]},options:{responsive:!0,maintainAspectRatio:!1,cutout:"62%",plugins:{legend:{position:"bottom",labels:{boxWidth:12,boxHeight:12,borderRadius:3,useBorderRadius:!0,padding:14,font:{size:11,weight:"600",family:"'DM Sans', sans-serif"},color:"#334155"}},tooltip:{backgroundColor:"#1e293b",titleFont:{size:11,weight:"700"},bodyFont:{size:13,weight:"700"},padding:14,cornerRadius:12,callbacks:{label:B=>{const L=B.dataset.data.reduce((oe,me)=>oe+me,0),A=(B.parsed/L*100).toFixed(1);return` ${B.parsed} uds — ${A}%`}}}}},plugins:[{id:"centerText",beforeDraw(B){const{ctx:L,chartArea:A}=B;if(!A)return;L.save();const oe=(A.left+A.right)/2,me=(A.top+A.bottom)/2,ce=Math.min(A.right-A.left,A.bottom-A.top)/7;L.font=`bold ${ce}px 'DM Sans', sans-serif`,L.textBaseline="middle",L.textAlign="center",L.fillStyle="#1e293b";const we=B.data.datasets[0].data.reduce((X,pe)=>X+pe,0);L.fillText(we,oe,me-ce*.35),L.font=`600 ${ce*.42}px 'DM Sans', sans-serif`,L.fillStyle="#94a3b8",L.fillText("unidades",oe,me+ce*.55),L.restore()}}]})}const Ce=(s=document.getElementById("genreBarChart"))==null?void 0:s.getContext("2d");if(Ce&&ne.length>0){this.genreBarChartInstance&&this.genreBarChartInstance.destroy();const h=ne.map(B=>B[0]),k=ne.map(B=>B[1]),K=ne.map((B,L)=>le[L%le.length]);this.genreBarChartInstance=new Chart(Ce,{type:"bar",data:{labels:h,datasets:[{label:"Unidades",data:k,backgroundColor:K.map(B=>B+"30"),borderColor:K,borderWidth:2,borderRadius:8,borderSkipped:!1,barThickness:28}]},options:{responsive:!0,maintainAspectRatio:!1,indexAxis:"y",plugins:{legend:{display:!1},tooltip:{backgroundColor:"#1e293b",titleFont:{size:11,weight:"700"},bodyFont:{size:13,weight:"700"},padding:14,cornerRadius:12,callbacks:{label:B=>` ${B.parsed.x} unidades vendidas`}}},scales:{x:{beginAtZero:!0,grid:{color:"#f1f5f9"},ticks:{font:{size:10,weight:"600"},color:"#94a3b8"}},y:{grid:{display:!1},ticks:{font:{size:11,weight:"700",family:"'DM Sans', sans-serif"},color:"#334155",padding:8}}}}})}}catch(o){console.error("Dashboard render error:",o),t.innerHTML=`<div class="p-12 text-center text-red-500 font-bold bg-red-50 rounded-3xl m-8 border border-red-100">
    ...
    `},renderInventoryContent(t,e,s,o,r){t.innerHTML=`
            ...
            `}
        `},renderInventory(t){const e=[...new Set(this.state.inventory.flatMap(g=>{const w=[g.genre,g.genre2,g.genre3,g.genre4,g.genre5].filter(Boolean),u=[];w.forEach(E=>{u.push(...E.split(",").map(C=>C.trim()).filter(Boolean))});const f=[...new Set(u)],$=f.filter(E=>E.toLowerCase()!=="electronic");return $.length>0?$:f.length>0?f:["Otros"]}))].sort(),s=[...new Set(this.state.inventory.map(g=>g.owner).filter(Boolean))].sort(),o=[...new Set(this.state.inventory.map(g=>g.label).filter(Boolean))].sort(),r=[...new Set(this.state.inventory.map(g=>g.storageLocation).filter(Boolean))].sort(),a=this.getFilteredInventory(),n=this.state.sortBy||"dateDesc";a.sort((g,w)=>{if(n==="priceDesc")return(w.price||0)-(g.price||0);if(n==="priceAsc")return(g.price||0)-(w.price||0);if(n==="stockDesc")return(w.stock||0)-(g.stock||0);const u=g.created_at?g.created_at.seconds?g.created_at.seconds*1e3:new Date(g.created_at).getTime():0,f=w.created_at?w.created_at.seconds?w.created_at.seconds*1e3:new Date(w.created_at).getTime():0;return n==="dateDesc"?f-u:n==="dateAsc"?u-f:0});const l=this.state.inventory.length,i=this.state.inventory.reduce((g,w)=>{const u=Number(w.stock)||0;return g+(u>0?(parseFloat(w.price)||0)*u:0)},0),d=this.state.inventory.filter(g=>(g.stock||0)>0).length,p=this.state.inventory.filter(g=>g.discogs_listing_id).length,b=[this.state.filterGenre!=="all"?1:0,this.state.filterOwner!=="all"?1:0,this.state.filterLabel!=="all"?1:0,this.state.filterStorage!=="all"?1:0,this.state.filterDiscogs&&this.state.filterDiscogs!=="all"?1:0,this.state.filterHero&&this.state.filterHero!=="all"?1:0].reduce((g,w)=>g+w,0);document.getElementById("inventory-layout-root")||(t.innerHTML=`
    ...
    `);const c=document.getElementById("inventory-kpi-container");c&&(c.innerHTML=`
                <div class="kpi-card">
                ...
                <div class="kpi-card">
                    ...
                    <p class="text-xl font-bold text-brand-orange font-display mt-1">${this.formatCurrency(i)}</p>
                </div>
                <div class="kpi-card">
                ...
                <div class="kpi-card">
                    ...
                    <p class="text-xl font-bold text-purple-600 font-display mt-1">${p} <span class="text-xs text-slate-400 font-normal">/ ${l}</span></p>
                </div>
                ...
                <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-6 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    ...
                    <div class="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                        ...
                        <div class="flex flex-wrap gap-1">
                            ${["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"].map((u,f)=>`
                                <button onclick="app.toggleMonthFilter(${f})" 
                                    class="px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all ${l.includes(f)?"bg-brand-dark text-white":"bg-slate-50 text-slate-400 hover:bg-slate-100"}">
                                    ${u}
                                </button>
                            `).join("")}
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-slate-50/50 rounded-3xl p-6 border border-slate-100">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ${["El Cuartito",...this.state.consignors.map(u=>u.name)].map(u=>{const f=this.state.inventory.filter($=>$.owner===u).reduce(($,E)=>$+E.stock,0);return`
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-3 max-h-[80vh] overflow-y-auto pr-2 custom-scrollbar pb-10">
                            ${c.map(u=>{const f=u.status==="shipped",$=u.status==="paid"||u.source==="STORE"||u.paymentMethod!=="Pending",E=u.channel==="Discogs",C=u.source==="STORE",D=u.items&&u.items.length>0?u.items[0]:{album:u.album||"Venta Manual",artist:u.artist||"Desconocido"},y=u.items&&u.items.length>1?u.items.length-1:0;return`
                                <div class="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm hover:border-slate-200 transition-all cursor-pointer group flex items-center gap-4 relative" onclick="app.openUnifiedOrderDetailModal('${u.id}')">
                                    <!-- Source Icon -->
                                    <div class="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${E?"bg-slate-900 text-white":C?"bg-orange-100 text-brand-orange":"bg-blue-100 text-blue-600"}">
                                        <i class="ph-bold ${E?"ph-disc":C?"ph-storefront":"ph-globe"} text-xl"></i>
                                    </div>
        ...
        `;if(t.innerHTML=g,this.state.salesHistorySearch){const u=document.getElementById("sales-history-search");if(u){u.focus();const f=u.value;u.value="",u.value=f}}},renderSalesCartWidget(){return`
    ...
    <div id="modal-overlay" class="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
                                    <table class="w-full text-sm">
                                        ...
                                        <tbody class="divide-y divide-slate-50">
                                            ${(e.items||[]).map(u=>{var f,$,E,C,D;return`
                                                <tr>
                                                    <td class="px-4 py-4">
                                                        <div class="flex items-center gap-3">
                                                            <img src="${u.image||u.cover_image||((f=u.record)==null?void 0:f.cover_image)||"https://elcuartito.dk/default-vinyl.png"}" class="w-10 h-10 rounded-lg object-cover shadow-sm">
                                                            <div>
                                                    ...
                                                    <td class="px-4 py-4 text-center font-mono text-xs text-slate-400">${u.sku||((C=u.record)==null?void 0:C.sku)||"-"}</td>
                                                    <td class="px-4 py-4 text-center font-medium">${u.quantity||u.qty||1}</td>
                                                    <td class="px-4 py-4 text-right font-bold text-brand-dark">${this.formatCurrency(u.unitPrice||u.priceAtSale||((D=u.record)==null?void 0:D.price)||0)}</td>
                                                </tr>
                                            `}).join("")}
                                        </tbody>
                                    </table>
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="relative pl-4 border-l-2 border-slate-100 space-y-6">
                                    ${a.map((u,f)=>{const $=n(u.status);return`
                                            <div class="relative">
        ...
        `;document.body.insertAdjacentHTML("beforeend",c)},openInvoiceModal(t){var f;const e=this.state.sales.find($=>$.id===t);if(!e){this.showToast("Sale not found","error");return}const s=e.items||[],o=(f=e.date)!=null&&f.toDate?e.date.toDate():new Date(e.date||e.timestamp),r=o.toISOString().slice(0,10).replace(/-/g,""),a=e.invoiceNumber||`ECR-${r}-${t.slice(-4).toUpperCase()}`,n=s.filter($=>$.productCondition==="New"),l=s.filter($=>$.productCondition!=="New");let i=0,d=0;const p=($,E)=>$.map(C=>{const D=C.priceAtSale||C.price||0,y=C.qty||C.quantity||1,S=D*y;if(d+=S,E){const _=S*.2;return i+=_,`
                        <tr>
                    ...
                    `}).join("");let b="";n.length>0&&l.length>0?b=`
                ...
                ${p(l,!1)}
            `:b=p(n,!0)+p(l,!1);const c=parseFloat(e.shipping_income||e.shipping||e.shipping_cost||0),v=c*.2,m=d+c,g=e.customer?`${e.customer.firstName||""} ${e.customer.lastName||""}`.trim():e.customerName||"Customer",w=e.customer?`${e.customer.address||""}<br>${e.customer.postalCode||""} ${e.customer.city||""}<br>${e.customer.country||""}`:"",u=`
        ...
        `),e.document.close()},navigateInventoryFolder(t,e){t==="genre"&&(this.state.filterGenre=e),t==="owner"&&(this.state.filterOwner=e),t==="label"&&(this.state.filterLabel=e),t==="storage"&&(this.state.filterStorage=e),this.refreshCurrentView()},toggleSelection(t){this.state.selectedItems.has(t)?this.state.selectedItems.delete(t):this.state.selectedItems.add(t),this.refreshCurrentView()},async openPrintLabelModal(t){const e=this.state.inventory.find(n=>n.id===t||n.sku===t);if(!e)return;let s={...e};try{const n=await I.collection("products").doc(e.id).get();n.exists&&(s={...e,...n.data()})}catch(n){console.warn("[printLabel] Could not fetch fresh product data, using state copy",n)}const o=s.year&&Number(s.year)!==0?String(s.year):"—",r=s.price?Number(s.price).toLocaleString("da-DK"):"—",a=`
...
`;document.body.insertAdjacentHTML("beforeend",a),setTimeout(()=>{const n=document.getElementById("qr-container");n&&typeof QRCode<"u"&&(n.innerHTML="",new QRCode(n,{text:s.sku,width:57,height:57,colorDark:"#000000",colorLight:"#ffffff",correctLevel:QRCode.CorrectLevel.M}))},50)},closePrintLabelModal(){const t=document.getElementById("print-label-modal");if(t){const e=document.getElementById("label-comment");e&&(e.value=""),t.remove()}},setLabelOrientation(t){const e=document.getElementById("print-label-modal");if(!e)return;e.dataset.orientation=t;const s=document.getElementById("orient-h"),o=document.getElementById("orient-v"),r="flex-1 py-2 bg-white text-brand-dark font-bold rounded-lg text-sm shadow-sm flex items-center justify-center gap-1.5 transition-all",a="flex-1 py-2 text-slate-500 font-bold rounded-lg text-sm flex items-center justify-center gap-1.5 transition-all";s&&(s.className=t==="landscape"?r:a),o&&(o.className=t==="portrait"?r:a);const n=document.getElementById("printable-label");n&&n.classList.toggle("label-b--portrait",t==="portrait")},async confirmPrintLabel(){const t=document.getElementById("label-comment"),e=document.getElementById("preview-comment"),s=t?t.value:"";t&&e&&(e.innerText=s);const o=document.querySelector('#print-label-modal button[onclick="app.confirmPrintLabel()"]');o&&(o.disabled=!0,o.innerHTML='<i class="ph-bold ph-circle-notch animate-spin"></i> Imprimiendo…');try{const r=document.getElementById("print-label-modal"),a=r&&r.dataset.orientation||"landscape",l=(await this._drawLabelCanvas(s,a)).toDataURL("image/png").split(",")[1],i=await fetch(`${R}/api/print-label`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({image:l})}),d=await i.json();if(!i.ok)throw new Error(d.error||"Error desconocido");o&&(o.innerHTML='<i class="ph-bold ph-check"></i> ¡Enviado!',o.classList.replace("bg-brand-dark","bg-green-600")),this.showToast("✅ Etiqueta enviada a la impresora"),setTimeout(()=>this.closePrintLabelModal(),1500)}catch(r){o&&(o.disabled=!1,o.innerHTML='<i class="ph-bold ph-printer"></i> Imprimir'),this.showToast("Error al imprimir: "+r.message,"error")}},async _drawLabelCanvas(t,e="landscape"){const s=document.getElementById("print-label-modal"),o=s?s.dataset.sku:null,r=o?this.state.inventory.find(w=>w.id===o||w.sku===o)||{}:{},a=(w,u)=>{const f=document.getElementById(w);return f&&f.value.trim()?f.value.trim():u},n={...r,album:a("label-edit-title",r.album),artist:a("label-edit-artist",r.artist),year:a("label-edit-year",r.year),price:a("label-edit-price",r.price),genre:a("label-edit-genre1",r.genre),genre2:a("label-edit-genre2",r.genre2),condition:a("label-edit-cond",r.condition),storageLocation:a("label-edit-loc",r.storageLocation)},l=300/25.4,i=e==="portrait",d=Math.round((i?40:62)*l),p=Math.round((i?62:40)*l),b=document.createElement("canvas");b.width=d,b.height=p;const c=b.getContext("2d");"wordSpacing"in c&&(c.wordSpacing="0px"),"letterSpacing"in c&&(c.letterSpacing="0px"),c.fillStyle="#ffffff",c.fillRect(0,0,d,p);const v=Math.round(5.5*l);c.fillStyle="#000000",c.fillRect(0,0,d,v);const m=Math.round(2.2*l);c.fillStyle="#ffffff",c.font=`800 ${m}px "DM Sans", Arial, sans-serif`,c.textBaseline="middle",c.textAlign="left";const g=((n.genre||"VINYL")+(n.genre2?" / "+n.genre2:"")).toUpperCase();c.fillText(g,Math.round(1.6*l),v/2);try{const w=await new Promise(u=>{const f=new Image;f.crossOrigin="anonymous",f.onload=()=>u(f),f.onerror=()=>u(null),f.src="logo-broadsheet.png"});if(w){const u=Math.round(3*l),f=Math.round(w.naturalWidth*(u/w.naturalHeight)),$=document.createElement("canvas");$.width=w.naturalWidth,$.height=w.naturalHeight;const E=$.getContext("2d");E.drawImage(w,0,0);const C=E.getImageData(0,0,$.width,$.height),D=C.data;for(let V=0;V<D.length;V+=4)D[V+3]>0&&(D[V]=255,D[V+1]=255,D[V+2]=255);E.putImageData(C,0,0);const S=Math.round(18*l),_=d-S+Math.round((S-f)/2),M=(v-u)/2;c.drawImage($,_,M,f,u)}}catch{}if(i){const w=Math.round(1.6*l),u=d-w*2;c.textAlign="left",c.textBaseline="top";const f=Math.round(3.8*l);c.font=`800 ${f}px "DM Sans", Arial, sans-serif`,c.fillStyle="#000000";const $=this._wrapText(c,n.album||"",u,2);let E=v+Math.round(1.1*l);$.forEach((z,F)=>{c.fillText(z,w,E+F*Math.round(f*1.1))}),E+=$.length*Math.round(f*1.1);const C=Math.round(2.4*l);if(c.font=`600 ${C}px "DM Sans", Arial, sans-serif`,c.fillStyle="rgba(0,0,0,0.5)",E+=Math.round(.5*l),c.fillText(this._truncateText(c,n.artist||"",u),w,E),E+=C,n.label&&n.label!=="Desconocido"){const z=Math.round(2.2*l);E+=Math.round(.6*l),c.font=`700 ${Math.round(1.9*l)}px "DM Sans", Arial, sans-serif`,c.fillStyle="rgba(0,0,0,0.35)",c.fillText("LABEL",w,E);const F=c.measureText("LABEL ").width;c.font=`600 ${z}px "DM Sans", Arial, sans-serif`,c.fillStyle="#333333",c.fillText(this._truncateText(c,n.label,u-F),w+F,E),E+=z}if(t){const z=Math.round(2.1*l);c.font=`italic 600 ${z}px "DM Sans", Arial, sans-serif`,c.fillStyle="rgba(0,0,0,0.4)",c.textBaseline="top";const F=u-Math.round(3*l),q=this._wrapText(c,t,F,5),ue=Math.round(z*1.35),ne=E+Math.round(1.8*l);q.forEach((ve,ye)=>{c.fillText(ve,w+Math.round(1.5*l),ne+ye*ue)})}const D=Math.round(12*l),y=Math.round(20*l),S=Math.round((d-y)/2),_=p-D-Math.round(1.6*l),M=_-Math.round(4.5*l),V=M-Math.round(2.2*l);c.strokeStyle="rgba(0,0,0,0.15)",c.lineWidth=1,c.beginPath(),c.moveTo(w,M),c.lineTo(d-w,M),c.stroke();const Y=Math.round(2*l),J=Math.round(1.8*l),G=u/3;c.textBaseline="middle";const Z=(z,F,q)=>{c.textAlign="left",c.font=`700 ${J}px "DM Sans", Arial, sans-serif`,c.fillStyle="rgba(0,0,0,0.45)",c.fillText(z+" ",q,V);const ue=c.measureText(z+" ").width;c.font=`700 ${Y}px "DM Mono", "Courier New", monospace`,c.fillStyle="rgba(0,0,0,0.7)",c.fillText(F,q+ue,V)};Z("Loc",n.storageLocation||"—",w),Z("Cond",n.condition||"—",w+G),Z("Year",n.year&&Number(n.year)!==0?String(n.year):"—",w+G*2);const Q=Math.round(13*l),ee=Math.round((d-Q)/2),te=Math.round(1.9*l),x=E+Math.round(2.5*l),T=V-Math.round(te+Math.round(.4*l)+4),O=x+Math.max(0,Math.round((T-x-Q)/2)),se=document.getElementById("qr-container"),ae=se?se.querySelector("canvas"):null;ae?c.drawImage(ae,ee,O,Q,Q):(c.strokeStyle="#ccc",c.lineWidth=1,c.strokeRect(ee,O,Q,Q)),c.font=`600 ${te}px "DM Mono", "Courier New", monospace`,c.fillStyle="rgba(0,0,0,0.45)",c.textAlign="center",c.textBaseline="top",c.fillText(n.sku||"",d/2,O+Q+Math.round(.4*l)),c.fillStyle="#000000";const j=Math.round(.8*l);c.beginPath(),c.moveTo(S+j,_),c.lineTo(S+y-j,_),c.quadraticCurveTo(S+y,_,S+y,_+j),c.lineTo(S+y,_+D-j),c.quadraticCurveTo(S+y,_+D,S+y-j,_+D),c.lineTo(S+j,_+D),c.quadraticCurveTo(S,_+D,S,_+D-j),c.lineTo(S,_+j),c.quadraticCurveTo(S,_,S+j,_),c.closePath(),c.fill();const ie=n.price?Number(n.price).toLocaleString("da-DK"):"—",de=Math.round(4.8*l),H=Math.round(2.2*l),W=de*1.1,N=H*1.1,P=_+(D-W-N)/2+W/2,U=P+W/2+N/2;c.textAlign="center",c.textBaseline="middle",c.font=`800 ${de}px "DM Mono", "Courier New", monospace`,c.fillStyle="#ffffff",c.fillText(ie,S+y/2,P),c.font=`700 ${H}px "DM Sans", Arial, sans-serif`,c.fillStyle="#ffffff",c.fillText("DKK",S+y/2,U)}else{const w=v,u=Math.round(18*l),f=d-u,$=Math.round(1.6*l),E=Math.round(1.1*l);c.textAlign="left",c.textBaseline="top";const C=Math.round(3.8*l);c.font=`800 ${C}px "DM Sans", Arial, sans-serif`,c.fillStyle="#000000";const D=f-$-Math.round(.95*l),y=this._wrapText(c,n.album||"",D,2);y.forEach((be,le)=>{c.fillText(be,$,w+E+le*Math.round(C*1.1))});const S=Math.round(2.4*l);c.font=`600 ${S}px "DM Sans", Arial, sans-serif`,c.fillStyle="rgba(0,0,0,0.5)";const _=w+E+y.length*Math.round(C*1.1)+Math.round(.5*l),M=this._truncateText(c,n.artist||"",D);c.fillText(M,$,_);let V=_+S;if(n.label&&n.label!=="Desconocido"){const be=Math.round(2.2*l),le=V+Math.round(.6*l);c.font=`700 ${Math.round(1.9*l)}px "DM Sans", Arial, sans-serif`,c.fillStyle="rgba(0,0,0,0.35)",c.fillText("LABEL",$,le),c.font=`600 ${be}px "DM Sans", Arial, sans-serif`,c.fillStyle="#333333";const he=$+c.measureText("LABEL ").width;c.fillText(this._truncateText(c,n.label,D-he+$),he,le),V=le+be}const Y=p-Math.round(5*l);if(t){const be=Math.round(2.1*l);c.font=`italic 600 ${be}px "DM Sans", Arial, sans-serif`,c.fillStyle="rgba(0,0,0,0.4)",c.textBaseline="top";const le=D-Math.round(2*l),he=this._wrapText(c,t,le,5),$e=Math.round(be*1.35),Me=he.length*$e,Be=V+(Y-V-Me)/2;he.forEach((Ie,Ce)=>{c.fillText(Ie,$+Math.round(1.5*l),Be+Ce*$e)})}const J=Math.round(2*l),G=Math.round(1.8*l),Z=p-Math.round(2.5*l),ee=(f-$-Math.round(.95*l))/3;c.strokeStyle="rgba(0,0,0,0.15)",c.lineWidth=1,c.beginPath(),c.moveTo($,Y),c.lineTo(f-Math.round(.95*l),Y),c.stroke(),c.textBaseline="middle",c.textAlign="left";const te=(be,le,he)=>{c.font=`700 ${G}px "DM Sans", Arial, sans-serif`,c.fillStyle="rgba(0,0,0,0.45)",c.fillText(be+" ",he,Z);const $e=c.measureText(be+" ").width;c.font=`700 ${J}px "DM Mono", "Courier New", monospace`,c.fillStyle="rgba(0,0,0,0.7)",c.fillText(le,he+$e,Z)};te("Loc",n.storageLocation||"—",$),te("Cond",n.condition||"—",$+ee),te("Year",n.year&&Number(n.year)!==0?String(n.year):"—",$+ee*2);const x=f;c.strokeStyle="rgba(0,0,0,0.12)",c.lineWidth=1,c.beginPath(),c.moveTo(x,w),c.lineTo(x,p),c.stroke();const T=document.getElementById("qr-container"),O=T?T.querySelector("canvas"):null,ae=Math.round(15*l),j=x+Math.round((u-ae)/2),ie=w+Math.round(1.1*l);O?c.drawImage(O,j,ie,ae,ae):(c.strokeStyle="#ccc",c.strokeRect(j,ie,ae,ae));const de=Math.round(1.9*l);c.font=`600 ${de}px "DM Mono", "Courier New", monospace`,c.fillStyle="rgba(0,0,0,0.45)",c.textAlign="center",c.textBaseline="top",c.fillText(n.sku||"",x+u/2,ie+ae+Math.round(.4*l));const H=Math.round(12*l),W=Math.round(15*l),N=x+Math.round((u-W)/2),P=p-H-Math.round(1.6*l);c.fillStyle="#000000";const U=Math.round(.8*l);c.beginPath(),c.moveTo(N+U,P),c.lineTo(N+W-U,P),c.quadraticCurveTo(N+W,P,N+W,P+U),c.lineTo(N+W,P+H-U),c.quadraticCurveTo(N+W,P+H,N+W-U,P+H),c.lineTo(N+U,P+H),c.quadraticCurveTo(N,P+H,N,P+H-U),c.lineTo(N,P+U),c.quadraticCurveTo(N,P,N+U,P),c.closePath(),c.fill();const z=n.price?Number(n.price).toLocaleString("da-DK"):"—",F=Math.round(4.8*l),q=Math.round(2*l),ue=F*1.1,ne=q*1.1,ve=ue+ne,ye=P+(H-ve)/2+ue/2,Te=ye+ue/2+ne/2;c.textAlign="center",c.textBaseline="middle",c.font=`800 ${F}px "DM Mono", "Courier New", monospace`,c.fillStyle="#ffffff",c.fillText(z,N+W/2,ye);const De=Math.round(2.2*l);c.font=`700 ${De}px "DM Sans", Arial, sans-serif`,c.fillStyle="#ffffff",c.fillText("DKK",N+W/2,Te)}return b},_wrapText(t,e,s,o){const r=e.split(" "),a=[];let n="";for(const l of r){const i=n?n+" "+l:l;if(t.measureText(i).width>s&&n){if(a.push(n),n=l,a.length>=o)break}else n=i}if(n&&a.length<o&&a.push(n),a.length>0){const l=a[a.length-1];a[a.length-1]=this._truncateText(t,l,s)}return a},_truncateText(t,e,s){if(t.measureText(e).width<=s)return e;let o=e;for(;o.length>1&&t.measureText(o+"…").width>s;)o=o.slice(0,-1);return o+"…"},initFuse(){if(typeof Fuse>"u"){console.warn("Fuse.js not loaded yet");return}const t={keys:[{name:"artist",weight:.35},{name:"album",weight:.25},{name:"label",weight:.15},{name:"storageLocation",weight:.15},{name:"sku",weight:.1},{name:"quickId",weight:.1},{name:"genre",weight:.03},{name:"notes",weight:.02}],threshold:.4,distance:100,ignoreLocation:!0,minMatchCharLength:2};this.fuse=new Fuse(this.state.inventory,t)},getFilteredInventory(){const t=(this.state.inventorySearch||"").trim().toLowerCase(),e=this.state.filterGenre||"all",s=this.state.filterOwner||"all",o=this.state.filterLabel||"all",r=this.state.filterStorage||"all",a=this.state.filterDiscogs||"all",n=this.state.filterHero||"all";let l=this.state.inventory;if(t.length>=2)if(this.fuse)l=this.fuse.search(t).map(i=>i.item);else{const i=t.split(" ").filter(d=>d.length>0);l=l.filter(d=>i.every(p=>(d.artist||"").toLowerCase().includes(p)||(d.album||"").toLowerCase().includes(p)||(d.label||"").toLowerCase().includes(p)||(d.storageLocation||"").toLowerCase().includes(p)||(d.genre||"").toLowerCase().includes(p)||(d.notes||"").toLowerCase().includes(p)||(d.sku||"").toLowerCase().includes(p)))}return l.filter(i=>{const d=[i.genre,i.genre2,i.genre3,i.genre4,i.genre5].filter(Boolean),p=[];d.forEach(S=>{p.push(...S.split(",").map(_=>_.trim()).filter(Boolean))});const b=[...new Set(p)],c=b.filter(S=>S.toLowerCase()!=="electronic"),v=c.length>0?c:b.length>0?b:["Otros"],m=e==="all"||v.includes(e),g=s==="all"||i.owner===s,w=o==="all"||i.label===o,u=r==="all"||i.storageLocation===r,f=!!i.discogs_listing_id,$=a==="all"||a==="yes"&&f||a==="no"&&!f,E=i.tags&&(Array.isArray(i.tags),i.tags.includes("hero")),C=n==="all"||n==="yes"&&E||n==="no"&&!E,D=this.getTimeInStockCategory(i.created_at||null),y=this.state.filterStockTime.length===0||this.state.filterStockTime.includes(D);return m&&g&&w&&u&&$&&C&&y})},toggleSelectAll(){const t=this.getFilteredInventory();t.length>0&&t.every(e=>this.state.selectedItems.has(e.sku))?t.forEach(e=>this.state.selectedItems.delete(e.sku)):t.forEach(e=>this.state.selectedItems.add(e.sku)),this.refreshCurrentView()},addSelectionToCart(){this.state.selectedItems.forEach(t=>{const e=this.state.inventory.find(s=>s.id===t||s.sku===t);e&&e.stock>0&&(this.state.cart.find(s=>s.sku===t)||this.state.cart.push(e))}),this.state.selectedItems.clear(),this.showToast(`${this.state.cart.length} items agregados al carrito`),this.refreshCurrentView()},deleteSelection(){if(!confirm(`¿Estás seguro de eliminar ${this.state.selectedItems.size} productos ? `))return;const t=I.batch(),e=[];this.state.selectedItems.forEach(s=>{const o=I.collection("products").doc(s),r=this.state.inventory.find(a=>a.id===s||a.sku===s);r&&e.push(r),t.delete(o)}),t.commit().then(()=>{this.showToast("Productos eliminados"),e.forEach(s=>this.logInventoryMovement("DELETE",s)),this.state.selectedItems.clear()}).catch(s=>{console.error("Error logging movement:",s),alert("Error al eliminar")})},openAddExpenseModal(){const t=["Alquiler","Servicios","Marketing","Suministros","Honorarios"],s=`
    <div id="modal-overlay" class="fixed inset-0 bg-brand-dark/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" >
                ...
                `:`
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody>
                                ${e.map(a=>{var l;const n=(l=a.timestamp)!=null&&l.toDate?a.timestamp.toDate():new Date(a.date||0);return{...a,_sortDate:n.getTime()}}).sort((a,n)=>n._sortDate-a._sortDate).map(a=>{var v,m,g,w,u,f,$;const n=a.customer||{},l=a.orderNumber||"N/A",i=(v=a.timestamp)!=null&&v.toDate?a.timestamp.toDate():new Date(a.date),p=((m=a.completed_at)!=null&&m.toDate?a.completed_at.toDate():null)||i,b={completed:"bg-green-50 text-green-700 border-green-200",PENDING:"bg-yellow-50 text-yellow-700 border-yellow-200",failed:"bg-red-50 text-red-700 border-red-200"},c={completed:"✅ Completado",PENDING:"⏳ Pendiente",failed:"❌ Fallido"};return`
                                        <tr class="border-b border-slate-50 hover:bg-slate-50 transition-colors cursor-pointer" onclick="app.openUnifiedOrderDetailModal('${a.id}')">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="font-semibold text-brand-dark">${n.name||(n.firstName?`${n.firstName} ${n.lastName||""}`:"")||((g=n.stripe_info)==null?void 0:g.name)||"Cliente"}</div>
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="text-sm text-slate-600 truncate max-w-[200px]">
                                                    ${((u=n.shipping)==null?void 0:u.line1)||n.address||(($=(f=n.stripe_info)==null?void 0:f.shipping)==null?void 0:$.line1)||"Sin dirección"}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4">
                                                <div class="text-sm">
    ...
    `},openOnlineSaleDetailModal(t){var i,d,p;const e=this.state.sales.find(b=>b.id===t);if(!e)return;const s=e.customer||{},o=s.stripe_info||{},r=s.shipping||o.shipping||{},a={line1:r.line1||s.address||"Sin dirección",line2:r.line2||"",city:r.city||s.city||"",postal:r.postal_code||s.postalCode||"",country:r.country||s.country||"Denmark"},n=`
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="bg-slate-50 p-5 rounded-2xl border border-slate-100 space-y-4 text-sm text-brand-dark">
                                ...
                                <div class="flex justify-between items-center pb-2 border-b border-slate-200/50">
                                    ...
                                    <span class="font-bold">${new Date((d=e.timestamp)!=null&&d.toDate?e.timestamp.toDate():(p=e.completed_at)!=null&&p.toDate?e.completed_at.toDate():e.date).toLocaleDateString("es-ES",{day:"numeric",month:"long",year:"numeric"})}</span>
                                </div>
                                <div class="space-y-1">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-white border border-slate-100 rounded-2xl overflow-hidden">
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-slate-50">
                                    ${(e.items||[]).map(b=>{var c,v,m;return`
                                        <tr>
                                            <td class="px-4 py-3">
                                                <p class="font-bold text-brand-dark">${b.album||((c=b.record)==null?void 0:c.album)||"Unknown"}</p>
                                            ...
                                            <td class="px-4 py-3 text-right font-bold text-brand-dark">DKK ${(b.unitPrice||((m=b.record)==null?void 0:m.price)||0).toFixed(2)}</td>
                                        </tr>
                                    `}).join("")}
                                </tbody>
                            </table>
    ...
    `;document.body.insertAdjacentHTML("beforeend",l)},renderCartWidget(){const t=document.getElementById("cart-widget");if(!t)return;const e=document.getElementById("cart-count"),s=document.getElementById("cart-items-mini"),o=document.getElementById("cart-total-mini");if(this.state.cart.length===0){t.classList.add("hidden");return}t.classList.remove("hidden"),e.innerText=this.state.cart.length;const r=this.state.cart.reduce((a,n)=>a+n.price,0);o.innerText=this.formatCurrency(r),s.innerHTML=this.state.cart.map((a,n)=>`
                                                                ...
                                                                `).join("")},openCheckoutModal(t,e,s=0){if(this.state.cart.length===0)return;const o=this.state.cart.reduce((b,c)=>b+this.getEffectivePrice(c),0),r=s>0?Math.round(o*(1-s)*100)/100:o,a=`
            <div id="modal-overlay" class="fixed inset-0 bg-brand-dark/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
...
`).replace(/\s+/g," "),o=t.split(/\r?\n/).map(d=>d.trim()).filter(d=>d),r=[/(\d{1,2}[-\/\.]\d{1,2}[-\/\.]\d{2,4})/,/(\d{4}[-\/\.]\d{1,2}[-\/\.]\d{1,2})/,/(\d{1,2}\.\s?\w+\.?\s?\d{2,4})/i];for(const d of r){const p=s.match(d);if(p){e.fecha=this.normalizeDate(p[1]);break}}const a=[/(?:i\s*alt|total|sum|totalt|att\s*betala)[:\s]*(\d+[.,]\d{2})/i,/(?:total|sum)[:\s]*(?:kr\.?|dkk)?\s*(\d+[.,]\d{2})/i,/(\d+[.,]\d{2})\s*(?:dkk|kr)/i];for(const d of a){const p=s.match(d);if(p){e.monto_total=parseFloat(p[1].replace(",","."));break}}const n=[/(?:moms|25%|heraf\s*moms)[:\s]*(\d+[.,]\d{2})/i,/(?:vat|iva|tax)[:\s]*(\d+[.,]\d{2})/i,/moms\s*(?:kr\.?|dkk)?\s*(\d+[.,]\d{2})/i];for(const d of n){const p=s.match(d);if(p){e.monto_iva=parseFloat(p[1].replace(",","."));break}}e.monto_total&&!e.monto_iva&&(e.monto_iva=Math.round(e.monto_total*.2*100)/100);const l=["kvittering","receipt","bon","faktura","invoice","kopi","copy"];for(const d of o.slice(0,5)){const p=d.trim();if(p.length>2&&p.length<50&&!l.some(b=>p.toLowerCase().includes(b))&&!/^\d+$/.test(p)&&!/^[\d\s\-\/\.]+$/.test(p)){e.proveedor=p;break}}const i=s.match(/(?:cvr|org\.?\s*nr)[:\s]*(\d{8})/i);if(i&&o.length>0){const d=o.findIndex(p=>p.includes(i[0]));d>0&&!e.proveedor&&(e.proveedor=o[d-1])}return console.log("Parsed Receipt Data:",e),e},normalizeDate(t){try{const s=t.replace(/\s/g,"").replace(/[\.\/]/g,"-").split("-");if(s.length>=3){let o,r,a;return s[0].length===4?[a,r,o]=s:([o,r,a]=s,a.length===2&&(a="20"+a)),o=o.padStart(2,"0"),r=r.padStart(2,"0"),`${a}-${r}-${o}`}}catch{console.warn("Date normalization failed:",t)}return null},autoFillExpenseForm(t){if(t.fecha){const e=document.getElementById("expense-fecha");e&&(e.value=t.fecha,this.highlightAutoFilled(e))}if(t.proveedor){const e=document.getElementById("expense-proveedor");e&&(e.value=t.proveedor,this.highlightAutoFilled(e))}if(t.monto_total){const e=document.getElementById("expense-monto");e&&(e.value=t.monto_total.toFixed(2),this.highlightAutoFilled(e))}if(t.monto_iva){const e=document.getElementById("expense-iva");e&&!e.disabled&&(e.value=t.monto_iva.toFixed(2),this.highlightAutoFilled(e))}},highlightAutoFilled(t){t.classList.add("ring-2","ring-green-400","bg-green-50");const e=()=>{t.classList.remove("ring-2","ring-green-400","bg-green-50"),t.removeEventListener("focus",e)};t.addEventListener("focus",e),setTimeout(e,5e3)},clearReceiptUpload(){document.getElementById("receipt-file").value="",document.getElementById("receipt-url").value="",document.getElementById("upload-placeholder").classList.remove("hidden"),document.getElementById("upload-preview").classList.add("hidden"),document.getElementById("receipt-preview-img").src="",document.getElementById("receipt-filename").textContent=""},renderConsignments(t){if(!t)return;const e=`
    <div class="max-w-7xl mx-auto px-4 md:px-8 pb-24 md:pb-8 pt-6 animate-fadeIn" >
                                                                    ...
                                                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                                                        ${this.state.consignors.map(s=>{const o=s.name,a=this.state.inventory.filter(p=>p.owner===o).reduce((p,b)=>p+b.stock,0),n=[];this.state.sales.forEach(p=>{(p.items||[]).filter(c=>{if((c.owner||"").toLowerCase()===o.toLowerCase())return!0;const v=this.state.inventory.find(m=>m.id===(c.productId||c.recordId));return v&&(v.owner||"").toLowerCase()===o.toLowerCase()}).forEach(c=>{const v=Number(c.priceAtSale||c.unitPrice||0),m=s.agreementSplit||s.split||70,g=v*m/100;n.push({...c,id:p.id,date:p.date,cost:c.costAtSale||c.cost||g,payoutStatus:p.payoutStatus||"pending",payoutDate:p.payoutDate||null})}),(!p.items||p.items.length===0)&&(p.owner||"").toLowerCase()===o.toLowerCase()&&n.push({...p,album:p.album||p.sku||"Record",cost:p.cost||(Number(p.total)||0)*(s.agreementSplit||70)/100})}),n.sort((p,b)=>new Date(b.date)-new Date(p.date)),n.reduce((p,b)=>p+(Number(b.qty||b.quantity)||1),0);const l=n.reduce((p,b)=>p+(Number(b.cost)||0),0),i=n.filter(p=>p.payoutStatus==="paid").reduce((p,b)=>p+(Number(b.cost)||0),0),d=l-i;return`
...
`)},handleAddConsignor(t){t.preventDefault();const e=new FormData(t.target),s={name:e.get("name"),agreementSplit:parseFloat(e.get("split")),email:e.get("email"),phone:e.get("phone")};I.collection("consignors").add(s).then(()=>{this.showToast("✅ Socio registrado correctamente"),document.getElementById("modal-overlay").remove(),this.loadData()}).catch(o=>{console.error(o),this.showToast("❌ Error al crear socio: "+o.message,"error")})},deleteConsignor(t){confirm("¿Eliminar este socio?")&&I.collection("consignors").doc(t).delete().then(()=>{this.showToast("✅ Socio eliminado"),this.loadData()}).catch(e=>{console.error(e),this.showToast("❌ Error al eliminar socio: "+e.message,"error")})},saveData(){try{const t={};localStorage.setItem("el-cuartito-settings",JSON.stringify(t))}catch(t){console.error("Error saving settings:",t)}},searchDiscogs(){const t=document.getElementById("discogs-search-input").value,e=document.getElementById("discogs-results");if(t){if(e.innerHTML='<p class="text-xs text-slate-400 animate-pulse p-2">Buscando en Discogs...</p>',e.classList.remove("hidden"),/^\d+$/.test(t.trim())){this.fetchDiscogsById(t.trim());return}fetch(`${R}/discogs/search?q=${encodeURIComponent(t)}`).then(s=>{if(!s.ok)throw new Error(`Error ${s.status}`);return s.json()}).then(s=>{const o=s.results||[];o.length>0?e.innerHTML=o.slice(0,10).map(r=>`
                ...
                `})}},resyncMusic(){["input-discogs-id","input-discogs-release-id","input-discogs-url","input-cover-image"].forEach(o=>{const r=document.getElementById(o);r&&(r.value="")});const t=document.querySelector('input[name="artist"]').value,e=document.querySelector('input[name="album"]').value,s=document.getElementById("discogs-search-input");s&&t&&e?(s.value=`${t} - ${e}`,this.searchDiscogs(),this.showToast("✅ Música desvinculada. Selecciona una nueva edición.","success")):this.showToast("⚠️ Falta Artista o Álbum para buscar.","error")},handleDiscogsSelection(t){const e=document.getElementById("discogs-results");e&&e.classList.add("hidden");const s=t.title.split(" - "),o=s[0]||"",r=s.slice(1).join(" - ")||t.title,a=document.querySelector("#modal-overlay form");if(!a)return;if(a.artist&&(a.artist.value=o),a.album&&(a.album.value=r),a.year&&t.year&&(a.year.value=t.year),t.thumb||t.cover_image){const l=t.cover_image||t.thumb,i=document.getElementById("input-cover-image"),d=document.getElementById("cover-preview");if(i&&(i.value=l),d){const p=d.querySelector("img"),b=document.getElementById("cover-placeholder");p&&(p.src=l,p.classList.remove("hidden")),b&&b.classList.add("hidden")}}const n=document.getElementById("input-discogs-id");if(n&&t.id&&(n.value=t.id),t.uri||t.resource_url){const l=t.uri||t.resource_url,i=l.startsWith("http")?l:"https://www.discogs.com"+l,d=document.getElementById("input-discogs-url");d&&(d.value=i)}if(t.id){const l=document.getElementById("discogs-metadata-area"),i=document.getElementById("metadata-tracks"),d=document.getElementById("metadata-tags"),p=document.getElementById("discogs-link");console.log("Metadata Area Found:",!!l),l&&(l.classList.remove("hidden"),l.style.display="grid"),i&&(i.innerHTML='<p class="text-[10px] text-slate-400 animate-pulse">Loading tracks...</p>'),this.showToast("⏳ Cargando detalles...","info"),fetch(`${R}/discogs/release/${t.id}`).then(b=>b.json()).then(b=>{const c=b.release||b;if(console.log("Full Release Data:",c),l&&(l.classList.remove("hidden"),l.style.display="grid"),p&&c.uri){const g=c.uri.startsWith("http")?c.uri:"https://www.discogs.com"+c.uri;p.href=g,p.classList.remove("hidden"),p.style.display="flex"}const v=c.styles||[],m=[...new Set(v)];d&&(d.innerHTML=m.map(g=>`<span class="meta-chip border border-slate-200">${g}</span>`).join(""));for(let g=0;g<Math.min(m.length,3);g++){const w=document.getElementById(`genre-${g+1}`);w&&(w.value=m[g])}if(i)if(c.tracklist&&c.tracklist.length>0){const g=document.getElementById("input-tracks");g&&(g.value=JSON.stringify(c.tracklist)),i.innerHTML=c.tracklist.map(w=>`
                            ...
                            `).join("")}else i.innerHTML='<p class="text-[10px] text-slate-400 italic">No tracks found.</p>';a.label&&c.labels&&c.labels.length>0&&(a.label.value=c.labels[0].name)}).catch(b=>{console.error("Error fetching full release:",b),i&&(i.innerHTML='<p class="text-[10px] text-red-400">Error loading tracklist.</p>')})}},openTracklistModal(t){const e=this.state.inventory.find(a=>a.id===t||a.sku===t);if(!e)return;let s=e.discogsId;document.body.insertAdjacentHTML("beforeend",`
                                                                ...
                                                                `);const r=a=>{fetch(`${R}/discogs/release/${a}`).then(n=>{if(!n.ok)throw new Error("Release not found");return n.json()}).then(n=>{const i=(n.release||n).tracklist||[],d=i.map(b=>`
                    ...
                    `})}},renderDiscogsSales(t){const e=this.state.sales.filter(i=>i.channel==="discogs"),s=i=>parseFloat(i.total)||0,o=i=>parseFloat(i.originalTotal)||parseFloat(i.total)+(parseFloat(i.discogsFee||0)+parseFloat(i.paypalFee||0)),r=i=>o(i)-s(i),a=e.reduce((i,d)=>i+s(d),0),n=e.reduce((i,d)=>i+r(d),0),l=e.reduce((i,d)=>{const p=s(d);let b=0;return d.items&&Array.isArray(d.items)&&(b=d.items.reduce((c,v)=>{const m=parseFloat(v.costAtSale||0),g=parseInt(v.qty||v.quantity)||1;return c+m*g},0)),i+(p-b)},0);t.innerHTML=`
        <div class="p-6">
            <!-- Header -->
            <div class="flex items-center justify-between mb-8">
                <div>
                ...
                `:`
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody>
                                ${e.map(i=>{var p;const d=(p=i.timestamp)!=null&&p.toDate?i.timestamp.toDate():i.date?new Date(i.date):new Date(0);return{...i,_sortDate:d.getTime()}}).sort((i,d)=>d._sortDate-i._sortDate).map(i=>{var m;const d=(m=i.timestamp)!=null&&m.toDate?i.timestamp.toDate():new Date(i.date),p=i.items&&i.items[0],b=i.originalTotal||i.total+(i.discogsFee||0)+(i.paypalFee||0);i.discogsFee,i.paypalFee;const c=i.total,v=i.status==="pending_review"||i.needsReview;return`
                                        <tr class="border-b border-slate-50 hover:bg-purple-50/30 transition-colors cursor-pointer ${v?"bg-orange-50/50":""}" onclick="app.openUnifiedOrderDetailModal('${i.id}')">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="text-[10px] text-red-500 font-bold">Total Fees: -${this.formatCurrency(b-c)}</div>
                                                <div class="text-[10px] text-slate-400 font-medium">
                                                    ${b>0?`(${((b-c)/b*100).toFixed(1)}%)`:""}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4">
                                                <div class="text-sm font-bold text-brand-dark">${this.formatCurrency(c)}</div>
                                            </td>
                                            <td class="px-6 py-4">
                                                <div class="flex flex-col gap-2">
                                                    ${v?`
                                                    ...
                                                    `}
                                                    <button onclick="app.openUpdateSaleValueModal('${i.id}', ${b}, ${c})" class="w-full py-1.5 bg-slate-100 text-slate-600 hover:bg-slate-200 text-[10px] font-bold rounded-lg transition-colors border border-slate-200 flex items-center justify-center gap-1">
...
¿CONFIRMAS que deseas eliminar permanentemente este registro y su recibo?`)){this.showToast("ℹ️ Eliminación cancelada");return}}else if(!confirm("¿Estás seguro de que quieres eliminar este gasto?"))return;try{await I.collection("expenses").doc(t).delete(),this.showToast("✅ Gasto eliminado"),this.loadData()}catch(s){console.error("Error deleting expense:",s),this.showToast("❌ Error al eliminar gasto")}},renderVATReport(t){const e=new Date,s=Math.floor(e.getMonth()/3)+1,o=e.getFullYear(),r=this.state.vatReportQuarter!==void 0?this.state.vatReportQuarter:s,a=this.state.vatReportYear||o;let n,l;if(r===0)n=new Date(a,0,1),l=new Date(a,11,31,23,59,59);else{const x=(r-1)*3;n=new Date(a,x,1),l=new Date(a,x+3,0,23,59,59)}const i=this.state.sales.filter(x=>{var O;const T=(O=x.timestamp)!=null&&O.toDate?x.timestamp.toDate():new Date(x.timestamp||x.date);return T>=n&&T<=l});let d=[],p=[],b=[],c=0,v=0,m=0,g=0;i.forEach(x=>{var ae;const T=(ae=x.timestamp)!=null&&ae.toDate?x.timestamp.toDate():new Date(x.timestamp||x.date);(x.items||[]).forEach(j=>{const ie=j.priceAtSale||j.price||0;let de=j.costAtSale||j.cost||0;const H=j.productId||j.recordId,W=j.album;let N=j.providerOrigin||j.provider_origin;if(de===0||!N){const F=this.state.inventory.find(q=>H&&(q.id===H||q.sku===H)||W&&q.album===W);F&&(de===0&&(de=F.cost||0),N||(N=F.provider_origin||"Local_Used"))}N||(N="Local_Used");const P=j.qty||j.quantity||1,U=ie*P,z=de*P;if(N==="EU_B2B"||N==="DK_B2B"){const F=U*.2;c+=F,d.push({date:T,productId:j.productId||j.album||"N/A",album:j.album||"N/A",salePrice:U,vat:F})}else{const F=U-z,q=F>0?F*.2:0;v+=q,p.push({date:T,productId:j.productId||j.album||"N/A",album:j.album||"N/A",cost:z,salePrice:U,margin:F,vat:q})}});const se=parseFloat(x.shipping_income||x.shipping||x.shipping_cost||0);if(se>0){const j=se*.2;m+=j,g+=se,b.push({date:T,orderId:x.orderNumber||(x.id&&typeof x.id=="string"?x.id.slice(-8):"N/A"),income:se,vat:j})}}),(this.state.extraIncome||[]).filter(x=>{const T=new Date(x.date);return T>=n&&T<=l}).forEach(x=>{const T=Number(x.amount)||0,O=Number(x.vatAmount)||0;c+=O,d.push({date:new Date(x.date),productId:"EXTRA",album:`💰 ${x.description||"Ingreso Extra"} (${x.category||"other"})`,salePrice:T,vat:O})});const u=c+v+m,f=this.state.expenses.filter(x=>{var se;const T=x.fecha_factura?new Date(x.fecha_factura):(se=x.timestamp)!=null&&se.toDate?x.timestamp.toDate():new Date(x.timestamp||x.date);return(x.categoria_tipo==="operativo"||x.categoria_tipo==="stock_nuevo"||x.is_vat_deductible)&&T>=n&&T<=l}),$=f.filter(x=>x.categoria!=="envios"),E=f.filter(x=>x.categoria==="envios"),C=$.reduce((x,T)=>x+(parseFloat(T.monto_iva)||0),0),D=E.reduce((x,T)=>x+(parseFloat(T.monto_iva)||0),0);E.reduce((x,T)=>x+(parseFloat(T.monto_total)||0),0);const y=(this.state.inventory||[]).filter(x=>{if(!x.item_phantom_vat||x.item_phantom_vat<=0||x.provider_origin!=="EU_B2B")return!1;const T=x.acquisition_date?new Date(x.acquisition_date):null;return T?T>=n&&T<=l:!1}),S=y.reduce((x,T)=>x+(T.item_phantom_vat||0),0),_=(this.state.inventory||[]).filter(x=>{if(!x.item_real_vat||x.item_real_vat<=0||x.provider_origin!=="DK_B2B")return!1;const T=x.acquisition_date?new Date(x.acquisition_date):null;return T?T>=n&&T<=l:!1}),M=_.reduce((x,T)=>x+(T.item_real_vat||0),0),V=u+S,Y=C+D+S+M,J=V-Y,Z={0:`Resumen anual ${a}`,1:`1 de junio, ${a}`,2:`1 de septiembre, ${a}`,3:`1 de diciembre, ${a}`,4:`1 de marzo, ${a+1}`}[r],te=`
            <div class="max-w-7xl mx-auto px-4 md:px-8 pb-24 md:pb-8 pt-6">
                <!-- Header Section -->
                <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-6 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <div class="flex items-center gap-4">
                            ...
                            <div class="p-6 space-y-6">
                                ...
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-[10px] text-slate-400 uppercase font-bold tracking-tighter">Monto: ${this.formatCurrency(d.reduce((x,T)=>x+T.salePrice,0))}</p>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-lg font-bold text-blue-600">${this.formatCurrency(c)}</p>
                                        <p class="text-[10px] text-slate-400 font-bold">IVA (25%)</p>
                                    </div>
                                </div>
                                <div class="h-1.5 w-full bg-slate-50 rounded-full overflow-hidden">
                                    <div class="h-full bg-blue-500 rounded-full" style="width: ${u>0?c/u*100:0}%"></div>
                                </div>
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tfoot class="bg-slate-50/30 font-bold">
                                    <tr class="text-brand-dark">
                                        ...
                                        <td class="px-6 py-4 text-right text-lg text-blue-600">${this.formatCurrency(c)}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
        ...
        `;t.innerHTML=te},updateVATQuarter(){const t=parseInt(document.getElementById("vat-quarter-select").value),e=parseInt(document.getElementById("vat-year-select").value);this.state.vatReportQuarter=t,this.state.vatReportYear=e,this.renderVATReport(document.getElementById("app-content"))},downloadVATAuditReport(){const t=new Date,e=Math.floor(t.getMonth()/3)+1,s=t.getFullYear(),o=this.state.vatReportQuarter||e,r=this.state.vatReportYear||s,a=(o-1)*3,n=new Date(r,a,1),l=new Date(r,a+3,0,23,59,59),i=this.state.sales.filter(y=>{var _;const S=(_=y.timestamp)!=null&&_.toDate?y.timestamp.toDate():new Date(y.timestamp||y.date);return S>=n&&S<=l}),d=[];let p=1;i.forEach(y=>{var J;const S=(J=y.timestamp)!=null&&J.toDate?y.timestamp.toDate():new Date(y.timestamp||y.date),_=S.toISOString().slice(0,10).replace(/-/g,""),M=y.channel||"N/A";(y.items||[]).forEach(G=>{const Z=G.priceAtSale||G.price||0;let Q=G.costAtSale||G.cost||0;const ee=G.productId||G.recordId,te=G.album;let x="Local_Used",T="N/A";const O=this.state.inventory.find(q=>ee&&(q.id===ee||q.sku===ee)||te&&q.album===te);O&&(Q=Q===0?O.cost||0:Q,x=O.provider_origin||"Local_Used",O.acquisition_date&&(T=new Date(O.acquisition_date).toISOString().slice(0,10)));const se=G.qty||G.quantity||1,ae=G.providerOrigin||x,j=ae==="EU_B2B"||ae==="DK_B2B",ie=Z*se,de=Q*se;let H,W,N;if(j)H=ie,W=ie*.2,N="Standard Rate";else{const q=ie-de;H=q>0?q:0,W=q>0?q*.2:0,N="Margin Scheme"}const P=O&&O.acquisition_date?new Date(O.acquisition_date):null,U=P&&P>=n&&P<=l,z=U&&x==="EU_B2B"&&O.item_phantom_vat||0,F=U?x==="DK_B2B"?O.item_real_vat||0:z:0;d.push({transactionId:`ECR-${_}-${String(p).padStart(4,"0")}`,date:S.toISOString().slice(0,10),channel:M,productName:`${G.album||"N/A"} - ${G.artist||"N/A"}`,sku:G.sku||ee||"N/A",providerOrigin:x,acquisitionDate:T,condition,costPrice:de.toFixed(2),salesPrice:ie.toFixed(2),calculationBasis:H.toFixed(2),schemeApplied:N,outputVat:W.toFixed(2),euPhantomVat:z.toFixed(2),inputVat:F.toFixed(2)}),p++});const Y=parseFloat(y.shipping_income||y.shipping||y.shipping_cost||0);Y>0&&(d.push({transactionId:`ECR-SHIP-${_}-${String(p).padStart(4,"0")}`,date:S.toISOString().slice(0,10),channel:M,productName:`Envío Cobrado - Orden: ${y.orderNumber||"N/A"}`,sku:"SHIPPING",providerOrigin:"N/A",acquisitionDate:"N/A",condition:"Service",costPrice:"0.00",salesPrice:Y.toFixed(2),calculationBasis:Y.toFixed(2),schemeApplied:"Standard Rate",outputVat:(Y*.2).toFixed(2),euPhantomVat:"0.00",inputVat:"0.00"}),p++)});const b=[];let c=1;this.state.expenses.filter(y=>{var M;const S=y.fecha_factura?new Date(y.fecha_factura):(M=y.timestamp)!=null&&M.toDate?y.timestamp.toDate():new Date(y.timestamp||y.date);return(y.categoria_tipo==="operativo"||y.categoria_tipo==="stock_nuevo"||y.is_vat_deductible)&&S>=n&&S<=l}).forEach(y=>{var M;const S=y.fecha_factura?new Date(y.fecha_factura):(M=y.timestamp)!=null&&M.toDate?y.timestamp.toDate():new Date(y.timestamp||y.date),_=S.toISOString().slice(0,10).replace(/-/g,"");b.push({transactionId:`ECP-EXP-${_}-${String(c).padStart(4,"0")}`,invoiceDate:S.toISOString().slice(0,10),category:y.categoria==="envios"?"Shipping Expense":"Operational Expense",vendor:y.proveedor||y.nombre||"N/A",description:y.descripcion||y.categoria||"N/A",sku:"N/A",grossAmount:parseFloat(y.monto_total||0).toFixed(2),euPhantomVat:"0.00",inputVat:parseFloat(y.monto_iva||0).toFixed(2)}),c++}),(this.state.inventory||[]).filter(y=>{if(!(y.provider_origin==="EU_B2B"||y.provider_origin==="DK_B2B"))return!1;const _=y.acquisition_date?new Date(y.acquisition_date):null;return _?_>=n&&_<=l:!1}).forEach(y=>{const S=new Date(y.acquisition_date),_=S.toISOString().slice(0,10).replace(/-/g,""),M=parseFloat(y.cost||0),V=y.provider_origin==="EU_B2B"&&y.item_phantom_vat||0,Y=y.provider_origin==="DK_B2B"?y.item_real_vat||0:V;b.push({transactionId:`ECP-INV-${_}-${String(c).padStart(4,"0")}`,invoiceDate:S.toISOString().slice(0,10),category:`Stock Import (${y.provider_origin})`,vendor:y.provider_origin,description:`${y.album||"N/A"} - ${y.artist||"N/A"}`,sku:y.sku||"N/A",grossAmount:M.toFixed(2),euPhantomVat:V.toFixed(2),inputVat:Y.toFixed(2)}),c++});const g="\uFEFF",w=(y,S)=>[y.join(","),...S.map(_=>y.map(M=>{const V=Object.keys(_)[y.indexOf(M)];return`"${String(_[V]||"").replace(/"/g,'""')}"`}).join(","))].join(`
`),u=["Transaction ID","Transaction Date","Sales Channel","Product Name","SKU / Item ID","Provider Origin","Acquisition Date","Condition","Cost Price (DKK)","Sales Price (DKK)","Calculation Basis (DKK)","VAT Scheme Applied","Output VAT / Salgsmoms (DKK)","EU Phantom VAT / Rubrik A (DKK)","Input VAT / Købsmoms (DKK)"],f=["Transaction ID","Invoice Date","Category","Vendor / Origin","Description","SKU / Item ID","Gross Amount / Cost (DKK)","EU Phantom VAT / Rubrik A (DKK)","Input VAT / Købsmoms (DKK)"],$=w(u,d),E=w(f,b),C=new Blob([g+$],{type:"text/csv;charset=utf-8;"}),D=document.createElement("a");D.href=URL.createObjectURL(C),D.download=`Sales_VAT_Ledger_Q${o}_${r}.csv`,D.style.display="none",document.body.appendChild(D),D.click(),setTimeout(()=>{const y=new Blob([g+E],{type:"text/csv;charset=utf-8;"}),S=document.createElement("a");S.href=URL.createObjectURL(y),S.download=`Purchases_VAT_Ledger_Q${o}_${r}.csv`,S.style.display="none",document.body.appendChild(S),S.click(),document.body.removeChild(D),document.body.removeChild(S),URL.revokeObjectURL(D.href),URL.revokeObjectURL(S.href)},300),this.showToast(`✅ Exported ${d.length} sales & ${b.length} purchase records.`)},renderInvestments(t){const e=["Alejo","Facundo","Rafael"],s=this.state.investments||[],o=e.reduce((n,l)=>(n[l]=s.filter(i=>i.partner===l).reduce((i,d)=>i+(parseFloat(d.amount)||0),0),n),{}),r=Object.values(o).reduce((n,l)=>n+l,0),a=`
            <div class="max-w-7xl mx-auto px-4 md:px-8 pb-24 pt-6">
                <div class="flex justify-between items-center mb-8">
                    <div>
                ...
                ${e.map(n=>{const l=s.filter(i=>i.partner===n).sort((i,d)=>new Date(d.date)-new Date(i.date));return`
        ...
        `;document.body.insertAdjacentHTML("beforeend",s)},async saveInvestment(t){t.preventDefault();const e=t.target,s={partner:e.partner.value,amount:parseFloat(e.amount.value),description:e.description.value,date:e.date.value,created_at:firebase.firestore.FieldValue.serverTimestamp()};try{await I.collection("investments").add(s),document.getElementById("add-investment-modal").remove(),this.showToast("✅ Inversión registrada"),await this.loadInvestments(),this.refreshCurrentView()}catch(o){this.showToast("❌ Error: "+o.message,"error")}},async deleteInvestment(t){if(confirm("¿Eliminar esta inversión?"))try{await I.collection("investments").doc(t).delete(),this.showToast("🗑️ Inversión eliminada"),await this.loadInvestments(),this.refreshCurrentView()}catch(e){this.showToast("❌ Error: "+e.message,"error")}},async loadInvestments(){const t=await I.collection("investments").get();this.state.investments=t.docs.map(e=>({id:e.id,...e.data()}))},renderShipping(t){const e=i=>{var d;return((d=i.shipping_method)==null?void 0:d.id)==="local_pickup"||i.shipping_method&&typeof i.shipping_method=="string"&&i.shipping_method.toLowerCase().includes("pickup")||i.shippingMethod&&i.shippingMethod.toLowerCase().includes("pickup")||Number(i.shipping)===0||Number(i.shipping_cost)===0||Number(i.shipping_income)===0},s=i=>!e(i),o=i=>!["shipped","picked_up","delivered","fulfilled"].includes(i.fulfillment_status),r=this.state.sales.filter(i=>{var d;return(i.channel==="online"||((d=i.channel)==null?void 0:d.toLowerCase())==="discogs")&&e(i)&&o(i)}).sort((i,d)=>new Date(i.date)-new Date(d.date)),a=this.state.sales.filter(i=>{var d;return(i.channel==="online"||((d=i.channel)==null?void 0:d.toLowerCase())==="discogs")&&s(i)&&o(i)}).sort((i,d)=>new Date(i.date)-new Date(d.date)),n=this.state.sales.filter(i=>{var d;return(i.channel==="online"||((d=i.channel)==null?void 0:d.toLowerCase())==="discogs")&&!o(i)}).sort((i,d)=>{var p,b;return new Date((p=d.updated_at)!=null&&p.toDate?d.updated_at.toDate():d.updated_at||d.date)-new Date((b=i.updated_at)!=null&&b.toDate?i.updated_at.toDate():i.updated_at||i.date)}).slice(0,20),l=`
            <div class="max-w-7xl mx-auto px-4 md:px-8 pb-24 pt-6 animate-fadeIn">
                   ...
                   <div class="flex gap-4">
                        <div class="bg-indigo-500 text-white px-5 py-3 rounded-2xl shadow-lg shadow-indigo-500/20 flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-2xl font-display font-bold">${this.formatCurrency(this.state.sales.reduce((i,d)=>i+parseFloat(d.shipping||d.shipping_cost||0),0))}</p>
                            </div>
                        </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-blue-50">
                                ${r.length>0?r.map(i=>{var f;const d=this.getCustomerInfo(i),p=i.fulfillment_status||"unfulfilled";let b=p==="unfulfilled"?`onclick="app.notifyPreparingDiscogs('${i.id}')"`:"disabled",c=p==="unfulfilled"?"bg-blue-500 text-white shadow-sm hover:bg-blue-600":p==="preparing"||p==="ready_for_pickup"?"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-75":"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-50",v=p==="preparing"?`onclick="app.notifyPickupReadyDiscogs('${i.id}')"`:"disabled",m=p==="preparing"?"bg-brand-orange text-white shadow-sm hover:bg-orange-600":p==="ready_for_pickup"?"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-75":"bg-slate-50 border border-slate-100 text-slate-300 cursor-not-allowed opacity-50",g=p==="ready_for_pickup"?`onclick="app.markPickedUpDiscogs('${i.id}')"`:"disabled",u=`
                <div class="flex flex-col gap-2 relative pl-2">
                    ...
                    <button ${b} class="w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-between ${c}">
                    ...
                    <button ${g} class="w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-between ${p==="ready_for_pickup"?"bg-brand-dark text-white shadow-sm hover:bg-black":"bg-slate-50 border border-slate-100 text-slate-300 cursor-not-allowed opacity-50"}">
                        <span class="flex items-center gap-2">
                                    ...
                                    <tr class="hover:bg-blue-50/20 transition-colors">
                                        ...
                                        <td class="p-4">
                                            <div class="flex -space-x-2 overflow-hidden">
                                                ${(i.items||[]).slice(0,3).map($=>`<img src="${$.image||$.cover_image||"https://elcuartito.dk/default-vinyl.png"}" 
                                            ...
                                            <div class="text-[10px] text-slate-400 mt-1">${((f=i.items)==null?void 0:f.length)||0} items</div>
                                        </td>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-orange-100 overflow-hidden mb-8">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-orange-50">
                                ${a.length>0?a.map(i=>{var u;const d=this.getCustomerInfo(i),p=i.fulfillment_status||"unfulfilled";let b=p==="unfulfilled"?`onclick="app.notifyPreparingDiscogs('${i.id}')"`:"disabled",c=p==="unfulfilled"?"bg-brand-orange text-white shadow-sm hover:bg-orange-600":p==="preparing"||p==="in_transit"?"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-75":"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-50",v=p==="in_transit"?`onclick="app.markDispatchedDiscogs('${i.id}')"`:"disabled",m=p==="in_transit"?"bg-brand-dark text-white shadow-sm hover:bg-black":"bg-slate-50 border border-slate-100 text-slate-300 cursor-not-allowed opacity-50",g="";p==="preparing"?g=`
            ...
            `;return`
                                    <tr class="hover:bg-orange-50/20 transition-colors">
                                        ...
                                        <td class="p-4">
                                            <div class="flex -space-x-2 overflow-hidden">
                                                ${(i.items||[]).slice(0,3).map(f=>`<img src="${f.image||f.cover_image||"https://elcuartito.dk/default-vinyl.png"}" 
                                                         class="inline-block h-8 w-8 rounded-full ring-2 ring-white object-cover" 
                                                         title="${f.album}">`).join("")}
                                                ${(i.items||[]).length>3?`<span class="h-8 w-8 rounded-full bg-gray-100 flex items-center justify-center text-[10px] ring-2 ring-white text-slate-500 font-bold">+${i.items.length-3}</span>`:""}
                                            </div>
                                            <div class="text-[10px] text-slate-400 mt-1">${((u=i.items)==null?void 0:u.length)||0} items</div>
                                        </td>
                                ...
                                ${n.map(i=>{var d;return`
                                    <tr class="hover:bg-slate-50 transition-colors cursor-pointer" onclick="app.openUnifiedOrderDetailModal('${i.id}')" title="Ver historial">
                                        ...
                                        <td class="p-4 text-xs text-slate-400">
                                            ${this.formatDate((d=i.updated_at)!=null&&d.toDate?i.updated_at.toDate():i.updated_at||i.date)}
                                        </td>
                                        <td class="p-4 text-right">
                ...
                <div class="p-8 max-h-[60vh] overflow-y-auto">
                    ...
                    <div class="mt-8 pt-6 border-t border-slate-50 flex justify-between items-end">
                       <div class="text-xs text-slate-400">
                            Cliente: <span class="font-bold text-slate-600">${e.customerName||((p=e.customer)==null?void 0:p.name)||((b=e.customer)==null?void 0:b.firstName)+" "+((c=e.customer)==null?void 0:c.lastName)}</span><br>
