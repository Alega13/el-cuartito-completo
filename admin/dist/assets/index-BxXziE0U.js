...
Por favor, recarga la página.`)}else{const i=S.collection("metadata").doc("vinylCounter"),l=await i.get(),d=l.exists&&l.data().current||0;e>d&&(await i.set({current:e},{merge:!0}),console.log(`🆙 Contador actualizado a ${e}`)),console.log("✅ No hay conflictos de SKU.")}}catch(t){console.error("❌ Error en script histórico:",t)}}},state:{inventory:[],sales:[],expenses:[],consignors:[],cart:[],viewMode:"list",selectedItems:new Set,currentView:"dashboard",filterMonths:[new Date().getMonth()],filterYear:new Date().getFullYear(),inventorySearch:"",salesHistorySearch:"",expensesSearch:"",events:[],selectedDate:new Date,vatActive:!1,manualSaleSearch:"",posCondition:"Used",posSelectedItemSku:null,orderFeedFilter:"all",filterGenre:"all",filterOwner:"all",filterLabel:"all",filterStorage:"all",filterDiscogs:"all",filterStockTime:[],privacyMode:!1,rsdExtraDiscount:!1,dashboardAnalysisMode:"genre"},getEffectivePrice(t){return t.is_rsd_discount?Math.round(t.price*.9):t.price},async init(){this.runSkuFix_v3(),!this._initialized&&(this._initialized=!0,re.onAuthStateChanged(async t=>{if(t)try{document.getElementById("login-view").classList.add("hidden"),document.getElementById("main-app").classList.remove("hidden"),document.getElementById("mobile-nav").classList.remove("hidden"),await this.loadData(),this._pollInterval&&clearInterval(this._pollInterval),this._pollInterval=setInterval(()=>this.loadData(),6e4),this.setupListeners(),this.setupMobileMenu(),this.setupNavigation()}catch(e){console.error("Auth token error:",e),this.logout()}else{document.getElementById("login-view").classList.remove("hidden"),document.getElementById("main-app").classList.add("hidden"),document.getElementById("mobile-nav").classList.add("hidden");const e=document.getElementById("login-btn");e&&(e.disabled=!1,e.innerHTML="<span>Entrar</span>")}}),document.addEventListener("click",t=>{const e=document.getElementById("discogs-results"),s=document.getElementById("discogs-search-input");e&&!e.contains(t.target)&&t.target!==s&&e.classList.add("hidden");const o=document.getElementById("sku-results"),r=document.getElementById("sku-search");o&&!o.contains(t.target)&&t.target!==r&&o.classList.add("hidden")}))},async handleLogin(t){t.preventDefault();const e=t.target.email.value,s=t.target.password.value,o=document.getElementById("login-error"),r=document.getElementById("login-btn");o.classList.add("hidden"),r.disabled=!0,r.innerHTML="<span>Cargando...</span>";try{await re.signInWithEmailAndPassword(e,s)}catch(a){console.error("Login error:",a),o.innerText="Error: "+a.message,o.classList.remove("hidden"),r.disabled=!1,r.innerHTML='<span>Ingresar</span><i class="ph-bold ph-arrow-right"></i>'}},async updateFulfillmentStatus(t,e,s){var o,r,a;try{const n=((o=t==null?void 0:t.target)==null?void 0:o.closest("button"))||((a=(r=window.event)==null?void 0:r.target)==null?void 0:a.closest("button"));if(n){n.disabled=!0;const i=n.innerHTML;n.innerHTML='<i class="ph ph-circle-notch animate-spin"></i>'}await S.collection("sales").doc(e).update({fulfillment_status:s}),await this.loadData(),document.getElementById("modal-overlay")&&(document.getElementById("modal-overlay").remove(),this.openOnlineSaleDetailModal(e)),this.showToast("Estado de envío actualizado")}catch(n){console.error("Fulfillment update error:",n),this.showToast("Error al actualizar estado: "+n.message,"error")}},async manualShipOrder(t){var e,s,o,r,a,n;try{const i=prompt("Introduce el número de seguimiento:");if(!i)return;const l=((e=event==null?void 0:event.target)==null?void 0:e.closest("button"))||((o=(s=window.event)==null?void 0:s.target)==null?void 0:o.closest("button"));l&&(l.disabled=!0,l.innerHTML='<i class="ph ph-circle-notch animate-spin"></i> Guardando...');const d=await fetch(`${R}/api/manual-ship`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({orderId:t,trackingNumber:i})}),c=await d.json();if(d.ok&&c.success){if(this.showToast("✅ Pedido marcado como enviado"),c.emailSent)this.showToast("📧 Cliente notificado por email","success");else{const p=typeof c.emailError=="object"?JSON.stringify(c.emailError):c.emailError;this.showToast("⚠️ Pedido marcado pero EL EMAIL FALLÓ: "+p,"warning")}await this.loadData();const u=document.getElementById("sale-detail-modal");u&&(u.remove(),this.openUnifiedOrderDetailModal(t))}else throw new Error(c.error||c.message||"Error desconocido")}catch(i){console.error("Error shipping manually:",i),this.showToast("❌ Error: "+(i.message||"No se pudo procesar el envío"),"error");const l=((r=event==null?void 0:event.target)==null?void 0:r.closest("button"))||((n=(a=window.event)==null?void 0:a.target)==null?void 0:n.closest("button"));l&&(l.disabled=!1,l.innerHTML='<i class="ph-bold ph-truck"></i> Ingresar Tracking y Cerrar')}},async logout(){try{await re.signOut(),location.reload()}catch(t){console.error("Sign out error:",t),location.reload()}},setupListeners(){this._unsubscribeProducts&&this._unsubscribeProducts(),this._unsubscribeProducts=S.collection("products").onSnapshot(t=>{this.state.inventory=t.docs.map(e=>{const s=e.data();return{id:e.id,...s,condition:s.condition||"VG",owner:s.owner||"El Cuartito",label:s.label||"Desconocido",storageLocation:s.storageLocation||"Tienda",cover_image:s.cover_image||s.coverImage||null}}),(this.state.currentTab==="inventory"||this.state.currentTab==="dashboard")&&this.renderCurrentTab()},t=>{console.error("Inventory listener error:",t)})},async loadData(){try{const[t,e,s,o,r,a]=await Promise.all([S.collection("products").get(),S.collection("sales").get(),S.collection("expenses").get(),S.collection("events").orderBy("date","desc").get(),S.collection("consignors").get(),S.collection("extra_income").get()]);this.state.inventory=t.docs.map(n=>{const i=n.data();return{id:n.id,...i,condition:i.condition||"VG",owner:i.owner||"El Cuartito",label:i.label||"Desconocido",storageLocation:i.storageLocation||"Tienda",cover_image:i.cover_image||i.coverImage||null}}),this.state.sales=e.docs.map(n=>{var d,c;const i=n.data(),l={id:n.id,...i,date:i.date||((d=i.timestamp)!=null&&d.toDate?i.timestamp.toDate().toISOString().split("T")[0]:(c=i.created_at)!=null&&c.toDate?i.created_at.toDate().toISOString().split("T")[0]:new Date().toISOString().split("T")[0])};return i.total_amount!==void 0&&i.total===void 0&&(l.total=i.total_amount),i.payment_method&&!i.paymentMethod&&(l.paymentMethod=i.payment_method),l.items&&Array.isArray(l.items)&&(l.items=l.items.map(u=>({...u,priceAtSale:u.priceAtSale!==void 0?u.priceAtSale:u.unitPrice||0,qty:u.qty!==void 0?u.qty:u.quantity||1,costAtSale:u.costAtSale!==void 0?u.costAtSale:u.cost||0}))),l}).filter(n=>n.status!=="PENDING"&&n.status!=="failed").sort((n,i)=>{const l=new Date(n.date);return new Date(i.date)-l}),this.state.expenses=s.docs.map(n=>{var l;const i=n.data();return{id:n.id,...i,date:i.fecha_factura||i.date||((l=i.timestamp)==null?void 0:l.split("T")[0])||new Date().toISOString().split("T")[0]}}).sort((n,i)=>new Date(i.date)-new Date(n.date)),this.state.events=o.docs.map(n=>({id:n.id,...n.data()})),this.state.consignors=r.docs.map(n=>{const i=n.data();return{id:n.id,...i,agreementSplit:i.split||i.agreementSplit||(i.percentage?Math.round(i.percentage*100):70)}}),await this.loadInvestments(),this.state.extraIncome=a.docs.map(n=>({id:n.id,...n.data()})).sort((n,i)=>new Date(i.date)-new Date(n.date)),this.initFuse(),this.refreshCurrentView()}catch(t){console.error("Failed to load data:",t),this.showToast("❌ Error de conexión: "+t.message,"error")}},refreshCurrentView(){const t=document.getElementById("app-content");if(t)switch(this.state.currentView){case"dashboard":this.renderDashboard(t);break;case"inventory":this.renderInventory(t);break;case"sales":this.renderSales(t);break;case"onlineSales":this.renderOnlineSales(t);break;case"discogsSales":this.renderDiscogsSales(t);break;case"expenses":this.renderExpenses(t);break;case"consignments":this.renderConsignments(t);break;case"backup":this.renderBackup(t);break;case"settings":this.renderSettings(t);break;case"calendar":this.renderCalendar(t);break;case"shipping":this.renderShipping(t);break;case"pickups":this.renderPickups(t);break;case"investments":this.renderInvestments(t);break;case"vatReport":this.renderVATReport(t);break;case"datosLegales":this.renderDatosLegales(t);break;case"contabilidad":this.renderContabilidad(t);break;case"facturasManual":this.renderFacturasManual(t);break;case"extraIncome":this.renderExtraIncome(t);break}},renderDatosLegales(t){const e=`
        ...
        `;t.innerHTML=e},renderContabilidad(t){const e=new Date().getFullYear(),s=Math.floor(new Date().getMonth()/3)+1;this.state.contabilidadYear||(this.state.contabilidadYear=e),this.state.contabilidadQuarter||(this.state.contabilidadQuarter=s),this.state.contabilidadInvoices||(this.state.contabilidadInvoices=[]),this.state.contabilidadLoading||(this.state.contabilidadLoading=!1);const o=this.state.contabilidadYear,r=this.state.contabilidadQuarter,a=this.state.contabilidadInvoices,n=this.state.contabilidadLoading,i=d=>{const c={local:"bg-emerald-100 text-emerald-700",online:"bg-blue-100 text-blue-700",discogs:"bg-purple-100 text-purple-700"},u={local:"Tienda",online:"Webshop",discogs:"Discogs"};return`<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold ${c[d]||"bg-slate-100 text-slate-600"}">${u[d]||d}</span>`},l=`
            <div class="max-w-6xl mx-auto px-4 md:px-8 pb-24 md:pb-8 pt-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div class="kpi-card">
                        ...
                        <div class="text-2xl font-bold text-brand-orange">${this.formatCurrency(a.reduce((d,c)=>d+(c.totalAmount||0),0))}</div>
                    </div>
                    <div class="kpi-card">
                                ...
                                <tbody>
                                    ${a.map((d,c)=>`
                                        <tr class="inv-row border-b border-slate-50 ${c%2===0?"bg-white":"bg-slate-50/30"}">
                                            ...
                                            <td class="px-5 py-3">${i(d.channel)}</td>
                                            ...
                                            <td class="px-5 py-3 text-sm font-bold text-brand-dark text-right">${this.formatCurrency(d.totalAmount||0)}</td>
...
${t.message}`),this.showToast("Error en backfill","error")}},renderFacturasManual(t){const e=(this.state.contabilidadInvoices||[]).filter(o=>o.channel==="manual"||o.isManual),s=`
                ...
                <div class="flex flex-col lg:flex-row gap-8 h-[calc(100vh-140px)]">
                    ...
                    <div class="flex-1 bg-white rounded-2xl shadow-sm border border-orange-100 p-6 flex flex-col">
                        ...
                        <div class="grid grid-cols-7 gap-2 flex-1 auto-rows-fr">
                            ...
                            ${Array.from({length:n},(u,p)=>{const v=p+1,m=`${s}-${String(o+1).padStart(2,"0")}-${String(v).padStart(2,"0")}`,x=e.getDate()===v,w=d(v),b=new Date().toDateString()===new Date(s,o,v).toDateString();return`
                                    <button onclick="app.selectCalendarDate('${m}')" 
                                        ...
                                        ${b?"bg-blue-50":""}">
                                        <span class="text-sm font-bold ${x?"text-brand-orange":"text-slate-700"} ${b?"text-blue-600":""}">${v}</span>
        ...
        `;document.body.insertAdjacentHTML("beforeend",e)},handleAddEvent(t){t.preventDefault();const e=new FormData(t.target),s={date:e.get("date"),title:e.get("title"),description:e.get("description"),createdAt:new Date().toISOString()};S.collection("events").add(s).then(()=>{this.showToast("✅ Evento agregado"),document.getElementById("modal-overlay").remove(),this.loadData()}).catch(o=>console.error(o))},deleteEvent(t){confirm("¿Eliminar este evento?")&&S.collection("events").doc(t).delete().then(()=>{this.showToast("✅ Evento eliminado"),this.loadData()}).catch(e=>console.error(e))},renderBackup(t){const e=`
...
¿Estás seguro?`))return;if(prompt("Para confirmar, ingresa la contraseña de administrador:")!=="alejo13"){alert("Contraseña incorrecta. Operación cancelada.");return}this.showToast("Borrando todas las ventas..."),S.collection("sales").get().then(e=>{const s=S.batch();return e.docs.forEach(o=>{s.delete(o.ref)}),s.commit()}).then(()=>{this.showToast("✅ Todas las ventas han sido eliminadas"),setTimeout(()=>location.reload(),1500)}).catch(e=>{console.error(e),alert("Error al borrar ventas: "+e.message)})},async findProductBySku(t){try{const e=await S.collection("products").where("sku","==",t).get();if(e.empty)return null;const s=e.docs[0];return{id:s.id,ref:s.ref,data:s.data()}}catch(e){return console.error("Error finding product by SKU:",e),null}},logInventoryMovement(t,e){let s="";t==="EDIT"?s="Producto actualizado":t==="ADD"?s="Ingreso de inventario":t==="DELETE"?s="Egreso manual":t==="SOLD"&&(s="Venta registrada"),S.collection("inventory_logs").add({type:t,sku:e.sku||"Unknown",album:e.album||"Unknown",artist:e.artist||"Unknown",timestamp:firebase.firestore.FieldValue.serverTimestamp(),details:s}).catch(o=>console.error("Error logging movement:",o))},openInventoryLogModal(){S.collection("inventory_logs").orderBy("timestamp","desc").limit(50).get().then(t=>{const s=`
        ...
        `;try{const s=R,r=await(await fetch(`${s}/discogs/sync`,{method:"POST",headers:{"Content-Type":"application/json"}})).json(),n=await(await fetch(`${s}/discogs/sync-orders`,{method:"POST",headers:{"Content-Type":"application/json"}})).json();if(r.success||n&&n.success){let i=`✅ Sincronizado: ${r.synced||0} productos`;n&&n.salesCreated>0&&(i+=`. ¡Detectadas ${n.salesCreated} nuevas ventas!`),this.showToast(i),await this.loadData(),this.refreshCurrentView()}else throw new Error(r.error||n&&n.error||"Error desconocido")}catch(s){console.error("Sync error:",s),this.showToast(`❌ Error al sincronizar: ${s.message}`)}finally{t.disabled=!1,t.innerHTML=e}},formatCurrency(t,e=!0){const s=new Intl.NumberFormat("da-DK",{style:"currency",currency:"DKK"}).format(t);return e?`<span class="blur-money">${s}</span>`:`<span>${s}</span>`},formatDate(t){return t?new Date(t).toLocaleDateString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric"}):"-"},getMonthName(t){return["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"][t]},generateId(){return Date.now().toString(36)+Math.random().toString(36).substr(2)},showToast(t){const e=document.getElementById("toast");document.getElementById("toast-message").innerHTML=t,e.classList.remove("opacity-0","-translate-y-20","md:translate-y-20"),setTimeout(()=>{e.classList.add("opacity-0","-translate-y-20","md:translate-y-20")},3e3)},setupNavigation(){},setupMobileMenu(){},togglePrivacyMode(){this.state.privacyMode=!this.state.privacyMode,this.state.privacyMode?document.body.classList.add("privacy-active"):document.body.classList.remove("privacy-active");const t=this.state.privacyMode?"ph-bold ph-eye-slash":"ph-bold ph-eye",e=document.querySelector("#privacy-toggle-desktop i"),s=document.querySelector("#privacy-toggle-mobile i");e&&(e.className=t),s&&(s.className=t),this.showToast(this.state.privacyMode?"🔒 Modo Privacidad Activado":"👁️ Modo Privacidad Desactivado")},toggleMobileMenu(){const t=document.getElementById("mobile-menu"),e=document.getElementById("mobile-menu-overlay");!t||!e||(t.classList.contains("translate-y-full")?(t.classList.remove("translate-y-full"),e.classList.remove("hidden")):(t.classList.add("translate-y-full"),e.classList.add("hidden")))},toggleMonthFilter(t){const e=this.state.filterMonths.indexOf(t);e===-1?this.state.filterMonths.push(t):this.state.filterMonths.length>1&&this.state.filterMonths.splice(e,1),this.state.filterMonths.sort((s,o)=>s-o),this.refreshCurrentView()},renderDashboard(t){var e,s;try{const o=this.state.filterMonths,r=this.state.filterYear,a=["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"],n=this.state.sales.filter(h=>{var K;const k=(K=h.timestamp)!=null&&K.toDate?h.timestamp.toDate():new Date(h.timestamp||h.date);return k.getFullYear()===r&&o.includes(k.getMonth())}),i=[...n].sort((h,k)=>{var A,L;const K=(A=h.timestamp)!=null&&A.toDate?h.timestamp.toDate():new Date(h.timestamp||h.date);return((L=k.timestamp)!=null&&L.toDate?k.timestamp.toDate():new Date(k.timestamp||k.date))-K}),l=[...this.state.sales.map(h=>({...h,type:"sale",sortDate:new Date(h.date)})),...this.state.expenses.map(h=>({...h,type:"expense",sortDate:new Date(h.date||h.fecha_factura)}))].sort((h,k)=>k.sortDate-h.sortDate).slice(0,5),d=[],c=[];for(let h=29;h>=0;h--){const k=new Date;k.setDate(k.getDate()-h);const K=k.toISOString().split("T")[0];d.push(k.getDate());const B=this.state.sales.filter(A=>A.date===K).reduce((A,L)=>A+(Number(L.total||L.total_amount)||0),0);c.push(B)}const u=new Date,p=u.getMonth(),v=u.getFullYear(),m=p===0?11:p-1,x=p===0?v-1:v,w=this.state.sales.filter(h=>{const k=new Date(h.date);return k.getMonth()===p&&k.getFullYear()===v}).reduce((h,k)=>h+(Number(k.originalTotal||k.total_amount||k.total)||0),0)+(this.state.extraIncome||[]).filter(h=>{const k=new Date(h.date);return k.getMonth()===p&&k.getFullYear()===v}).reduce((h,k)=>h+(Number(k.amount)||0),0),b=this.state.sales.filter(h=>{const k=new Date(h.date);return k.getMonth()===m&&k.getFullYear()===x}).reduce((h,k)=>h+(Number(k.originalTotal||k.total_amount||k.total)||0),0)+(this.state.extraIncome||[]).filter(h=>{const k=new Date(h.date);return k.getMonth()===m&&k.getFullYear()===x}).reduce((h,k)=>h+(Number(k.amount)||0),0),f=b>0?(w-b)/b*100:0,$=`${f>=0?"+":""}${f.toFixed(1)}% vs ${this.getMonthName(m)}`;let E=0,_=0,D=0,y=0,I=0,C=0,M=0,V=0;n.forEach(h=>{var we;const k=((we=h.channel)==null?void 0:we.toLowerCase())==="discogs",K=Number(h.originalTotal)||Number(h.total_amount)||Number(h.total)||0,B=Number(h.total)||Number(h.total_amount)||0,A=k?K-B:0,L=Number(h.shipping_cost)||0;E+=K,D+=L;let oe=0;const me=h.items||[];me.length>0?me.forEach(X=>{const pe=Number(X.priceAtSale||X.unitPrice||X.price)||0,Ee=Number(X.qty||X.quantity)||1;let xe=Number(X.costAtSale||X.cost)||0;const Le=(X.owner||"").toLowerCase();let ke=X.providerOrigin||X.provider_origin;const je=pe*Ee;if(xe===0||!ke){const ge=X.productId||X.recordId,Ce=X.album,Ie=this.state.inventory.find(Ae=>ge&&(Ae.id===ge||Ae.sku===ge)||Ce&&Ae.album===Ce);Ie&&(xe===0&&(xe=Ie.cost||0),ke||(ke=Ie.provider_origin))}if(ke||(ke="Local_Used"),ke==="EU_B2B"||ke==="DK_B2B")I+=je*.2;else{const ge=je-xe*Ee;C+=ge>0?ge*.2:0}if(Le==="el cuartito"||Le==="")xe=Number(X.costAtSale||X.cost)||0;else{if(xe===0||isNaN(xe)){const ge=this.state.consignors?this.state.consignors.find(Ie=>(Ie.name||"").toLowerCase()===Le):null,Ce=ge&&(ge.agreementSplit||ge.split)||70;xe=pe*(Number(Ce)||70)/100}y+=xe*Ee}oe+=(pe-xe)*Ee}):(oe=K,I+=K*.2);const ce=parseFloat(h.shipping_income||h.shipping||h.shipping_cost||0);ce>0&&(M+=ce*.2,V+=ce),_+=oe-A}),(this.state.extraIncome||[]).filter(h=>{const k=new Date(h.date);return k.getFullYear()===r&&o.includes(k.getMonth())}).forEach(h=>{const k=Number(h.amount)||0,K=Number(h.vatAmount)||0;E+=k,_+=k,I+=K});const G=this.state.expenses.filter(h=>{var B;const k=h.fecha_factura?new Date(h.fecha_factura):(B=h.timestamp)!=null&&B.toDate?h.timestamp.toDate():new Date(h.timestamp||h.date);return(h.categoria_tipo==="operativo"||h.categoria_tipo==="stock_nuevo"||h.is_vat_deductible)&&k.getFullYear()===r&&o.includes(k.getMonth())}).reduce((h,k)=>h+(parseFloat(k.monto_iva)||0),0),Z=(this.state.inventory||[]).filter(h=>{if(!h.item_phantom_vat||h.item_phantom_vat<=0||h.provider_origin!=="EU_B2B")return!1;const k=h.acquisition_date?new Date(h.acquisition_date):null;return k?k.getFullYear()===r&&o.includes(k.getMonth()):!1}).reduce((h,k)=>h+(k.item_phantom_vat||0),0),Q=(this.state.inventory||[]).filter(h=>{if(!h.item_real_vat||h.item_real_vat<=0||h.provider_origin!=="DK_B2B")return!1;const k=h.acquisition_date?new Date(h.acquisition_date):null;return k?k.getFullYear()===r&&o.includes(k.getMonth()):!1}).reduce((h,k)=>h+(k.item_real_vat||0),0),ee=I+C+M+Z,te=G+Z+Q,T=ee-te,O=this.state.expenses.filter(h=>{const k=new Date(h.date||h.fecha_factura);return k.getFullYear()===r&&o.includes(k.getMonth())}).reduce((h,k)=>h+(Number(k.monto_total||k.amount)||0),0),se=_-T-O,ae=_-T,j=this.state.inventory.reduce((h,k)=>h+k.price*k.stock,0),ie=this.state.inventory.reduce((h,k)=>h+k.stock,0),de=this.state.inventory.filter(h=>h.stock>0&&h.stock<1),H=this.state.sales.filter(h=>{var k;return h.fulfillment_status==="preparing"||h.status==="paid"||((k=h.channel)==null?void 0:k.toLowerCase())==="discogs"&&h.status!=="shipped"&&h.fulfillment_status!=="shipped"}),W=T,N=o.length===12?`Año ${r} `:`${o.map(h=>this.getMonthName(h)).join(", ")} ${r} `,P=this.state.dashboardAnalysisMode||"genre",U={},z={};let F=0,q=0,ue=0;n.forEach(h=>{const k=h.items||[],K=(B,A)=>{const L=this.state.inventory.find(pe=>B&&(pe.id===B||pe.sku===B)||A&&pe.album===A);if(!L)return null;if(P==="storage")return L.storageLocation||null;const oe=[L.genre,L.genre2,L.genre3,L.genre4,L.genre5].filter(Boolean),me=[];oe.forEach(pe=>{me.push(...pe.split(",").map(Ee=>Ee.trim()).filter(Boolean))});const ce=[...new Set(me)],we=ce.filter(pe=>pe.toLowerCase()!=="electronic");return(we.length>0?we:ce.length>0?ce:["Otros"])[0]||null};if(k.length>0)k.forEach(B=>{const A=B.productId||B.recordId,L=K(A,B.album)||(P==="storage"?"Sin ubicación":h.genre||"Otros"),oe=Number(B.qty||B.quantity)||1,me=Number(B.priceAtSale||B.unitPrice||B.price)||0;U[L]=(U[L]||0)+oe,z[L]=(z[L]||0)+me*oe,F+=oe,(B.productCondition||B.condition||"Used")==="New"?q+=oe:ue+=oe});else{const B=Number(h.quantity)||1,A=Number(h.originalTotal||h.total_amount||h.total)||0,L=P==="storage"?"Sin ubicación":h.genre||"Otros";U[L]=(U[L]||0)+B,z[L]=(z[L]||0)+A,F+=B,ue+=B}});const ne=Object.entries(U).sort((h,k)=>k[1]-h[1]),ve=Object.entries(z).sort((h,k)=>k[1]-h[1]),ye=ve.length>0?{name:ve[0][0],revenue:ve[0][1]}:{name:"N/A",revenue:0},Te=n.length>0?E/n.length:0,De=F>0?Math.round(q/F*100):0,be=F>0?Math.round(ue/F*100):0,le=["#FF6B4A","#F59E0B","#14B8A6","#8B5CF6","#F43F5E","#0EA5E9","#84CC16","#D946EF","#64748B"],he=P==="storage"?"Análisis por Ubicación":"Análisis por Género Musical",$e=P==="storage"?"ph-map-pin":"ph-music-notes-simple",Me=P==="storage"?"Ubicación Más Rentable":"Género Más Rentable",Be=`
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-slate-500 text-sm">Monitor de actividad: <span class="font-bold text-brand-orange">${N}</span></p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
                        ...
                        <div class="flex items-center gap-3">
                            <select onchange="app.state.dashboardAnalysisMode = this.value; app.renderDashboard(document.getElementById('app-content'))"
                                ...
                                <option value="genre" ${P==="genre"?"selected":""}>🎵 Por Género</option>
                                <option value="storage" ${P==="storage"?"selected":""}>📍 Por Ubicación</option>
                            </select>
                            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
                                <i class="ph-bold ph-vinyl-record mr-1"></i> ${F} unidades vendidas
                            </span>
                        </div>
                    </div>
                    ${ne.length>0?`
                        ...
                        <div class="mt-2">
                            ...
                            <div class="flex justify-between mt-2.5">
                                <span class="text-[10px] font-bold text-teal-600 flex items-center gap-1">
                                    <span class="inline-block w-2 h-2 rounded-full bg-teal-500"></span> Nuevo ${De}% (${q})
                                </span>
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-left">
                                    <tbody class="divide-y divide-slate-50">
                                        ${l.map(h=>{const k=h.type==="sale",K=k?h.album||"Venta de Items":h.proveedor||h.description||"Gasto registrado",B=k?h.channel||"Tienda Local":h.categoria||"Operativo";let A="ph-receipt";if(k){const L=(h.channel||"").toLowerCase();L.includes("web")&&(A="ph-globe-simple"),L.includes("discogs")&&(A="ph-vinyl-record")}else A="ph-credit-card";return`
                                                    ...
                                                    <td class="px-6 py-4">
                                                     ...
                                                     <td class="px-6 py-4 text-right">
                                                         <span class="font-bold text-sm ${k?"text-brand-dark":"text-red-500"}">
                                                            ${k?"":"-"}${this.formatCurrency(h.total||h.monto_total||h.amount||0)}
                                                         </span>
                                                     </td>
                                                 </tr>
        ...
        `;t.innerHTML=Be,this.renderDashboardCharts(n,d,c);const Se=(e=document.getElementById("genreDonutChart"))==null?void 0:e.getContext("2d");if(Se&&ne.length>0){this.genreDonutChartInstance&&this.genreDonutChartInstance.destroy();const h=ne.map(B=>B[0]),k=ne.map(B=>B[1]),K=ne.map((B,A)=>le[A%le.length]);this.genreDonutChartInstance=new Chart(Se,{type:"doughnut",data:{labels:h,datasets:[{data:k,backgroundColor:K,borderWidth:0,hoverOffset:8}]},options:{responsive:!0,maintainAspectRatio:!1,cutout:"62%",plugins:{legend:{position:"bottom",labels:{boxWidth:12,boxHeight:12,borderRadius:3,useBorderRadius:!0,padding:14,font:{size:11,weight:"600",family:"'DM Sans', sans-serif"},color:"#334155"}},tooltip:{backgroundColor:"#1e293b",titleFont:{size:11,weight:"700"},bodyFont:{size:13,weight:"700"},padding:14,cornerRadius:12,callbacks:{label:B=>{const A=B.dataset.data.reduce((oe,me)=>oe+me,0),L=(B.parsed/A*100).toFixed(1);return` ${B.parsed} uds — ${L}%`}}}}},plugins:[{id:"centerText",beforeDraw(B){const{ctx:A,chartArea:L}=B;if(!L)return;A.save();const oe=(L.left+L.right)/2,me=(L.top+L.bottom)/2,ce=Math.min(L.right-L.left,L.bottom-L.top)/7;A.font=`bold ${ce}px 'DM Sans', sans-serif`,A.textBaseline="middle",A.textAlign="center",A.fillStyle="#1e293b";const we=B.data.datasets[0].data.reduce((X,pe)=>X+pe,0);A.fillText(we,oe,me-ce*.35),A.font=`600 ${ce*.42}px 'DM Sans', sans-serif`,A.fillStyle="#94a3b8",A.fillText("unidades",oe,me+ce*.55),A.restore()}}]})}const _e=(s=document.getElementById("genreBarChart"))==null?void 0:s.getContext("2d");if(_e&&ne.length>0){this.genreBarChartInstance&&this.genreBarChartInstance.destroy();const h=ne.map(B=>B[0]),k=ne.map(B=>B[1]),K=ne.map((B,A)=>le[A%le.length]);this.genreBarChartInstance=new Chart(_e,{type:"bar",data:{labels:h,datasets:[{label:"Unidades",data:k,backgroundColor:K.map(B=>B+"30"),borderColor:K,borderWidth:2,borderRadius:8,borderSkipped:!1,barThickness:28}]},options:{responsive:!0,maintainAspectRatio:!1,indexAxis:"y",plugins:{legend:{display:!1},tooltip:{backgroundColor:"#1e293b",titleFont:{size:11,weight:"700"},bodyFont:{size:13,weight:"700"},padding:14,cornerRadius:12,callbacks:{label:B=>` ${B.parsed.x} unidades vendidas`}}},scales:{x:{beginAtZero:!0,grid:{color:"#f1f5f9"},ticks:{font:{size:10,weight:"600"},color:"#94a3b8"}},y:{grid:{display:!1},ticks:{font:{size:11,weight:"700",family:"'DM Sans', sans-serif"},color:"#334155",padding:8}}}}})}}catch(o){console.error("Dashboard render error:",o),t.innerHTML=`<div class="p-12 text-center text-red-500 font-bold bg-red-50 rounded-3xl m-8 border border-red-100">
    ...
    `},renderInventoryContent(t,e,s,o,r){t.innerHTML=`
            ...
            `}
        `},renderInventory(t){const e=[...new Set(this.state.inventory.flatMap(x=>{const w=[x.genre,x.genre2,x.genre3,x.genre4,x.genre5].filter(Boolean),b=[];w.forEach(E=>{b.push(...E.split(",").map(_=>_.trim()).filter(Boolean))});const f=[...new Set(b)],$=f.filter(E=>E.toLowerCase()!=="electronic");return $.length>0?$:f.length>0?f:["Otros"]}))].sort(),s=[...new Set(this.state.inventory.map(x=>x.owner).filter(Boolean))].sort(),o=[...new Set(this.state.inventory.map(x=>x.label).filter(Boolean))].sort(),r=[...new Set(this.state.inventory.map(x=>x.storageLocation).filter(Boolean))].sort(),a=this.getFilteredInventory(),n=this.state.sortBy||"dateDesc";a.sort((x,w)=>{if(n==="priceDesc")return(w.price||0)-(x.price||0);if(n==="priceAsc")return(x.price||0)-(w.price||0);if(n==="stockDesc")return(w.stock||0)-(x.stock||0);const b=x.created_at?x.created_at.seconds?x.created_at.seconds*1e3:new Date(x.created_at).getTime():0,f=w.created_at?w.created_at.seconds?w.created_at.seconds*1e3:new Date(w.created_at).getTime():0;return n==="dateDesc"?f-b:n==="dateAsc"?b-f:0});const i=this.state.inventory.length,l=this.state.inventory.reduce((x,w)=>{const b=Number(w.stock)||0;return x+(b>0?(parseFloat(w.price)||0)*b:0)},0),d=this.state.inventory.filter(x=>(x.stock||0)>0).length,c=this.state.inventory.filter(x=>x.discogs_listing_id).length,u=[this.state.filterGenre!=="all"?1:0,this.state.filterOwner!=="all"?1:0,this.state.filterLabel!=="all"?1:0,this.state.filterStorage!=="all"?1:0,this.state.filterDiscogs&&this.state.filterDiscogs!=="all"?1:0,this.state.filterHero&&this.state.filterHero!=="all"?1:0].reduce((x,w)=>x+w,0);document.getElementById("inventory-layout-root")||(t.innerHTML=`
    ...
    `);const p=document.getElementById("inventory-kpi-container");p&&(p.innerHTML=`
                <div class="kpi-card">
                ...
                <div class="kpi-card">
                    ...
                    <p class="text-xl font-bold text-emerald-600 font-display mt-1">${d} <span class="text-xs text-slate-400 font-normal">/ ${i}</span></p>
                </div>
                <div class="kpi-card">
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">En Discogs</p>
                    <p class="text-xl font-bold text-purple-600 font-display mt-1">${c} <span class="text-xs text-slate-400 font-normal">/ ${i}</span></p>
                </div>
            `);const v=document.getElementById("inventory-filters-container");v&&(v.innerHTML=`
                ...
                ${u>0||this.state.filterStockTime.length>0?`
                    <button onclick="app.state.filterGenre='all'; app.state.filterOwner='all'; app.state.filterLabel='all'; app.state.filterStorage='all'; app.state.filterDiscogs='all'; app.state.filterHero='all'; app.state.filterStockTime=[]; app.refreshCurrentView()" class="filter-chip hover:!bg-red-50 hover:!border-red-300 hover:!text-red-500">
                        <i class="ph-bold ph-x text-xs"></i> Limpiar (${u+this.state.filterStockTime.length})
                    </button>
                `:""}
            `),this.renderInventoryCart();const m=document.getElementById("inventory-content-container");m&&this.renderInventoryContent(m,a,e,s,r)},getTimeInStockCategory(t){if(!t)return"unknown";const e=t.seconds?new Date(t.seconds*1e3):new Date(t);if(isNaN(e.getTime()))return"unknown";const o=Math.abs(new Date-e),a=Math.ceil(o/(1e3*60*60*24))/30.44;return a<=2?"green":a<=4?"orange":a<=6?"red":"purple"},getTimeInStockBadge(t){switch(t){case"green":return'<span class="w-3 h-3 block rounded-full bg-emerald-500 shadow-sm" title="Antigüedad: 0 a 2 meses"></span>';case"orange":return'<span class="w-3 h-3 block rounded-full bg-orange-500 shadow-sm" title="Antigüedad: 2 a 4 meses"></span>';case"red":return'<span class="w-3 h-3 block rounded-full bg-red-500 shadow-sm" title="Antigüedad: 4 a 6 meses"></span>';case"purple":return'<span class="w-3 h-3 block rounded-full bg-purple-500 shadow-sm" title="Antigüedad: Más de 6 meses"></span>';default:return'<span class="w-3 h-3 block rounded-full bg-slate-300 shadow-sm" title="Antigüedad: Desconocida"></span>'}},toggleStockTimeFilter(t){const e=this.state.filterStockTime.indexOf(t);e===-1?this.state.filterStockTime.push(t):this.state.filterStockTime.splice(e,1),this.refreshCurrentView()},getStatusBadge(t){return`<span class="text-[10px] font-bold px-2 py-0.5 rounded-md border ${{NM:"bg-green-100 text-green-700 border-green-200","VG+":"bg-blue-100 text-blue-700 border-blue-200",VG:"bg-yellow-100 text-yellow-700 border-yellow-200",G:"bg-orange-100 text-orange-700 border-orange-200",B:"bg-red-100 text-red-700 border-red-200",S:"bg-purple-100 text-purple-700 border-purple-200"}[t]||"bg-slate-100 text-slate-600 border-slate-200"}"> ${t}</span> `},renderCharts(t,e){const s=this.state.filterMonths;this.state.filterYear;const o=[],r=[],a=[];s.forEach(i=>{o.push(this.getMonthName(i).substring(0,3));const l=t.filter(c=>new Date(c.date).getMonth()===i).reduce((c,u)=>c+u.total,0),d=e.filter(c=>new Date(c.date).getMonth()===i).reduce((c,u)=>c+u.amount,0);r.push(l),a.push(d)});const n={};t.forEach(i=>{n[i.genre]=(n[i.genre]||0)+i.quantity}),new Chart(document.getElementById("financeChart"),{type:"bar",data:{labels:o,datasets:[{label:"Ventas",data:r,backgroundColor:"#F05A28",borderRadius:6},{label:"Gastos",data:a,backgroundColor:"#94a3b8",borderRadius:6}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{position:"bottom"}},scales:{y:{grid:{color:"#f1f5f9"},beginAtZero:!0},x:{grid:{display:!1}}}}})},renderDashboardCharts(t=[],e=[],s=[]){var v,m,x,w,b;const o=t,r=(v=document.getElementById("last30DaysChart"))==null?void 0:v.getContext("2d");r&&(this.last30ChartInstance&&this.last30ChartInstance.destroy(),this.last30ChartInstance=new Chart(r,{type:"line",data:{labels:e,datasets:[{label:"Ventas ($)",data:s,borderColor:"#F05A28",backgroundColor:f=>{const $=f.chart,{ctx:E,chartArea:_}=$;if(!_)return null;const D=E.createLinearGradient(0,_.top,0,_.bottom);return D.addColorStop(0,"rgba(240, 90, 40, 0.2)"),D.addColorStop(1,"rgba(240, 90, 40, 0)"),D},borderWidth:3,fill:!0,tension:.4,pointRadius:0,pointHoverRadius:6,pointBackgroundColor:"#F05A28",pointBorderColor:"#fff",pointBorderWidth:2}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1},tooltip:{mode:"index",intersect:!1,backgroundColor:"#1e293b",titleFont:{size:10},bodyFont:{size:12,weight:"bold"},padding:12,cornerRadius:12,displayColors:!1,callbacks:{label:f=>this.formatCurrency(f.parsed.y)}}},scales:{y:{beginAtZero:!0,grid:{color:"#f8fafc"},ticks:{font:{size:10},color:"#94a3b8"}},x:{grid:{display:!1},ticks:{font:{size:10},color:"#94a3b8",autoSkip:!0,maxRotation:0,callback:function(f,$){return $%5===0?this.getLabelForValue(f):""}}}},interaction:{mode:"index",intersect:!1}}}));const a=(f,$)=>({type:"doughnut",data:{labels:Object.keys(f),datasets:[{data:Object.values(f),backgroundColor:["#F05A28","#FDE047","#8b5cf6","#10b981","#f43f5e","#64748b"],borderWidth:0}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{position:"right",labels:{boxWidth:10,font:{size:10}}}}}}),n={};o.forEach(f=>{const $=f.genre||"Otros";let E=Number(f.quantity)||0;E===0&&f.items&&Array.isArray(f.items)&&(E=f.items.reduce((_,D)=>_+(Number(D.qty||D.quantity)||1),0)),E<=0&&(E=1),n[$]=(n[$]||0)+Number(E)}),this.genreChartInstance&&this.genreChartInstance.destroy();const i=(m=document.getElementById("genreChart"))==null?void 0:m.getContext("2d");i&&(this.genreChartInstance=new Chart(i,a(n)));const l={};o.forEach(f=>{const $=f.paymentMethod||"Otros";let E=Number(f.quantity)||0;E===0&&f.items&&Array.isArray(f.items)&&(E=f.items.reduce((_,D)=>_+(Number(D.qty||D.quantity)||1),0)),E<=0&&(E=1),l[$]=(l[$]||0)+Number(E)}),this.paymentChartInstance&&this.paymentChartInstance.destroy();const d=(x=document.getElementById("paymentChart"))==null?void 0:x.getContext("2d");d&&(this.paymentChartInstance=new Chart(d,a(l)));const c={};o.forEach(f=>{const $=f.channel||"Tienda";let E=Number(f.quantity)||0;E===0&&f.items&&Array.isArray(f.items)&&(E=f.items.reduce((_,D)=>_+(Number(D.qty||D.quantity)||1),0)),E<=0&&(E=1),c[$]=(c[$]||0)+Number(E)}),this.channelChartInstance&&this.channelChartInstance.destroy();const u=(w=document.getElementById("channelChart"))==null?void 0:w.getContext("2d");u&&(this.channelChartInstance=new Chart(u,a(c)));const p=(b=document.getElementById("salesTrendChart"))==null?void 0:b.getContext("2d");if(p){const f=new Array(31).fill(0).map((E,_)=>_+1),$=new Array(31).fill(0);o.forEach(E=>{const _=new Date(E.date);isNaN(_.getDate())||($[_.getDate()-1]+=parseFloat(E.total)||0)}),this.trendChartInstance&&this.trendChartInstance.destroy(),this.trendChartInstance=new Chart(p,{type:"line",data:{labels:f,datasets:[{label:"Ventas ($)",data:$,borderColor:"#F05A28",backgroundColor:"rgba(240, 90, 40, 0.1)",borderWidth:3,fill:!0,tension:.4,pointRadius:2}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{y:{beginAtZero:!0,grid:{color:"#f1f5f9"}},x:{grid:{display:!1}}}}})}},updateFilter(t,e){if(t==="month"){const s=parseInt(e);this.state.filterMonth=s,this.state.filterMonths=[s]}t==="year"&&(this.state.filterYear=parseInt(e)),this.renderDashboard(document.getElementById("app-content"))},renderSales(t){var w;const e=new Date().toISOString().split("T")[0],s=new Date(Date.now()-864e5).toISOString().split("T")[0],o=this.state.sales.filter(b=>b.date===e).reduce((b,f)=>b+(parseFloat(f.total)||0),0),r=this.state.sales.filter(b=>b.date===s).reduce((b,f)=>b+(parseFloat(f.total)||0),0),a=this.state.sales.filter(b=>b.fulfillment_status==="preparing"||b.status==="paid"||b.channel==="Discogs"&&b.status!=="shipped").length,n=this.state.filterYear,i=this.state.filterMonths,l=((w=document.getElementById("sales-payment-filter"))==null?void 0:w.value)||"all",c=this.state.salesHistorySearch.toLowerCase().split(" ").filter(b=>b.length>0),u=this.state.orderFeedFilter||"all",p=this.state.sales.filter(b=>{const f=new Date(b.date),$=f.getFullYear()===n&&i.includes(f.getMonth()),E=l==="all"||b.paymentMethod===l;let _=!0;c.length>0&&(_=c.every(y=>{const I=Array.isArray(b.items)&&b.items.some(M=>{var Z,Q,ee,te;const V=(M.album||((Z=M.record)==null?void 0:Z.album)||"").toLowerCase(),Y=(M.artist||((Q=M.record)==null?void 0:Q.artist)||"").toLowerCase(),J=(M.label||((ee=M.record)==null?void 0:ee.label)||"").toLowerCase(),G=(M.sku||((te=M.record)==null?void 0:te.sku)||"").toLowerCase();return V.includes(y)||Y.includes(y)||J.includes(y)||G.includes(y)}),C=(b.album||"").toLowerCase().includes(y)||(b.sku||"").toLowerCase().includes(y)||(b.customerName||"").toLowerCase().includes(y)||(b.orderNumber||"").toLowerCase().includes(y);return I||C}));let D=!0;return u==="to_ship"?D=b.status!=="shipped"&&b.source!=="STORE":u==="completed"?D=b.status==="shipped":u==="store"&&(D=b.source==="STORE"),$&&E&&_&&D}),v=p.reduce((b,f)=>b+(parseFloat(f.total)||0),0),m=p.length>0?v/p.length:0,x=`
            <div class="max-w-7xl mx-auto px-4 md:px-8 pb-24 md:pb-8 pt-6">
                <!-- Header Component -->
                <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-6 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    ...
                    <div class="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                        ...
                        <div class="flex flex-wrap gap-1">
                            ${["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"].map((b,f)=>`
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-slate-50/50 rounded-3xl p-6 border border-slate-100">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ${["El Cuartito",...this.state.consignors.map(b=>b.name)].map(b=>{const f=this.state.inventory.filter($=>$.owner===b).reduce(($,E)=>$+E.stock,0);return`
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-3 max-h-[80vh] overflow-y-auto pr-2 custom-scrollbar pb-10">
                            ${p.map(b=>{const f=b.status==="shipped",$=b.status==="paid"||b.source==="STORE"||b.paymentMethod!=="Pending",E=b.channel==="Discogs",_=b.source==="STORE",D=b.items&&b.items.length>0?b.items[0]:{album:b.album||"Venta Manual",artist:b.artist||"Desconocido"},y=b.items&&b.items.length>1?b.items.length-1:0;return`
                                <div class="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm hover:border-slate-200 transition-all cursor-pointer group flex items-center gap-4 relative" onclick="app.openUnifiedOrderDetailModal('${b.id}')">
                                    <!-- Source Icon -->
                                    <div class="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${E?"bg-slate-900 text-white":_?"bg-orange-100 text-brand-orange":"bg-blue-100 text-blue-600"}">
                                        <i class="ph-bold ${E?"ph-disc":_?"ph-storefront":"ph-globe"} text-xl"></i>
                                    </div>
        ...
        `;if(t.innerHTML=x,this.state.salesHistorySearch){const b=document.getElementById("sales-history-search");if(b){b.focus();const f=b.value;b.value="",b.value=f}}},renderSalesCartWidget(){return`
    ...
    <div id="modal-overlay" class="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
                                    <table class="w-full text-sm">
                                        ...
                                        <tbody class="divide-y divide-slate-50">
                                            ${(e.items||[]).map(b=>{var f,$,E,_,D;return`
                                                <tr>
                                                    <td class="px-4 py-4">
                                                        <div class="flex items-center gap-3">
                                                            <img src="${b.image||b.cover_image||((f=b.record)==null?void 0:f.cover_image)||"https://elcuartito.dk/default-vinyl.png"}" class="w-10 h-10 rounded-lg object-cover shadow-sm">
                                                            <div>
                                                                <p class="font-bold text-brand-dark">${b.album||(($=b.record)==null?void 0:$.album)||"Desconocido"}</p>
                                                                <p class="text-[10px] text-slate-500">${b.artist||((E=b.record)==null?void 0:E.artist)||""}</p>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td class="px-4 py-4 text-center font-mono text-xs text-slate-400">${b.sku||((_=b.record)==null?void 0:_.sku)||"-"}</td>
                                                    <td class="px-4 py-4 text-center font-medium">${b.quantity||b.qty||1}</td>
                                                    <td class="px-4 py-4 text-right font-bold text-brand-dark">${this.formatCurrency(b.unitPrice||b.priceAtSale||((D=b.record)==null?void 0:D.price)||0)}</td>
                                                </tr>
                                            `}).join("")}
                                        </tbody>
                                    </table>
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="relative pl-4 border-l-2 border-slate-100 space-y-6">
                                    ${a.map((b,f)=>{const $=n(b.status);return`
                                            <div class="relative">
        ...
        `;document.body.insertAdjacentHTML("beforeend",p)},openInvoiceModal(t){var f;const e=this.state.sales.find($=>$.id===t);if(!e){this.showToast("Sale not found","error");return}const s=e.items||[],o=(f=e.date)!=null&&f.toDate?e.date.toDate():new Date(e.date||e.timestamp),r=o.toISOString().slice(0,10).replace(/-/g,""),a=e.invoiceNumber||`ECR-${r}-${t.slice(-4).toUpperCase()}`,n=s.filter($=>$.productCondition==="New"),i=s.filter($=>$.productCondition!=="New");let l=0,d=0;const c=($,E)=>$.map(_=>{const D=_.priceAtSale||_.price||0,y=_.qty||_.quantity||1,I=D*y;if(d+=I,E){const C=I*.2;return l+=C,`
                        <tr>
                            <td style="padding: 12px 0; border-bottom: 1px solid #eee;">
                                <div style="font-weight: bold;">${_.album||"Product"}</div>
                                <div style="font-size: 11px; color: #666;">${_.artist||""}</div>
                    ...
                    `}).join("");let u="";n.length>0&&i.length>0?u=`
                ...
                ${c(i,!1)}
            `:u=c(n,!0)+c(i,!1);const p=parseFloat(e.shipping_income||e.shipping||e.shipping_cost||0),v=p*.2,m=d+p,x=e.customer?`${e.customer.firstName||""} ${e.customer.lastName||""}`.trim():e.customerName||"Customer",w=e.customer?`${e.customer.address||""}<br>${e.customer.postalCode||""} ${e.customer.city||""}<br>${e.customer.country||""}`:"",b=`
        ...
        `),e.document.close()},navigateInventoryFolder(t,e){t==="genre"&&(this.state.filterGenre=e),t==="owner"&&(this.state.filterOwner=e),t==="label"&&(this.state.filterLabel=e),t==="storage"&&(this.state.filterStorage=e),this.refreshCurrentView()},toggleSelection(t){this.state.selectedItems.has(t)?this.state.selectedItems.delete(t):this.state.selectedItems.add(t),this.refreshCurrentView()},async openPrintLabelModal(t){const e=this.state.inventory.find(n=>n.id===t||n.sku===t);if(!e)return;let s={...e};try{const n=await S.collection("products").doc(e.id).get();n.exists&&(s={...e,...n.data()})}catch(n){console.warn("[printLabel] Could not fetch fresh product data, using state copy",n)}const o=s.year&&Number(s.year)!==0?String(s.year):"—",r=s.price?Number(s.price).toLocaleString("da-DK"):"—",a=`
...
`;document.body.insertAdjacentHTML("beforeend",a),setTimeout(()=>{const n=document.getElementById("qr-container");n&&typeof QRCode<"u"&&(n.innerHTML="",new QRCode(n,{text:s.sku,width:57,height:57,colorDark:"#000000",colorLight:"#ffffff",correctLevel:QRCode.CorrectLevel.M}))},50)},closePrintLabelModal(){const t=document.getElementById("print-label-modal");if(t){const e=document.getElementById("label-comment");e&&(e.value=""),t.remove()}},setLabelOrientation(t){const e=document.getElementById("print-label-modal");if(!e)return;e.dataset.orientation=t;const s=document.getElementById("orient-h"),o=document.getElementById("orient-v"),r="flex-1 py-2 bg-white text-brand-dark font-bold rounded-lg text-sm shadow-sm flex items-center justify-center gap-1.5 transition-all",a="flex-1 py-2 text-slate-500 font-bold rounded-lg text-sm flex items-center justify-center gap-1.5 transition-all";s&&(s.className=t==="landscape"?r:a),o&&(o.className=t==="portrait"?r:a);const n=document.getElementById("printable-label");n&&n.classList.toggle("label-b--portrait",t==="portrait")},async confirmPrintLabel(){const t=document.getElementById("label-comment"),e=document.getElementById("preview-comment"),s=t?t.value:"";t&&e&&(e.innerText=s);const o=document.querySelector('#print-label-modal button[onclick="app.confirmPrintLabel()"]');o&&(o.disabled=!0,o.innerHTML='<i class="ph-bold ph-circle-notch animate-spin"></i> Imprimiendo…');try{const r=document.getElementById("print-label-modal"),a=r&&r.dataset.orientation||"landscape",i=(await this._drawLabelCanvas(s,a)).toDataURL("image/png").split(",")[1],l=await fetch(`${R}/api/print-label`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({image:i})}),d=await l.json();if(!l.ok)throw new Error(d.error||"Error desconocido");o&&(o.innerHTML='<i class="ph-bold ph-check"></i> ¡Enviado!',o.classList.replace("bg-brand-dark","bg-green-600")),this.showToast("✅ Etiqueta enviada a la impresora"),setTimeout(()=>this.closePrintLabelModal(),1500)}catch(r){o&&(o.disabled=!1,o.innerHTML='<i class="ph-bold ph-printer"></i> Imprimir'),this.showToast("Error al imprimir: "+r.message,"error")}},async _drawLabelCanvas(t,e="landscape"){const s=document.getElementById("print-label-modal"),o=s?s.dataset.sku:null,r=o?this.state.inventory.find(w=>w.id===o||w.sku===o)||{}:{},a=(w,b)=>{const f=document.getElementById(w);return f&&f.value.trim()?f.value.trim():b},n={...r,album:a("label-edit-title",r.album),artist:a("label-edit-artist",r.artist),year:a("label-edit-year",r.year),price:a("label-edit-price",r.price),genre:a("label-edit-genre1",r.genre),genre2:a("label-edit-genre2",r.genre2),condition:a("label-edit-cond",r.condition),storageLocation:a("label-edit-loc",r.storageLocation)},i=300/25.4,l=e==="portrait",d=Math.round((l?40:62)*i),c=Math.round((l?62:40)*i),u=document.createElement("canvas");u.width=d,u.height=c;const p=u.getContext("2d");"wordSpacing"in p&&(p.wordSpacing="0px"),"letterSpacing"in p&&(p.letterSpacing="0px"),p.fillStyle="#ffffff",p.fillRect(0,0,d,c);const v=Math.round(5.5*i);p.fillStyle="#000000",p.fillRect(0,0,d,v);const m=Math.round(2.2*i);p.fillStyle="#ffffff",p.font=`800 ${m}px "DM Sans", Arial, sans-serif`,p.textBaseline="middle",p.textAlign="left";const x=((n.genre||"VINYL")+(n.genre2?" / "+n.genre2:"")).toUpperCase();p.fillText(x,Math.round(1.6*i),v/2);try{const w=await new Promise(b=>{const f=new Image;f.crossOrigin="anonymous",f.onload=()=>b(f),f.onerror=()=>b(null),f.src="logo-broadsheet.png"});if(w){const b=Math.round(3*i),f=Math.round(w.naturalWidth*(b/w.naturalHeight)),$=document.createElement("canvas");$.width=w.naturalWidth,$.height=w.naturalHeight;const E=$.getContext("2d");E.drawImage(w,0,0);const _=E.getImageData(0,0,$.width,$.height),D=_.data;for(let V=0;V<D.length;V+=4)D[V+3]>0&&(D[V]=255,D[V+1]=255,D[V+2]=255);E.putImageData(_,0,0);const I=Math.round(18*i),C=d-I+Math.round((I-f)/2),M=(v-b)/2;p.drawImage($,C,M,f,b)}}catch{}if(l){const w=Math.round(1.6*i),b=d-w*2;p.textAlign="left",p.textBaseline="top";const f=Math.round(3.8*i);p.font=`800 ${f}px "DM Sans", Arial, sans-serif`,p.fillStyle="#000000";const $=this._wrapText(p,n.album||"",b,2);let E=v+Math.round(1.1*i);$.forEach((z,F)=>{p.fillText(z,w,E+F*Math.round(f*1.1))}),E+=$.length*Math.round(f*1.1);const _=Math.round(2.4*i);if(p.font=`600 ${_}px "DM Sans", Arial, sans-serif`,p.fillStyle="rgba(0,0,0,0.5)",E+=Math.round(.5*i),p.fillText(this._truncateText(p,n.artist||"",b),w,E),E+=_,n.label&&n.label!=="Desconocido"){const z=Math.round(2.2*i);E+=Math.round(.6*i),p.font=`700 ${Math.round(1.9*i)}px "DM Sans", Arial, sans-serif`,p.fillStyle="rgba(0,0,0,0.35)",p.fillText("LABEL",w,E);const F=p.measureText("LABEL ").width;p.font=`600 ${z}px "DM Sans", Arial, sans-serif`,p.fillStyle="#333333",p.fillText(this._truncateText(p,n.label,b-F),w+F,E),E+=z}if(t){const z=Math.round(2.1*i);p.font=`italic 600 ${z}px "DM Sans", Arial, sans-serif`,p.fillStyle="rgba(0,0,0,0.4)",p.textBaseline="top";const F=b-Math.round(3*i),q=this._wrapText(p,t,F,5),ue=Math.round(z*1.35),ne=E+Math.round(1.8*i);q.forEach((ve,ye)=>{p.fillText(ve,w+Math.round(1.5*i),ne+ye*ue)})}const D=Math.round(12*i),y=Math.round(20*i),I=Math.round((d-y)/2),C=c-D-Math.round(1.6*i),M=C-Math.round(4.5*i),V=M-Math.round(2.2*i);p.strokeStyle="rgba(0,0,0,0.15)",p.lineWidth=1,p.beginPath(),p.moveTo(w,M),p.lineTo(d-w,M),p.stroke();const Y=Math.round(2*i),J=Math.round(1.8*i),G=b/3;p.textBaseline="middle";const Z=(z,F,q)=>{p.textAlign="left",p.font=`700 ${J}px "DM Sans", Arial, sans-serif`,p.fillStyle="rgba(0,0,0,0.45)",p.fillText(z+" ",q,V);const ue=p.measureText(z+" ").width;p.font=`700 ${Y}px "DM Mono", "Courier New", monospace`,p.fillStyle="rgba(0,0,0,0.7)",p.fillText(F,q+ue,V)};Z("Loc",n.storageLocation||"—",w),Z("Cond",n.condition||"—",w+G),Z("Year",n.year&&Number(n.year)!==0?String(n.year):"—",w+G*2);const Q=Math.round(13*i),ee=Math.round((d-Q)/2),te=Math.round(1.9*i),g=E+Math.round(2.5*i),T=V-Math.round(te+Math.round(.4*i)+4),O=g+Math.max(0,Math.round((T-g-Q)/2)),se=document.getElementById("qr-container"),ae=se?se.querySelector("canvas"):null;ae?p.drawImage(ae,ee,O,Q,Q):(p.strokeStyle="#ccc",p.lineWidth=1,p.strokeRect(ee,O,Q,Q)),p.font=`600 ${te}px "DM Mono", "Courier New", monospace`,p.fillStyle="rgba(0,0,0,0.45)",p.textAlign="center",p.textBaseline="top",p.fillText(n.sku||"",d/2,O+Q+Math.round(.4*i)),p.fillStyle="#000000";const j=Math.round(.8*i);p.beginPath(),p.moveTo(I+j,C),p.lineTo(I+y-j,C),p.quadraticCurveTo(I+y,C,I+y,C+j),p.lineTo(I+y,C+D-j),p.quadraticCurveTo(I+y,C+D,I+y-j,C+D),p.lineTo(I+j,C+D),p.quadraticCurveTo(I,C+D,I,C+D-j),p.lineTo(I,C+j),p.quadraticCurveTo(I,C,I+j,C),p.closePath(),p.fill();const ie=n.price?Number(n.price).toLocaleString("da-DK"):"—",de=Math.round(4.8*i),H=Math.round(2.2*i),W=de*1.1,N=H*1.1,P=C+(D-W-N)/2+W/2,U=P+W/2+N/2;p.textAlign="center",p.textBaseline="middle",p.font=`800 ${de}px "DM Mono", "Courier New", monospace`,p.fillStyle="#ffffff",p.fillText(ie,I+y/2,P),p.font=`700 ${H}px "DM Sans", Arial, sans-serif`,p.fillStyle="#ffffff",p.fillText("DKK",I+y/2,U)}else{const w=v,b=Math.round(18*i),f=d-b,$=Math.round(1.6*i),E=Math.round(1.1*i);p.textAlign="left",p.textBaseline="top";const _=Math.round(3.8*i);p.font=`800 ${_}px "DM Sans", Arial, sans-serif`,p.fillStyle="#000000";const D=f-$-Math.round(.95*i),y=this._wrapText(p,n.album||"",D,2);y.forEach((be,le)=>{p.fillText(be,$,w+E+le*Math.round(_*1.1))});const I=Math.round(2.4*i);p.font=`600 ${I}px "DM Sans", Arial, sans-serif`,p.fillStyle="rgba(0,0,0,0.5)";const C=w+E+y.length*Math.round(_*1.1)+Math.round(.5*i),M=this._truncateText(p,n.artist||"",D);p.fillText(M,$,C);let V=C+I;if(n.label&&n.label!=="Desconocido"){const be=Math.round(2.2*i),le=V+Math.round(.6*i);p.font=`700 ${Math.round(1.9*i)}px "DM Sans", Arial, sans-serif`,p.fillStyle="rgba(0,0,0,0.35)",p.fillText("LABEL",$,le),p.font=`600 ${be}px "DM Sans", Arial, sans-serif`,p.fillStyle="#333333";const he=$+p.measureText("LABEL ").width;p.fillText(this._truncateText(p,n.label,D-he+$),he,le),V=le+be}const Y=c-Math.round(5*i);if(t){const be=Math.round(2.1*i);p.font=`italic 600 ${be}px "DM Sans", Arial, sans-serif`,p.fillStyle="rgba(0,0,0,0.4)",p.textBaseline="top";const le=D-Math.round(2*i),he=this._wrapText(p,t,le,5),$e=Math.round(be*1.35),Me=he.length*$e,Be=V+(Y-V-Me)/2;he.forEach((Se,_e)=>{p.fillText(Se,$+Math.round(1.5*i),Be+_e*$e)})}const J=Math.round(2*i),G=Math.round(1.8*i),Z=c-Math.round(2.5*i),ee=(f-$-Math.round(.95*i))/3;p.strokeStyle="rgba(0,0,0,0.15)",p.lineWidth=1,p.beginPath(),p.moveTo($,Y),p.lineTo(f-Math.round(.95*i),Y),p.stroke(),p.textBaseline="middle",p.textAlign="left";const te=(be,le,he)=>{p.font=`700 ${G}px "DM Sans", Arial, sans-serif`,p.fillStyle="rgba(0,0,0,0.45)",p.fillText(be+" ",he,Z);const $e=p.measureText(be+" ").width;p.font=`700 ${J}px "DM Mono", "Courier New", monospace`,p.fillStyle="rgba(0,0,0,0.7)",p.fillText(le,he+$e,Z)};te("Loc",n.storageLocation||"—",$),te("Cond",n.condition||"—",$+ee),te("Year",n.year&&Number(n.year)!==0?String(n.year):"—",$+ee*2);const g=f;p.strokeStyle="rgba(0,0,0,0.12)",p.lineWidth=1,p.beginPath(),p.moveTo(g,w),p.lineTo(g,c),p.stroke();const T=document.getElementById("qr-container"),O=T?T.querySelector("canvas"):null,ae=Math.round(15*i),j=g+Math.round((b-ae)/2),ie=w+Math.round(1.1*i);O?p.drawImage(O,j,ie,ae,ae):(p.strokeStyle="#ccc",p.strokeRect(j,ie,ae,ae));const de=Math.round(1.9*i);p.font=`600 ${de}px "DM Mono", "Courier New", monospace`,p.fillStyle="rgba(0,0,0,0.45)",p.textAlign="center",p.textBaseline="top",p.fillText(n.sku||"",g+b/2,ie+ae+Math.round(.4*i));const H=Math.round(12*i),W=Math.round(15*i),N=g+Math.round((b-W)/2),P=c-H-Math.round(1.6*i);p.fillStyle="#000000";const U=Math.round(.8*i);p.beginPath(),p.moveTo(N+U,P),p.lineTo(N+W-U,P),p.quadraticCurveTo(N+W,P,N+W,P+U),p.lineTo(N+W,P+H-U),p.quadraticCurveTo(N+W,P+H,N+W-U,P+H),p.lineTo(N+U,P+H),p.quadraticCurveTo(N,P+H,N,P+H-U),p.lineTo(N,P+U),p.quadraticCurveTo(N,P,N+U,P),p.closePath(),p.fill();const z=n.price?Number(n.price).toLocaleString("da-DK"):"—",F=Math.round(4.8*i),q=Math.round(2*i),ue=F*1.1,ne=q*1.1,ve=ue+ne,ye=P+(H-ve)/2+ue/2,Te=ye+ue/2+ne/2;p.textAlign="center",p.textBaseline="middle",p.font=`800 ${F}px "DM Mono", "Courier New", monospace`,p.fillStyle="#ffffff",p.fillText(z,N+W/2,ye);const De=Math.round(2.2*i);p.font=`700 ${De}px "DM Sans", Arial, sans-serif`,p.fillStyle="#ffffff",p.fillText("DKK",N+W/2,Te)}return u},_wrapText(t,e,s,o){const r=e.split(" "),a=[];let n="";for(const i of r){const l=n?n+" "+i:i;if(t.measureText(l).width>s&&n){if(a.push(n),n=i,a.length>=o)break}else n=l}if(n&&a.length<o&&a.push(n),a.length>0){const i=a[a.length-1];a[a.length-1]=this._truncateText(t,i,s)}return a},_truncateText(t,e,s){if(t.measureText(e).width<=s)return e;let o=e;for(;o.length>1&&t.measureText(o+"…").width>s;)o=o.slice(0,-1);return o+"…"},initFuse(){if(typeof Fuse>"u"){console.warn("Fuse.js not loaded yet");return}const t={keys:[{name:"artist",weight:.35},{name:"album",weight:.25},{name:"label",weight:.15},{name:"storageLocation",weight:.15},{name:"sku",weight:.1},{name:"quickId",weight:.1},{name:"genre",weight:.03},{name:"notes",weight:.02}],threshold:.4,distance:100,ignoreLocation:!0,minMatchCharLength:2};this.fuse=new Fuse(this.state.inventory,t)},getFilteredInventory(){const t=(this.state.inventorySearch||"").trim().toLowerCase(),e=this.state.filterGenre||"all",s=this.state.filterOwner||"all",o=this.state.filterLabel||"all",r=this.state.filterStorage||"all",a=this.state.filterDiscogs||"all",n=this.state.filterHero||"all";let i=this.state.inventory;if(t.length>=2)if(this.fuse)i=this.fuse.search(t).map(l=>l.item);else{const l=t.split(" ").filter(d=>d.length>0);i=i.filter(d=>l.every(c=>(d.artist||"").toLowerCase().includes(c)||(d.album||"").toLowerCase().includes(c)||(d.label||"").toLowerCase().includes(c)||(d.storageLocation||"").toLowerCase().includes(c)||(d.genre||"").toLowerCase().includes(c)||(d.notes||"").toLowerCase().includes(c)||(d.sku||"").toLowerCase().includes(c)))}return i.filter(l=>{const d=[l.genre,l.genre2,l.genre3,l.genre4,l.genre5].filter(Boolean),c=[];d.forEach(I=>{c.push(...I.split(",").map(C=>C.trim()).filter(Boolean))});const u=[...new Set(c)],p=u.filter(I=>I.toLowerCase()!=="electronic"),v=p.length>0?p:u.length>0?u:["Otros"],m=e==="all"||v.includes(e),x=s==="all"||l.owner===s,w=o==="all"||l.label===o,b=r==="all"||l.storageLocation===r,f=!!l.discogs_listing_id,$=a==="all"||a==="yes"&&f||a==="no"&&!f,E=l.tags&&(Array.isArray(l.tags),l.tags.includes("hero")),_=n==="all"||n==="yes"&&E||n==="no"&&!E,D=this.getTimeInStockCategory(l.created_at||null),y=this.state.filterStockTime.length===0||this.state.filterStockTime.includes(D);return m&&x&&w&&b&&$&&_&&y})},toggleSelectAll(){const t=this.getFilteredInventory();t.length>0&&t.every(e=>this.state.selectedItems.has(e.sku))?t.forEach(e=>this.state.selectedItems.delete(e.sku)):t.forEach(e=>this.state.selectedItems.add(e.sku)),this.refreshCurrentView()},addSelectionToCart(){this.state.selectedItems.forEach(t=>{const e=this.state.inventory.find(s=>s.id===t||s.sku===t);e&&e.stock>0&&(this.state.cart.find(s=>s.sku===t)||this.state.cart.push(e))}),this.state.selectedItems.clear(),this.showToast(`${this.state.cart.length} items agregados al carrito`),this.refreshCurrentView()},deleteSelection(){if(!confirm(`¿Estás seguro de eliminar ${this.state.selectedItems.size} productos ? `))return;const t=S.batch(),e=[];this.state.selectedItems.forEach(s=>{const o=S.collection("products").doc(s),r=this.state.inventory.find(a=>a.id===s||a.sku===s);r&&e.push(r),t.delete(o)}),t.commit().then(()=>{this.showToast("Productos eliminados"),e.forEach(s=>this.logInventoryMovement("DELETE",s)),this.state.selectedItems.clear()}).catch(s=>{console.error("Error logging movement:",s),alert("Error al eliminar")})},openAddExpenseModal(){const t=["Alquiler","Servicios","Marketing","Suministros","Honorarios"],s=`
    <div id="modal-overlay" class="fixed inset-0 bg-brand-dark/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" >
    ...
    `;document.body.insertAdjacentHTML("beforeend",s)},async handleAddVinyl(t,e){t.preventDefault();const s=new FormData(t.target);let o=s.get("genre"),r=s.get("collection");r==="other"&&(r=s.get("custom_collection"));const a=s.get("sku"),n=s.get("is_online")==="on",i=s.get("publish_discogs")==="on",l=s.get("publish_local")==="on",d={sku:a,artist:s.get("artist"),album:s.get("album"),genre:o,genre2:s.get("genre2")||null,genre3:s.get("genre3")||null,genre4:s.get("genre4")||null,genre5:s.get("genre5")||null,label:s.get("label"),collection:r||null,collectionNote:s.get("collectionNote")||null,year:s.get("year")?parseInt(s.get("year")):null,condition:s.get("condition"),provider_origin:s.get("provider_origin")||"Local_Used",sleeveCondition:s.get("sleeveCondition")||"",comments:s.get("comments")||"",price:parseFloat(s.get("price")),cost:parseFloat(s.get("cost"))||0,stock:parseInt(s.get("stock")),storageLocation:s.get("storageLocation"),owner:s.get("owner"),is_online:n,publish_webshop:n,publish_discogs:i,publish_local:l,cover_image:s.get("cover_image")||null,updated_at:firebase.firestore.FieldValue.serverTimestamp(),tags:[s.get("tag_hero")?"hero":null,s.get("tag_new")?"new_arrival":null,s.get("collection_tag")?s.get("collection_tag").trim():null].filter(Boolean),is_rsd_discount:s.get("is_rsd_discount")==="on",discogsUrl:s.get("discogsUrl"),discogsId:s.get("discogsId"),discogs_release_id:s.get("discogs_release_id")||s.get("discogsId"),tracks:(()=>{try{return JSON.parse(s.get("tracks")||"[]")}catch{return[]}})()};d.provider_origin==="EU_B2B"?(d.item_phantom_vat=Math.round(d.cost*.25*100)/100,d.item_real_vat=0,d.acquisition_date=s.get("acquisition_date")||new Date().toISOString().split("T")[0]):d.provider_origin==="DK_B2B"?(d.item_phantom_vat=0,d.item_real_vat=Math.round(d.cost*.25*100)/100,d.acquisition_date=s.get("acquisition_date")||new Date().toISOString().split("T")[0]):(d.item_phantom_vat=0,d.item_real_vat=0,d.acquisition_date=null),console.log(`[handleAddVinyl] editSku: ${e}, recordData:`,d);try{let c=null,u=null;if(e){const p=await this.findProductBySku(e);if(!p){this.showToast("❌ Producto no encontrado","error");return}u=p.data,c=p.id,await p.ref.update(d),this.showToast("✅ Disco actualizado")}else{const p=this.state.inventory.map(m=>{const x=m.sku&&typeof m.sku=="string"?m.sku.match(/^SKU\s*-\s*(\d+)/):null;return x?parseInt(x[1]):0}),v=Math.max(0,...p);d.created_at=firebase.firestore.FieldValue.serverTimestamp(),c=await S.runTransaction(async m=>{const x=S.collection("metadata").doc("vinylCounter"),w=await m.get(x);let b=0;w.exists&&(b=w.data().current||0);const f=Math.max(b,v)+1,$=String(f).padStart(4,"0");m.set(x,{current:f},{merge:!0}),d.quickId=$,d.sku=`SKU-${String(f).padStart(3,"0")}`;const E=S.collection("products").doc();return m.set(E,d),E.id}),this.showToast(`✅ Disco agregado (ID: ${d.quickId})`)}if(i){const p=s.get("discogs_release_id")||s.get("discogsId");if(u&&u.discogs_listing_id)try{const m=await(await fetch(`${R}/discogs/update-listing/${u.discogs_listing_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({product:d})})).json();if(m.success)this.showToast("💿 Listing de Discogs actualizado");else throw new Error(m.error||"Error desconocido")}catch(v){console.error("Error updating Discogs listing:",v),this.showToast(`⚠️ Error Discogs: ${v.message}`,"error")}else if(p)try{const m=await(await fetch(`${R}/discogs/create-listing`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({releaseId:parseInt(p),product:d})})).json();if(m.success&&m.listingId)await S.collection("products").doc(c).update({discogs_listing_id:String(m.listingId),discogs_release_id:parseInt(p)}),this.showToast("💿 Publicado en Discogs correctamente");else throw new Error(m.error||"Error desconocido")}catch(v){console.error("Error creating Discogs listing:",v);let m=v.message;(m.toLowerCase().includes("mp3")||m.toLowerCase().includes("digital")||m.toLowerCase().includes("format"))&&(m="Discogs solo permite formatos físicos (Vinyl, CD, Cassette). Este release es digital o MP3."),this.showToast(`⚠️ Error Discogs: ${m}`,"error")}else this.showToast("⚠️ Necesitas buscar el disco en Discogs primero para publicarlo","warning")}document.getElementById("modal-overlay").remove(),this.loadData()}catch(c){console.error(c),this.showToast("❌ Error: "+(c.message||"desconocido"),"error")}},async toggleProductTag(t,e){try{const s=this.state.inventory.find(n=>n.id===t||n.sku===t);if(!s){this.showToast("❌ Producto no encontrado","error");return}let o=s.tags||[];o.includes(e)?o=o.filter(n=>n!==e):o.push(e);const r=S.collection("products").doc(s.id);if(!(await r.get()).exists){this.showToast("❌ Error: Documento no encontrado","error");return}await r.update({tags:o,updated_at:firebase.firestore.FieldValue.serverTimestamp()}),this.showToast(`✅ ${e==="hero"?"Héroe":"Novedad"} actualizado`),s.tags=o,this.refreshCurrentView()}catch(s){console.error("Error toggling product tag:",s),this.showToast("❌ Error al actualizar tag","error")}},async toggleRsdDiscount(t){try{const e=this.state.inventory.find(a=>a.id===t||a.sku===t);if(!e){this.showToast("❌ Producto no encontrado","error");return}const s=!e.is_rsd_discount,o=S.collection("products").doc(e.id);if(!(await o.get()).exists){this.showToast("❌ Error: Documento no encontrado","error");return}await o.update({is_rsd_discount:s,updated_at:firebase.firestore.FieldValue.serverTimestamp()}),this.showToast(`✅ RSD ${s?"activado":"desactivado"} — ${e.album}`),e.is_rsd_discount=s,this.refreshCurrentView()}catch(e){console.error("Error toggling RSD discount:",e),this.showToast("❌ Error al actualizar RSD","error")}},deleteVinyl(t){const e=this.state.inventory.find(o=>o.id===t||o.sku===t);if(!e){alert("Error: Item not found");return}const s=`
                                                    ...
                                                    `;document.body.insertAdjacentHTML("beforeend",s)},async confirmDelete(t){const e=document.getElementById("delete-confirm-modal");e&&e.remove();const s=document.getElementById("modal-overlay");s&&s.remove();try{const o=this.state.inventory.find(i=>i.id===t||i.sku===t),r=o?o.id:t,a=await S.collection("products").doc(r).get();if(!a.exists){this.showToast("❌ Producto no encontrado","error");return}const n={id:a.id,ref:a.ref,data:a.data()};if(console.log("Product to delete:",n.data),console.log("Has discogs_listing_id?",n.data.discogs_listing_id),n.data.discogs_listing_id){console.log("Attempting to delete from Discogs:",n.data.discogs_listing_id);try{const i=await fetch(`${R}/discogs/delete-listing/${n.data.discogs_listing_id}`,{method:"DELETE"});console.log("Discogs delete response status:",i.status);const l=await i.json();console.log("Discogs delete result:",l),l.success?(console.log("Discogs listing deleted successfully"),this.showToast("💿 Eliminado de Discogs")):this.showToast("⚠️ "+(l.error||"Error en Discogs"),"warning")}catch(i){console.error("Error deleting from Discogs:",i),this.showToast("⚠️ Error eliminando de Discogs, pero continuando...","warning")}}else console.log("No discogs_listing_id found, skipping Discogs deletion");await n.ref.delete(),this.showToast("✅ Disco eliminado"),await this.loadData()}catch(o){console.error("Error removing document: ",o),this.showToast("❌ Error al eliminar: "+o.message,"error")}},handleSaleSubmit(t){var $,E,_,D,y,I,C;t.preventDefault();const e=new FormData(t.target);let s=e.get("sku");s||(s=($=document.getElementById("input-sku"))==null?void 0:$.value);const o=this.state.inventory.find(M=>M.sku===s);if(!o){this.showToast("⚠️ Debes seleccionar un producto válido del listado","error");const M=document.getElementById("sku-search");M&&(M.focus(),M.classList.add("border-red-500","animate-pulse"),setTimeout(()=>M.classList.remove("border-red-500","animate-pulse"),2e3));return}let r=parseInt(e.get("quantity"));if(isNaN(r)&&(r=parseInt((E=document.getElementById("input-qty"))==null?void 0:E.value)||1),o.stock<r){this.showToast(`❌ Stock insuficiente. Disponible: ${o.stock}`,"error");return}let a=parseFloat(e.get("price"));isNaN(a)&&(a=parseFloat((_=document.getElementById("input-price"))==null?void 0:_.value)||0);const n=parseFloat(e.get("cost"))||0,i=parseFloat(e.get("shipping_income"))||0,l=a*r+i;e.get("date")||new Date().toISOString();const d=e.get("paymentMethod"),c=e.get("soldAt");e.get("comment");let u=e.get("artist");u||(u=(D=document.getElementById("input-artist"))==null?void 0:D.value);let p=e.get("album");p||(p=(y=document.getElementById("input-album"))==null?void 0:y.value);let v=e.get("genre");v||(v=(I=document.getElementById("input-genre"))==null?void 0:I.value);let m=e.get("owner");m||(m=(C=document.getElementById("input-owner"))==null?void 0:C.value);const x=e.get("customerName"),w=e.get("customerEmail"),b=e.get("requestInvoice")==="on",f={items:[{recordId:o.id,quantity:r,unitPrice:a,costAtSale:n}],paymentMethod:d||"CASH",customerName:x||"Venta Manual",customerEmail:w||null,shipping_income:i,total_amount:l,source:"STORE",channel:(c==null?void 0:c.toLowerCase())||"store"};fe.createSale(f).then(()=>{this.showToast(b?"Venta registrada (Factura Solicitada)":"Venta registrada");const M=document.getElementById("modal-overlay");M&&M.remove();const V=t.target;V&&V.reset();const Y=document.getElementById("form-total");Y&&(Y.innerText="$0.00");const J=document.getElementById("sku-search");J&&(J.value=""),this.state.manualSaleSearch="",this.loadData()}).catch(M=>{console.error("Error adding sale: ",M),this.showToast("❌ Error al registrar venta: "+(M.message||""),"error")})},addToCart(t,e){e&&e.stopPropagation();const s=this.state.inventory.find(r=>r.id===t||r.sku===t);if(!s)return;if(this.state.cart.filter(r=>r.sku===t).length>=s.stock){this.showToast("⚠️ No hay más stock disponible");return}this.state.cart.push(s),document.getElementById("inventory-cart-container")?this.renderInventoryCart():this.renderCartWidget(),this.showToast("Agregado al carrito")},removeFromCart(t){this.state.cart.splice(t,1),this.renderCartWidget()},clearCart(){this.state.cart=[],this.renderCartWidget()},renderOnlineSales(t){const e=this.state.sales.filter(a=>a.channel==="online"),s=e.filter(a=>a.status==="completed"),o=e.filter(a=>a.status==="PENDING"),r=s.reduce((a,n)=>a+(parseFloat(n.total_amount||n.total)||0),0);t.innerHTML=`
        <div class="p-6">
            <!-- Header -->
            <div class="flex items-center justify-between mb-8">
                <div>
                            ...
                            <tbody>
                                ${e.map(a=>{var i;const n=(i=a.timestamp)!=null&&i.toDate?a.timestamp.toDate():new Date(a.date||0);return{...a,_sortDate:n.getTime()}}).sort((a,n)=>n._sortDate-a._sortDate).map(a=>{var v,m,x,w,b,f,$;const n=a.customer||{},i=a.orderNumber||"N/A",l=(v=a.timestamp)!=null&&v.toDate?a.timestamp.toDate():new Date(a.date),c=((m=a.completed_at)!=null&&m.toDate?a.completed_at.toDate():null)||l,u={completed:"bg-green-50 text-green-700 border-green-200",PENDING:"bg-yellow-50 text-yellow-700 border-yellow-200",failed:"bg-red-50 text-red-700 border-red-200"},p={completed:"✅ Completado",PENDING:"⏳ Pendiente",failed:"❌ Fallido"};return`
                                        <tr class="border-b border-slate-50 hover:bg-slate-50 transition-colors cursor-pointer" onclick="app.openUnifiedOrderDetailModal('${a.id}')">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="font-semibold text-brand-dark">${n.name||(n.firstName?`${n.firstName} ${n.lastName||""}`:"")||((x=n.stripe_info)==null?void 0:x.name)||"Cliente"}</div>
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="text-sm text-slate-600 truncate max-w-[200px]">
                                                    ${((b=n.shipping)==null?void 0:b.line1)||n.address||(($=(f=n.stripe_info)==null?void 0:f.shipping)==null?void 0:$.line1)||"Sin dirección"}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4">
                                                <div class="text-sm">
    ...
    `},openOnlineSaleDetailModal(t){var l,d,c;const e=this.state.sales.find(u=>u.id===t);if(!e)return;const s=e.customer||{},o=s.stripe_info||{},r=s.shipping||o.shipping||{},a={line1:r.line1||s.address||"Sin dirección",line2:r.line2||"",city:r.city||s.city||"",postal:r.postal_code||s.postalCode||"",country:r.country||s.country||"Denmark"},n=`
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-white border border-slate-100 rounded-2xl overflow-hidden">
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-slate-50">
                                    ${(e.items||[]).map(u=>{var p,v,m;return`
                                        <tr>
                                            <td class="px-4 py-3">
                                                <p class="font-bold text-brand-dark">${u.album||((p=u.record)==null?void 0:p.album)||"Unknown"}</p>
                                                <p class="text-xs text-slate-500">${u.artist||((v=u.record)==null?void 0:v.artist)||""}</p>
                                            </td>
    ...
    `;document.body.insertAdjacentHTML("beforeend",i)},renderCartWidget(){const t=document.getElementById("cart-widget");if(!t)return;const e=document.getElementById("cart-count"),s=document.getElementById("cart-items-mini"),o=document.getElementById("cart-total-mini");if(this.state.cart.length===0){t.classList.add("hidden");return}t.classList.remove("hidden"),e.innerText=this.state.cart.length;const r=this.state.cart.reduce((a,n)=>a+n.price,0);o.innerText=this.formatCurrency(r),s.innerHTML=this.state.cart.map((a,n)=>`
                                                                ...
                                                                `).join("")},openCheckoutModal(t,e,s=0){if(this.state.cart.length===0)return;const o=this.state.cart.reduce((u,p)=>u+this.getEffectivePrice(p),0),r=s>0?Math.round(o*(1-s)*100)/100:o,a=`
            <div id="modal-overlay" class="fixed inset-0 bg-brand-dark/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
...
`).replace(/\s+/g," "),o=t.split(/\r?\n/).map(d=>d.trim()).filter(d=>d),r=[/(\d{1,2}[-\/\.]\d{1,2}[-\/\.]\d{2,4})/,/(\d{4}[-\/\.]\d{1,2}[-\/\.]\d{1,2})/,/(\d{1,2}\.\s?\w+\.?\s?\d{2,4})/i];for(const d of r){const c=s.match(d);if(c){e.fecha=this.normalizeDate(c[1]);break}}const a=[/(?:i\s*alt|total|sum|totalt|att\s*betala)[:\s]*(\d+[.,]\d{2})/i,/(?:total|sum)[:\s]*(?:kr\.?|dkk)?\s*(\d+[.,]\d{2})/i,/(\d+[.,]\d{2})\s*(?:dkk|kr)/i];for(const d of a){const c=s.match(d);if(c){e.monto_total=parseFloat(c[1].replace(",","."));break}}const n=[/(?:moms|25%|heraf\s*moms)[:\s]*(\d+[.,]\d{2})/i,/(?:vat|iva|tax)[:\s]*(\d+[.,]\d{2})/i,/moms\s*(?:kr\.?|dkk)?\s*(\d+[.,]\d{2})/i];for(const d of n){const c=s.match(d);if(c){e.monto_iva=parseFloat(c[1].replace(",","."));break}}e.monto_total&&!e.monto_iva&&(e.monto_iva=Math.round(e.monto_total*.2*100)/100);const i=["kvittering","receipt","bon","faktura","invoice","kopi","copy"];for(const d of o.slice(0,5)){const c=d.trim();if(c.length>2&&c.length<50&&!i.some(u=>c.toLowerCase().includes(u))&&!/^\d+$/.test(c)&&!/^[\d\s\-\/\.]+$/.test(c)){e.proveedor=c;break}}const l=s.match(/(?:cvr|org\.?\s*nr)[:\s]*(\d{8})/i);if(l&&o.length>0){const d=o.findIndex(c=>c.includes(l[0]));d>0&&!e.proveedor&&(e.proveedor=o[d-1])}return console.log("Parsed Receipt Data:",e),e},normalizeDate(t){try{const s=t.replace(/\s/g,"").replace(/[\.\/]/g,"-").split("-");if(s.length>=3){let o,r,a;return s[0].length===4?[a,r,o]=s:([o,r,a]=s,a.length===2&&(a="20"+a)),o=o.padStart(2,"0"),r=r.padStart(2,"0"),`${a}-${r}-${o}`}}catch{console.warn("Date normalization failed:",t)}return null},autoFillExpenseForm(t){if(t.fecha){const e=document.getElementById("expense-fecha");e&&(e.value=t.fecha,this.highlightAutoFilled(e))}if(t.proveedor){const e=document.getElementById("expense-proveedor");e&&(e.value=t.proveedor,this.highlightAutoFilled(e))}if(t.monto_total){const e=document.getElementById("expense-monto");e&&(e.value=t.monto_total.toFixed(2),this.highlightAutoFilled(e))}if(t.monto_iva){const e=document.getElementById("expense-iva");e&&!e.disabled&&(e.value=t.monto_iva.toFixed(2),this.highlightAutoFilled(e))}},highlightAutoFilled(t){t.classList.add("ring-2","ring-green-400","bg-green-50");const e=()=>{t.classList.remove("ring-2","ring-green-400","bg-green-50"),t.removeEventListener("focus",e)};t.addEventListener("focus",e),setTimeout(e,5e3)},clearReceiptUpload(){document.getElementById("receipt-file").value="",document.getElementById("receipt-url").value="",document.getElementById("upload-placeholder").classList.remove("hidden"),document.getElementById("upload-preview").classList.add("hidden"),document.getElementById("receipt-preview-img").src="",document.getElementById("receipt-filename").textContent=""},renderConsignments(t){if(!t)return;const e=`
    <div class="max-w-7xl mx-auto px-4 md:px-8 pb-24 md:pb-8 pt-6 animate-fadeIn" >
                                                                    ...
                                                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                                                        ${this.state.consignors.map(s=>{const o=s.name,a=this.state.inventory.filter(c=>c.owner===o).reduce((c,u)=>c+u.stock,0),n=[];this.state.sales.forEach(c=>{(c.items||[]).filter(p=>{if((p.owner||"").toLowerCase()===o.toLowerCase())return!0;const v=this.state.inventory.find(m=>m.id===(p.productId||p.recordId));return v&&(v.owner||"").toLowerCase()===o.toLowerCase()}).forEach(p=>{const v=Number(p.priceAtSale||p.unitPrice||0),m=s.agreementSplit||s.split||70,x=v*m/100;n.push({...p,id:c.id,date:c.date,cost:p.costAtSale||p.cost||x,payoutStatus:c.payoutStatus||"pending",payoutDate:c.payoutDate||null})}),(!c.items||c.items.length===0)&&(c.owner||"").toLowerCase()===o.toLowerCase()&&n.push({...c,album:c.album||c.sku||"Record",cost:c.cost||(Number(c.total)||0)*(s.agreementSplit||70)/100})}),n.sort((c,u)=>new Date(u.date)-new Date(c.date)),n.reduce((c,u)=>c+(Number(u.qty||u.quantity)||1),0);const i=n.reduce((c,u)=>c+(Number(u.cost)||0),0),l=n.filter(c=>c.payoutStatus==="paid").reduce((c,u)=>c+(Number(u.cost)||0),0),d=i-l;return`
...
`)},handleAddConsignor(t){t.preventDefault();const e=new FormData(t.target),s={name:e.get("name"),agreementSplit:parseFloat(e.get("split")),email:e.get("email"),phone:e.get("phone")};S.collection("consignors").add(s).then(()=>{this.showToast("✅ Socio registrado correctamente"),document.getElementById("modal-overlay").remove(),this.loadData()}).catch(o=>{console.error(o),this.showToast("❌ Error al crear socio: "+o.message,"error")})},deleteConsignor(t){confirm("¿Eliminar este socio?")&&S.collection("consignors").doc(t).delete().then(()=>{this.showToast("✅ Socio eliminado"),this.loadData()}).catch(e=>{console.error(e),this.showToast("❌ Error al eliminar socio: "+e.message,"error")})},saveData(){try{const t={};localStorage.setItem("el-cuartito-settings",JSON.stringify(t))}catch(t){console.error("Error saving settings:",t)}},searchDiscogs(){const t=document.getElementById("discogs-search-input").value,e=document.getElementById("discogs-results");if(t){if(e.innerHTML='<p class="text-xs text-slate-400 animate-pulse p-2">Buscando en Discogs...</p>',e.classList.remove("hidden"),/^\d+$/.test(t.trim())){this.fetchDiscogsById(t.trim());return}fetch(`${R}/discogs/search?q=${encodeURIComponent(t)}`).then(s=>{if(!s.ok)throw new Error(`Error ${s.status}`);return s.json()}).then(s=>{const o=s.results||[];o.length>0?e.innerHTML=o.slice(0,10).map(r=>`
                ...
                `})}},resyncMusic(){["input-discogs-id","input-discogs-release-id","input-discogs-url","input-cover-image"].forEach(o=>{const r=document.getElementById(o);r&&(r.value="")});const t=document.querySelector('input[name="artist"]').value,e=document.querySelector('input[name="album"]').value,s=document.getElementById("discogs-search-input");s&&t&&e?(s.value=`${t} - ${e}`,this.searchDiscogs(),this.showToast("✅ Música desvinculada. Selecciona una nueva edición.","success")):this.showToast("⚠️ Falta Artista o Álbum para buscar.","error")},handleDiscogsSelection(t){const e=document.getElementById("discogs-results");e&&e.classList.add("hidden");const s=t.title.split(" - "),o=s[0]||"",r=s.slice(1).join(" - ")||t.title,a=document.querySelector("#modal-overlay form");if(!a)return;if(a.artist&&(a.artist.value=o),a.album&&(a.album.value=r),a.year&&t.year&&(a.year.value=t.year),t.thumb||t.cover_image){const i=t.cover_image||t.thumb,l=document.getElementById("input-cover-image"),d=document.getElementById("cover-preview");if(l&&(l.value=i),d){const c=d.querySelector("img"),u=document.getElementById("cover-placeholder");c&&(c.src=i,c.classList.remove("hidden")),u&&u.classList.add("hidden")}}const n=document.getElementById("input-discogs-id");if(n&&t.id&&(n.value=t.id),t.uri||t.resource_url){const i=t.uri||t.resource_url,l=i.startsWith("http")?i:"https://www.discogs.com"+i,d=document.getElementById("input-discogs-url");d&&(d.value=l)}if(t.id){const i=document.getElementById("discogs-metadata-area"),l=document.getElementById("metadata-tracks"),d=document.getElementById("metadata-tags"),c=document.getElementById("discogs-link");console.log("Metadata Area Found:",!!i),i&&(i.classList.remove("hidden"),i.style.display="grid"),l&&(l.innerHTML='<p class="text-[10px] text-slate-400 animate-pulse">Loading tracks...</p>'),this.showToast("⏳ Cargando detalles...","info"),fetch(`${R}/discogs/release/${t.id}`).then(u=>u.json()).then(u=>{const p=u.release||u;if(console.log("Full Release Data:",p),i&&(i.classList.remove("hidden"),i.style.display="grid"),c&&p.uri){const x=p.uri.startsWith("http")?p.uri:"https://www.discogs.com"+p.uri;c.href=x,c.classList.remove("hidden"),c.style.display="flex"}const v=p.styles||[],m=[...new Set(v)];d&&(d.innerHTML=m.map(x=>`<span class="meta-chip border border-slate-200">${x}</span>`).join(""));for(let x=0;x<Math.min(m.length,3);x++){const w=document.getElementById(`genre-${x+1}`);w&&(w.value=m[x])}if(l)if(p.tracklist&&p.tracklist.length>0){const x=document.getElementById("input-tracks");x&&(x.value=JSON.stringify(p.tracklist)),l.innerHTML=p.tracklist.map(w=>`
                            ...
                            `).join("")}else l.innerHTML='<p class="text-[10px] text-slate-400 italic">No tracks found.</p>';a.label&&p.labels&&p.labels.length>0&&(a.label.value=p.labels[0].name)}).catch(u=>{console.error("Error fetching full release:",u),l&&(l.innerHTML='<p class="text-[10px] text-red-400">Error loading tracklist.</p>')})}},openTracklistModal(t){const e=this.state.inventory.find(a=>a.id===t||a.sku===t);if(!e)return;let s=e.discogsId;document.body.insertAdjacentHTML("beforeend",`
                                                                ...
                                                                `);const r=a=>{fetch(`${R}/discogs/release/${a}`).then(n=>{if(!n.ok)throw new Error("Release not found");return n.json()}).then(n=>{const l=(n.release||n).tracklist||[],d=l.map(u=>`
                    ...
                    `})}},renderDiscogsSales(t){const e=this.state.sales.filter(l=>l.channel==="discogs"),s=l=>parseFloat(l.total)||0,o=l=>parseFloat(l.originalTotal)||parseFloat(l.total)+(parseFloat(l.discogsFee||0)+parseFloat(l.paypalFee||0)),r=l=>o(l)-s(l),a=e.reduce((l,d)=>l+s(d),0),n=e.reduce((l,d)=>l+r(d),0),i=e.reduce((l,d)=>{const c=s(d);let u=0;return d.items&&Array.isArray(d.items)&&(u=d.items.reduce((p,v)=>{const m=parseFloat(v.costAtSale||0),x=parseInt(v.qty||v.quantity)||1;return p+m*x},0)),l+(c-u)},0);t.innerHTML=`
        <div class="p-6">
            <!-- Header -->
            <div class="flex items-center justify-between mb-8">
                <div>
                ...
                `:`
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody>
                                ${e.map(l=>{var c;const d=(c=l.timestamp)!=null&&c.toDate?l.timestamp.toDate():l.date?new Date(l.date):new Date(0);return{...l,_sortDate:d.getTime()}}).sort((l,d)=>d._sortDate-l._sortDate).map(l=>{var m;const d=(m=l.timestamp)!=null&&m.toDate?l.timestamp.toDate():new Date(l.date),c=l.items&&l.items[0],u=l.originalTotal||l.total+(l.discogsFee||0)+(l.paypalFee||0);l.discogsFee,l.paypalFee;const p=l.total,v=l.status==="pending_review"||l.needsReview;return`
                                        <tr class="border-b border-slate-50 hover:bg-purple-50/30 transition-colors cursor-pointer ${v?"bg-orange-50/50":""}" onclick="app.openUnifiedOrderDetailModal('${l.id}')">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="text-[10px] text-red-500 font-bold">Total Fees: -${this.formatCurrency(u-p)}</div>
                                                <div class="text-[10px] text-slate-400 font-medium">
                                                    ${u>0?`(${((u-p)/u*100).toFixed(1)}%)`:""}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="flex flex-col gap-2">
                                                    ...
                                                    `}
                                                    <button onclick="app.openUpdateSaleValueModal('${l.id}', ${u}, ${p})" class="w-full py-1.5 bg-slate-100 text-slate-600 hover:bg-slate-200 text-[10px] font-bold rounded-lg transition-colors border border-slate-200 flex items-center justify-center gap-1">
...
¿CONFIRMAS que deseas eliminar permanentemente este registro y su recibo?`)){this.showToast("ℹ️ Eliminación cancelada");return}}else if(!confirm("¿Estás seguro de que quieres eliminar este gasto?"))return;try{await S.collection("expenses").doc(t).delete(),this.showToast("✅ Gasto eliminado"),this.loadData()}catch(s){console.error("Error deleting expense:",s),this.showToast("❌ Error al eliminar gasto")}},renderVATReport(t){const e=new Date,s=Math.floor(e.getMonth()/3)+1,o=e.getFullYear(),r=this.state.vatReportQuarter!==void 0?this.state.vatReportQuarter:s,a=this.state.vatReportYear||o;let n,i;if(r===0)n=new Date(a,0,1),i=new Date(a,11,31,23,59,59);else{const g=(r-1)*3;n=new Date(a,g,1),i=new Date(a,g+3,0,23,59,59)}const l=this.state.sales.filter(g=>{var O;const T=(O=g.timestamp)!=null&&O.toDate?g.timestamp.toDate():new Date(g.timestamp||g.date);return T>=n&&T<=i});let d=[],c=[],u=[],p=0,v=0,m=0,x=0;l.forEach(g=>{var ae;const T=(ae=g.timestamp)!=null&&ae.toDate?g.timestamp.toDate():new Date(g.timestamp||g.date);(g.items||[]).forEach(j=>{const ie=j.priceAtSale||j.price||0;let de=j.costAtSale||j.cost||0;const H=j.productId||j.recordId,W=j.album;let N=j.providerOrigin||j.provider_origin;if(de===0||!N){const F=this.state.inventory.find(q=>H&&(q.id===H||q.sku===H)||W&&q.album===W);F&&(de===0&&(de=F.cost||0),N||(N=F.provider_origin||"Local_Used"))}N||(N="Local_Used");const P=j.qty||j.quantity||1,U=ie*P,z=de*P;if(N==="EU_B2B"||N==="DK_B2B"){const F=U*.2;p+=F,d.push({date:T,productId:j.productId||j.album||"N/A",album:j.album||"N/A",salePrice:U,vat:F})}else{const F=U-z,q=F>0?F*.2:0;v+=q,c.push({date:T,productId:j.productId||j.album||"N/A",album:j.album||"N/A",cost:z,salePrice:U,margin:F,vat:q})}});const se=parseFloat(g.shipping_income||g.shipping||g.shipping_cost||0);if(se>0){const j=se*.2;m+=j,x+=se,u.push({date:T,orderId:g.orderNumber||(g.id&&typeof g.id=="string"?g.id.slice(-8):"N/A"),income:se,vat:j})}}),(this.state.extraIncome||[]).filter(g=>{const T=new Date(g.date);return T>=n&&T<=i}).forEach(g=>{const T=Number(g.amount)||0,O=Number(g.vatAmount)||0;p+=O,d.push({date:new Date(g.date),productId:"EXTRA",album:`💰 ${g.description||"Ingreso Extra"} (${g.category||"other"})`,salePrice:T,vat:O})});const b=p+v+m,f=this.state.expenses.filter(g=>{var se;const T=g.fecha_factura?new Date(g.fecha_factura):(se=g.timestamp)!=null&&se.toDate?g.timestamp.toDate():new Date(g.timestamp||g.date);return(g.categoria_tipo==="operativo"||g.categoria_tipo==="stock_nuevo"||g.is_vat_deductible)&&T>=n&&T<=i}),$=f.filter(g=>g.categoria!=="envios"),E=f.filter(g=>g.categoria==="envios"),_=$.reduce((g,T)=>g+(parseFloat(T.monto_iva)||0),0),D=E.reduce((g,T)=>g+(parseFloat(T.monto_iva)||0),0);E.reduce((g,T)=>g+(parseFloat(T.monto_total)||0),0);const y=(this.state.inventory||[]).filter(g=>{if(!g.item_phantom_vat||g.item_phantom_vat<=0||g.provider_origin!=="EU_B2B")return!1;const T=g.acquisition_date?new Date(g.acquisition_date):null;return T?T>=n&&T<=i:!1}),I=y.reduce((g,T)=>g+(T.item_phantom_vat||0),0),C=(this.state.inventory||[]).filter(g=>{if(!g.item_real_vat||g.item_real_vat<=0||g.provider_origin!=="DK_B2B")return!1;const T=g.acquisition_date?new Date(g.acquisition_date):null;return T?T>=n&&T<=i:!1}),M=C.reduce((g,T)=>g+(T.item_real_vat||0),0),V=b+I,Y=_+D+I+M,J=V-Y,Z={0:`Resumen anual ${a}`,1:`1 de junio, ${a}`,2:`1 de septiembre, ${a}`,3:`1 de diciembre, ${a}`,4:`1 de marzo, ${a+1}`}[r],te=`
            <div class="max-w-7xl mx-auto px-4 md:px-8 pb-24 md:pb-8 pt-6">
                <!-- Header Section -->
                <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-6 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <div class="flex items-center gap-4">
                    ...
                    <div class="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                        <div class="flex bg-slate-50 p-1 rounded-xl border border-slate-200 gap-1">
                            <select id="vat-year-select" onchange="app.updateVATQuarter()" class="bg-transparent px-3 py-1.5 text-sm font-bold text-slate-600 outline-none cursor-pointer">
                                ${[o,o-1,o-2].map(g=>`<option value="${g}" ${g===a?"selected":""}>${g}</option>`).join("")}
                            </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm">
                        ...
                        <p class="text-3xl font-display font-bold text-brand-dark">${this.formatCurrency(V)}</p>
                            ...
                            <div class="p-6 space-y-6">
                                ...
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-[10px] text-slate-400 uppercase font-bold tracking-tighter">Monto: ${this.formatCurrency(d.reduce((g,T)=>g+T.salePrice,0))}</p>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-lg font-bold text-blue-600">${this.formatCurrency(p)}</p>
                                        <p class="text-[10px] text-slate-400 font-bold">IVA (25%)</p>
                                    </div>
                                </div>
                                <div class="h-1.5 w-full bg-slate-50 rounded-full overflow-hidden">
                                    <div class="h-full bg-blue-500 rounded-full" style="width: ${b>0?p/b*100:0}%"></div>
                                </div>
                                ...
                                <div class="pt-4 border-t border-slate-50">
                                    <div class="flex items-center justify-between mb-1">
                                        <div>
                                            ...
                                            <p class="text-[10px] text-slate-400 uppercase font-bold tracking-tighter">Margen total: ${this.formatCurrency(c.reduce((g,T)=>g+T.margin,0))}</p>
                                        </div>
                                        <div class="text-right">
                                ...
                                <div class="pt-4 border-t border-slate-50">
                                    <div class="flex items-center justify-between">
                                        <div>
                                            ...
                                            <p class="text-[10px] text-slate-400 uppercase font-bold tracking-tighter">Total cobrado: ${this.formatCurrency(x)}</p>
                                        </div>
                                        <div class="text-right">
                    ...
                    <div class="space-y-8">
                            ...
                            <div class="bg-slate-900 rounded-3xl p-6 text-white shadow-xl shadow-slate-900/10">
                                <div class="flex justify-between items-center mb-6">
                                    <div class="space-y-1">
                                        ...
                                        <p class="text-2xl font-display font-bold ${m-D>=0?"text-emerald-400":"text-red-400"}">
                                            ${this.formatCurrency(m-D)}
                                        </p>
                                    </div>
                        ...
                        <div>
                            ...
                            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden divide-y divide-slate-50">
                                ${Object.entries($.reduce((g,T)=>{const O=T.categoria||"otros";return g[O]=(g[O]||0)+(parseFloat(T.monto_iva)||0),g},{})).sort((g,T)=>T[1]-g[1]).map(([g,T])=>`
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tfoot class="bg-slate-50/30 font-bold">
                                    <tr class="text-brand-dark">
                                        ...
                                        <td class="px-6 py-4 text-right text-lg text-blue-600">${this.formatCurrency(p)}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
        ...
        `;t.innerHTML=te},updateVATQuarter(){const t=parseInt(document.getElementById("vat-quarter-select").value),e=parseInt(document.getElementById("vat-year-select").value);this.state.vatReportQuarter=t,this.state.vatReportYear=e,this.renderVATReport(document.getElementById("app-content"))},downloadVATAuditReport(){const t=new Date,e=Math.floor(t.getMonth()/3)+1,s=t.getFullYear(),o=this.state.vatReportQuarter||e,r=this.state.vatReportYear||s,a=(o-1)*3,n=new Date(r,a,1),i=new Date(r,a+3,0,23,59,59),l=this.state.sales.filter(y=>{var C;const I=(C=y.timestamp)!=null&&C.toDate?y.timestamp.toDate():new Date(y.timestamp||y.date);return I>=n&&I<=i}),d=[];let c=1;l.forEach(y=>{var J;const I=(J=y.timestamp)!=null&&J.toDate?y.timestamp.toDate():new Date(y.timestamp||y.date),C=I.toISOString().slice(0,10).replace(/-/g,""),M=y.channel||"N/A";(y.items||[]).forEach(G=>{const Z=G.priceAtSale||G.price||0;let Q=G.costAtSale||G.cost||0;const ee=G.productId||G.recordId,te=G.album;let g="Local_Used",T="N/A";const O=this.state.inventory.find(q=>ee&&(q.id===ee||q.sku===ee)||te&&q.album===te);O&&(Q=Q===0?O.cost||0:Q,g=O.provider_origin||"Local_Used",O.acquisition_date&&(T=new Date(O.acquisition_date).toISOString().slice(0,10)));const se=G.qty||G.quantity||1,ae=G.providerOrigin||g,j=ae==="EU_B2B"||ae==="DK_B2B",ie=Z*se,de=Q*se;let H,W,N;if(j)H=ie,W=ie*.2,N="Standard Rate";else{const q=ie-de;H=q>0?q:0,W=q>0?q*.2:0,N="Margin Scheme"}const P=O&&O.acquisition_date?new Date(O.acquisition_date):null,U=P&&P>=n&&P<=i,z=U&&g==="EU_B2B"&&O.item_phantom_vat||0,F=U?g==="DK_B2B"?O.item_real_vat||0:z:0;d.push({transactionId:`ECR-${C}-${String(c).padStart(4,"0")}`,date:I.toISOString().slice(0,10),channel:M,productName:`${G.album||"N/A"} - ${G.artist||"N/A"}`,sku:G.sku||ee||"N/A",providerOrigin:g,acquisitionDate:T,condition,costPrice:de.toFixed(2),salesPrice:ie.toFixed(2),calculationBasis:H.toFixed(2),schemeApplied:N,outputVat:W.toFixed(2),euPhantomVat:z.toFixed(2),inputVat:F.toFixed(2)}),c++});const Y=parseFloat(y.shipping_income||y.shipping||y.shipping_cost||0);Y>0&&(d.push({transactionId:`ECR-SHIP-${C}-${String(c).padStart(4,"0")}`,date:I.toISOString().slice(0,10),channel:M,productName:`Envío Cobrado - Orden: ${y.orderNumber||"N/A"}`,sku:"SHIPPING",providerOrigin:"N/A",acquisitionDate:"N/A",condition:"Service",costPrice:"0.00",salesPrice:Y.toFixed(2),calculationBasis:Y.toFixed(2),schemeApplied:"Standard Rate",outputVat:(Y*.2).toFixed(2),euPhantomVat:"0.00",inputVat:"0.00"}),c++)});const u=[];let p=1;this.state.expenses.filter(y=>{var M;const I=y.fecha_factura?new Date(y.fecha_factura):(M=y.timestamp)!=null&&M.toDate?y.timestamp.toDate():new Date(y.timestamp||y.date);return(y.categoria_tipo==="operativo"||y.categoria_tipo==="stock_nuevo"||y.is_vat_deductible)&&I>=n&&I<=i}).forEach(y=>{var M;const I=y.fecha_factura?new Date(y.fecha_factura):(M=y.timestamp)!=null&&M.toDate?y.timestamp.toDate():new Date(y.timestamp||y.date),C=I.toISOString().slice(0,10).replace(/-/g,"");u.push({transactionId:`ECP-EXP-${C}-${String(p).padStart(4,"0")}`,invoiceDate:I.toISOString().slice(0,10),category:y.categoria==="envios"?"Shipping Expense":"Operational Expense",vendor:y.proveedor||y.nombre||"N/A",description:y.descripcion||y.categoria||"N/A",sku:"N/A",grossAmount:parseFloat(y.monto_total||0).toFixed(2),euPhantomVat:"0.00",inputVat:parseFloat(y.monto_iva||0).toFixed(2)}),p++}),(this.state.inventory||[]).filter(y=>{if(!(y.provider_origin==="EU_B2B"||y.provider_origin==="DK_B2B"))return!1;const C=y.acquisition_date?new Date(y.acquisition_date):null;return C?C>=n&&C<=i:!1}).forEach(y=>{const I=new Date(y.acquisition_date),C=I.toISOString().slice(0,10).replace(/-/g,""),M=parseFloat(y.cost||0),V=y.provider_origin==="EU_B2B"&&y.item_phantom_vat||0,Y=y.provider_origin==="DK_B2B"?y.item_real_vat||0:V;u.push({transactionId:`ECP-INV-${C}-${String(p).padStart(4,"0")}`,invoiceDate:I.toISOString().slice(0,10),category:`Stock Import (${y.provider_origin})`,vendor:y.provider_origin,description:`${y.album||"N/A"} - ${y.artist||"N/A"}`,sku:y.sku||"N/A",grossAmount:M.toFixed(2),euPhantomVat:V.toFixed(2),inputVat:Y.toFixed(2)}),p++});const x="\uFEFF",w=(y,I)=>[y.join(","),...I.map(C=>y.map(M=>{const V=Object.keys(C)[y.indexOf(M)];return`"${String(C[V]||"").replace(/"/g,'""')}"`}).join(","))].join(`
`),b=["Transaction ID","Transaction Date","Sales Channel","Product Name","SKU / Item ID","Provider Origin","Acquisition Date","Condition","Cost Price (DKK)","Sales Price (DKK)","Calculation Basis (DKK)","VAT Scheme Applied","Output VAT / Salgsmoms (DKK)","EU Phantom VAT / Rubrik A (DKK)","Input VAT / Købsmoms (DKK)"],f=["Transaction ID","Invoice Date","Category","Vendor / Origin","Description","SKU / Item ID","Gross Amount / Cost (DKK)","EU Phantom VAT / Rubrik A (DKK)","Input VAT / Købsmoms (DKK)"],$=w(b,d),E=w(f,u),_=new Blob([x+$],{type:"text/csv;charset=utf-8;"}),D=document.createElement("a");D.href=URL.createObjectURL(_),D.download=`Sales_VAT_Ledger_Q${o}_${r}.csv`,D.style.display="none",document.body.appendChild(D),D.click(),setTimeout(()=>{const y=new Blob([x+E],{type:"text/csv;charset=utf-8;"}),I=document.createElement("a");I.href=URL.createObjectURL(y),I.download=`Purchases_VAT_Ledger_Q${o}_${r}.csv`,I.style.display="none",document.body.appendChild(I),I.click(),document.body.removeChild(D),document.body.removeChild(I),URL.revokeObjectURL(D.href),URL.revokeObjectURL(I.href)},300),this.showToast(`✅ Exported ${d.length} sales & ${u.length} purchase records.`)},renderInvestments(t){const e=["Alejo","Facundo","Rafael"],s=this.state.investments||[],o=e.reduce((n,i)=>(n[i]=s.filter(l=>l.partner===i).reduce((l,d)=>l+(parseFloat(d.amount)||0),0),n),{}),r=Object.values(o).reduce((n,i)=>n+i,0),a=`
            <div class="max-w-7xl mx-auto px-4 md:px-8 pb-24 pt-6">
                <div class="flex justify-between items-center mb-8">
                    <div>
                ...
                ${e.map(n=>{const i=s.filter(l=>l.partner===n).sort((l,d)=>new Date(d.date)-new Date(l.date));return`
        ...
        `;document.body.insertAdjacentHTML("beforeend",s)},async saveInvestment(t){t.preventDefault();const e=t.target,s={partner:e.partner.value,amount:parseFloat(e.amount.value),description:e.description.value,date:e.date.value,created_at:firebase.firestore.FieldValue.serverTimestamp()};try{await S.collection("investments").add(s),document.getElementById("add-investment-modal").remove(),this.showToast("✅ Inversión registrada"),await this.loadInvestments(),this.refreshCurrentView()}catch(o){this.showToast("❌ Error: "+o.message,"error")}},async deleteInvestment(t){if(confirm("¿Eliminar esta inversión?"))try{await S.collection("investments").doc(t).delete(),this.showToast("🗑️ Inversión eliminada"),await this.loadInvestments(),this.refreshCurrentView()}catch(e){this.showToast("❌ Error: "+e.message,"error")}},async loadInvestments(){const t=await S.collection("investments").get();this.state.investments=t.docs.map(e=>({id:e.id,...e.data()}))},renderShipping(t){const e=l=>{var d;return((d=l.shipping_method)==null?void 0:d.id)==="local_pickup"||l.shipping_method&&typeof l.shipping_method=="string"&&l.shipping_method.toLowerCase().includes("pickup")||l.shippingMethod&&l.shippingMethod.toLowerCase().includes("pickup")||Number(l.shipping)===0||Number(l.shipping_cost)===0||Number(l.shipping_income)===0},s=l=>!e(l),o=l=>!["shipped","picked_up","delivered","fulfilled"].includes(l.fulfillment_status),r=this.state.sales.filter(l=>{var d;return(l.channel==="online"||((d=l.channel)==null?void 0:d.toLowerCase())==="discogs")&&e(l)&&o(l)}).sort((l,d)=>new Date(l.date)-new Date(d.date)),a=this.state.sales.filter(l=>{var d;return(l.channel==="online"||((d=l.channel)==null?void 0:d.toLowerCase())==="discogs")&&s(l)&&o(l)}).sort((l,d)=>new Date(l.date)-new Date(d.date)),n=this.state.sales.filter(l=>{var d;return(l.channel==="online"||((d=l.channel)==null?void 0:d.toLowerCase())==="discogs")&&!o(l)}).sort((l,d)=>{var c,u;return new Date((c=d.updated_at)!=null&&c.toDate?d.updated_at.toDate():d.updated_at||d.date)-new Date((u=l.updated_at)!=null&&u.toDate?l.updated_at.toDate():l.updated_at||l.date)}).slice(0,20),i=`
            <div class="max-w-7xl mx-auto px-4 md:px-8 pb-24 pt-6 animate-fadeIn">
                   ...
                   <div class="flex gap-4">
                        <div class="bg-indigo-500 text-white px-5 py-3 rounded-2xl shadow-lg shadow-indigo-500/20 flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-2xl font-display font-bold">${this.formatCurrency(this.state.sales.reduce((l,d)=>l+parseFloat(d.shipping||d.shipping_cost||0),0))}</p>
                            </div>
                        </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-blue-50">
                                ${r.length>0?r.map(l=>{var f;const d=this.getCustomerInfo(l),c=l.fulfillment_status||"unfulfilled";let u=c==="unfulfilled"?`onclick="app.notifyPreparingDiscogs('${l.id}')"`:"disabled",p=c==="unfulfilled"?"bg-blue-500 text-white shadow-sm hover:bg-blue-600":c==="preparing"||c==="ready_for_pickup"?"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-75":"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-50",v=c==="preparing"?`onclick="app.notifyPickupReadyDiscogs('${l.id}')"`:"disabled",m=c==="preparing"?"bg-brand-orange text-white shadow-sm hover:bg-orange-600":c==="ready_for_pickup"?"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-75":"bg-slate-50 border border-slate-100 text-slate-300 cursor-not-allowed opacity-50",x=c==="ready_for_pickup"?`onclick="app.markPickedUpDiscogs('${l.id}')"`:"disabled",b=`
                <div class="flex flex-col gap-2 relative pl-2">
                    ...
                    <button ${u} class="w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-between ${p}">
                    ...
                    <button ${v} class="w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-between ${m}">
                        <span class="flex items-center gap-2">
                    ...
                    <button ${x} class="w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-between ${c==="ready_for_pickup"?"bg-brand-dark text-white shadow-sm hover:bg-black":"bg-slate-50 border border-slate-100 text-slate-300 cursor-not-allowed opacity-50"}">
                        <span class="flex items-center gap-2">
            ...
            `;return`
                                    <tr class="hover:bg-blue-50/20 transition-colors">
                                        ...
                                        <td class="p-4">
                                            ...
                                            <div class="text-[10px] text-slate-400 mt-1">${((f=l.items)==null?void 0:f.length)||0} items</div>
                                        </td>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-orange-100 overflow-hidden mb-8">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-orange-50">
                                ${a.length>0?a.map(l=>{var b;const d=this.getCustomerInfo(l),c=l.fulfillment_status||"unfulfilled";let u=c==="unfulfilled"?`onclick="app.notifyPreparingDiscogs('${l.id}')"`:"disabled",p=c==="unfulfilled"?"bg-brand-orange text-white shadow-sm hover:bg-orange-600":c==="preparing"||c==="in_transit"?"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-75":"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-50",v=c==="in_transit"?`onclick="app.markDispatchedDiscogs('${l.id}')"`:"disabled",m=c==="in_transit"?"bg-brand-dark text-white shadow-sm hover:bg-black":"bg-slate-50 border border-slate-100 text-slate-300 cursor-not-allowed opacity-50",x="";c==="preparing"?x=`
            ...
            `;return`
                                    <tr class="hover:bg-orange-50/20 transition-colors">
                                        ...
                                        <td class="p-4">
                                            <div class="flex -space-x-2 overflow-hidden">
                                                ${(l.items||[]).slice(0,3).map(f=>`<img src="${f.image||f.cover_image||"https://elcuartito.dk/default-vinyl.png"}" 
                                                         class="inline-block h-8 w-8 rounded-full ring-2 ring-white object-cover" 
                                                         title="${f.album}">`).join("")}
                                                ${(l.items||[]).length>3?`<span class="h-8 w-8 rounded-full bg-gray-100 flex items-center justify-center text-[10px] ring-2 ring-white text-slate-500 font-bold">+${l.items.length-3}</span>`:""}
                                            </div>
                                ...
                                ${n.map(l=>{var d;return`
                                    <tr class="hover:bg-slate-50 transition-colors cursor-pointer" onclick="app.openUnifiedOrderDetailModal('${l.id}')" title="Ver historial">
                                        ...
                                        <td class="p-4 text-xs text-slate-400">
                                            ${this.formatDate((d=l.updated_at)!=null&&d.toDate?l.updated_at.toDate():l.updated_at||l.date)}
                                        </td>
                                        <td class="p-4 text-right">
                ...
                <div class="p-8 max-h-[60vh] overflow-y-auto">
                    ...
                    <div class="mt-8 pt-6 border-t border-slate-50 flex justify-between items-end">
                       <div class="text-xs text-slate-400">
                            Cliente: <span class="font-bold text-slate-600">${e.customerName||((c=e.customer)==null?void 0:c.name)||((u=e.customer)==null?void 0:u.firstName)+" "+((p=e.customer)==null?void 0:p.lastName)}</span><br>
        ...
        `,document.body.appendChild(t)},async handleBulkImportBatch(){const t=document.getElementById("bulk-csv-data").value.trim();if(!t){this.showToast("Por favor, pega el contenido del CSV.","error");return}const e=document.getElementById("start-bulk-import-btn");e.innerHTML,e.disabled=!0,e.innerHTML='<i class="ph-bold ph-spinner animate-spin"></i> Importando...';try{const s=await fetch(`${R}/discogs/bulk-import`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({csvData:t})}),o=await s.json();s.ok&&(this.showToast(`✅ ${o.summary}`),document.getElementById("bulk-import-modal").remove(),await this.loadData(),this.refreshCurrentView())}catch(s){console.error("Bulk import error:",s),this.showToast("❌ "+s.message,"error");const o=document.getElementById("start-bulk-import-btn");o&&(o.disabled=!1,o.innerHTML='<i class="ph-bold ph-rocket-launch"></i> Comenzar Importación')}},async refreshProductMetadata(t){const e=document.getElementById("refresh-metadata-btn");if(!e)return;const s=e.innerHTML;e.disabled=!0,e.innerHTML='<i class="ph-bold ph-spinner animate-spin"></i> ...';try{let o=t;const r=this.state.inventory.find(i=>i.sku===t||i.id===t);r&&r.id&&(o=r.id);const a=await fetch(`${R}/discogs/refresh-metadata/${o}`,{method:"POST",headers:{"Content-Type":"application/json"}}),n=await a.json();if(a.ok){this.showToast("✅ Metadata actualizada correctamente");const i=document.getElementById("modal-overlay");i&&i.remove(),await this.loadData(),this.refreshCurrentView(),r&&this.openProductModal(r.sku)}else throw new Error(n.error||"Error al actualizar metadata")}catch(o){console.error("Refresh metadata error:",o),this.showToast("❌ "+o.message,"error"),e.disabled=!1,e.innerHTML=s}}};window.app=Pe;document.addEventListener("DOMContentLoaded",()=>{Pe.init()});window.migrateAllData=async function(t=!0){const e=firebase.firestore();console.log(`🚀 Iniciando Migración Total... (Dry Run: ${t})`);try{const s=await e.collection("products").get(),o=await e.collection("sales").get();let r=[],a=[];const n=new Map;for(const i of s.docs){const l=i.data();let d=!1,c={};l.sku&&n.set(l.sku,l),n.set(i.id,l),(!l.provider_origin||l.provider_origin==="Local_Used")&&(l.product_condition==="New"||l.condition==="New"?l.provider_origin!=="EU_B2B"&&(c.provider_origin="EU_B2B",d=!0):l.provider_origin||(c.provider_origin="Local_Used",d=!0)),d&&(r.push({id:i.id,changes:c,name:l.album}),Object.assign(l,c))}for(const i of o.docs){const l=i.data();let d=!1;if(!l.items||!Array.isArray(l.items))continue;let c=l.items.map(u=>{let p={...u},v=null;if(u.productCondition==="New"||u.condition==="New")v="EU_B2B";else if(u.productCondition==="Second-hand"||u.productCondition==="Used")v="Local_Used";else{const m=u.productId||u.recordId,x=n.get(u.sku)||n.get(m);x?v=x.provider_origin||"Local_Used":v="Local_Used"}return p.providerOrigin!==v&&(p.providerOrigin=v,d=!0),p});d&&a.push({id:i.id,items:c})}if(console.log(`📦 Discos de inventario a corregir: ${r.length}`),r.length>0&&console.log("Ejemplo de disco:",r[0]),console.log(`🧾 Ventas a corregir: ${a.length}`),t)console.log("⚠️ MODO DRY RUN. Escribí window.migrateAllData(false) para aplicar los cambios.");else{console.log("💾 Guardando en Firestore...");const i=e.batch();let l=0;const d=async()=>{l>0&&(await i.commit(),console.log(`✅ Batch commited (${l} operaciones)`),l=0)};for(const c of r)i.update(e.collection("products").doc(c.id),c.changes),l++,l>=450&&await d();for(const c of a)i.update(e.collection("sales").doc(c.id),{items:c.items}),l++,l>=450&&await d();await d(),console.log("🎉 Migración Total Finalizada!")}}catch(s){console.error("❌ Error en la migración:",s)}};window.migrateAllData(!0);
