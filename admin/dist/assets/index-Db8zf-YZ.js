...
¿Estás seguro?`))return;if(prompt("Para confirmar, ingresa la contraseña de administrador:")!=="alejo13"){alert("Contraseña incorrecta. Operación cancelada.");return}this.showToast("Borrando todas las ventas..."),E.collection("sales").get().then(e=>{const s=E.batch();return e.docs.forEach(o=>{s.delete(o.ref)}),s.commit()}).then(()=>{this.showToast("✅ Todas las ventas han sido eliminadas"),setTimeout(()=>location.reload(),1500)}).catch(e=>{console.error(e),alert("Error al borrar ventas: "+e.message)})},async findProductBySku(t){try{const e=await E.collection("products").where("sku","==",t).get();if(e.empty)return null;const s=e.docs[0];return{id:s.id,ref:s.ref,data:s.data()}}catch(e){return console.error("Error finding product by SKU:",e),null}},logInventoryMovement(t,e){let s="";t==="EDIT"?s="Producto actualizado":t==="ADD"?s="Ingreso de inventario":t==="DELETE"?s="Egreso manual":t==="SOLD"&&(s="Venta registrada"),E.collection("inventory_logs").add({type:t,sku:e.sku||"Unknown",album:e.album||"Unknown",artist:e.artist||"Unknown",timestamp:firebase.firestore.FieldValue.serverTimestamp(),details:s}).catch(o=>console.error("Error logging movement:",o))},openInventoryLogModal(){E.collection("inventory_logs").orderBy("timestamp","desc").limit(50).get().then(t=>{const s=`
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-left">
                                    <tbody class="divide-y divide-slate-50">
                                        ${l.map(h=>{const y=h.type==="sale",L=y?h.album||"Venta de Items":h.proveedor||h.description||"Gasto registrado",R=y?h.channel||"Tienda Local":h.categoria||"Operativo";let _="ph-receipt";if(y){const F=(h.channel||"").toLowerCase();F.includes("web")&&(_="ph-globe-simple"),F.includes("discogs")&&(_="ph-vinyl-record")}else _="ph-credit-card";return`
    ...
    <div id="modal-overlay" class="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
        ...
        `;document.body.insertAdjacentHTML("beforeend",m)},openInvoiceModal(t){var f;const e=this.state.sales.find(k=>k.id===t);if(!e){this.showToast("Sale not found","error");return}const s=e.items||[],o=(f=e.date)!=null&&f.toDate?e.date.toDate():new Date(e.date||e.timestamp),r=o.toISOString().slice(0,10).replace(/-/g,""),a=e.invoiceNumber||`ECR-${r}-${t.slice(-4).toUpperCase()}`,l=s.filter(k=>k.productCondition==="New"),i=s.filter(k=>k.productCondition!=="New");let n=0,c=0;const d=(k,I)=>k.map($=>{const D=$.priceAtSale||$.price||0,S=$.qty||$.quantity||1,P=D*S;if(c+=P,I){const A=P*.2;return n+=A,`
                        <tr>
...
¿CONFIRMAS que deseas eliminar permanentemente este registro y su recibo?`)){this.showToast("ℹ️ Eliminación cancelada");return}}else if(!confirm("¿Estás seguro de que quieres eliminar este gasto?"))return;try{await E.collection("expenses").doc(t).delete(),this.showToast("✅ Gasto eliminado"),this.loadData()}catch(s){console.error("Error deleting expense:",s),this.showToast("❌ Error al eliminar gasto")}},renderVATReport(t){const e=new Date,s=Math.floor(e.getMonth()/3)+1,o=e.getFullYear(),r=this.state.vatReportQuarter||s,a=this.state.vatReportYear||o,l=(r-1)*3,i=new Date(a,l,1),n=new Date(a,l+3,0,23,59,59),c=this.state.sales.filter(v=>{var N;const C=(N=v.timestamp)!=null&&N.toDate?v.timestamp.toDate():new Date(v.timestamp||v.date);return C>=i&&C<=n});let d=[],u=[],m=[],g=0,b=0,x=0,w=0;c.forEach(v=>{var J;const C=(J=v.timestamp)!=null&&J.toDate?v.timestamp.toDate():new Date(v.timestamp||v.date);(v.items||[]).forEach(T=>{const H=T.priceAtSale||T.price||0;let W=T.costAtSale||T.cost||0;const Z=T.productId||T.recordId,X=T.album;if(W===0){const _=this.state.inventory.find(F=>Z&&(F.id===Z||F.sku===Z)||X&&F.album===X);_&&(W=_.cost||0)}const h=T.qty||T.quantity||1,y=T.productCondition||"Second-hand",L=H*h,R=W*h;if(y==="New"){const _=L*.2;g+=_,d.push({date:C,productId:T.productId||T.album||"N/A",album:T.album||"N/A",salePrice:L,vat:_})}else{const _=L-R,F=_>0?_*.2:0;b+=F,u.push({date:C,productId:T.productId||T.album||"N/A",album:T.album||"N/A",cost:R,salePrice:L,margin:_,vat:F})}});const O=parseFloat(v.shipping_income||v.shipping||v.shipping_cost||0);if(O>0){const T=O*.2;x+=T,w+=O,m.push({date:C,orderId:v.orderNumber||(v.id&&typeof v.id=="string"?v.id.slice(-8):"N/A"),income:O,vat:T})}}),(this.state.extraIncome||[]).filter(v=>{const C=new Date(v.date);return C>=i&&C<=n}).forEach(v=>{const C=Number(v.amount)||0,N=Number(v.vatAmount)||0;g+=N,d.push({date:new Date(v.date),productId:"EXTRA",album:`💰 ${v.description||"Ingreso Extra"} (${v.category||"other"})`,salePrice:C,vat:N})});const f=g+b+x,k=this.state.expenses.filter(v=>{var O;const C=v.fecha_factura?new Date(v.fecha_factura):(O=v.timestamp)!=null&&O.toDate?v.timestamp.toDate():new Date(v.timestamp||v.date);return(v.categoria_tipo==="operativo"||v.categoria_tipo==="stock_nuevo"||v.is_vat_deductible)&&C>=i&&C<=n}),I=k.filter(v=>v.categoria!=="envios"),$=k.filter(v=>v.categoria==="envios"),D=I.reduce((v,C)=>v+(parseFloat(C.monto_iva)||0),0),S=$.reduce((v,C)=>v+(parseFloat(C.monto_iva)||0),0);$.reduce((v,C)=>v+(parseFloat(C.monto_total)||0),0);const P=D+S,A=f-P,U={1:`1 de junio, ${a}`,2:`1 de septiembre, ${a}`,3:`1 de diciembre, ${a}`,4:`1 de marzo, ${a+1}`}[r],K=`
            <div class="max-w-7xl mx-auto px-4 md:px-8 pb-24 md:pb-8 pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="${A>0?"bg-red-50 border-red-100":"bg-emerald-50 border-emerald-100"} rounded-3xl p-8 border shadow-sm relative overflow-hidden group">
                        <p class="${A>0?"text-red-700/60":"text-emerald-700/60"} text-xs font-bold uppercase tracking-widest mb-4">Moms Tilsvar</p>
                        <p class="text-4xl font-display font-bold mb-2 ${A>0?"text-red-700":"text-emerald-700"}">${this.formatCurrency(A)}</p>
                        <p class="text-[11px] ${A>0?"text-red-600/70":"text-emerald-600/70"} mt-4 italic font-medium">
