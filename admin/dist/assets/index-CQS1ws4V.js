(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))o(r);new MutationObserver(r=>{for(const a of r)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function s(r){const a={};return r.integrity&&(a.integrity=r.integrity),r.referrerPolicy&&(a.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?a.credentials="include":r.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function o(r){if(r.ep)return;r.ep=!0;const a=s(r);fetch(r.href,a)}})();const D=firebase.firestore(),Ne="2026.03.20.1";console.log("🚀 El Cuartito Admin v"+Ne+" loaded");const ee=window.auth,Ve=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1",R=Ve?"http://localhost:3001":"https://el-cuartito-shop.up.railway.app",Oe="K85403890688957",ve={async createSale(t){let e=[];if(await D.runTransaction(async s=>{const o=[];for(const n of t.items){const l=D.collection("products").doc(n.recordId||n.productId),p=await s.get(l);if(!p.exists)throw new Error(`Producto ${n.recordId} no encontrado`);const c=p.data();if(c.stock<n.quantity)throw new Error(`Stock insuficiente para ${c.artist||"Sin Artista"} - ${c.album||"Sin Album"}. Disponible: ${c.stock}`);o.push({ref:l,data:c,quantity:n.quantity,price:c.price,cost:c.cost||0,providerOrigin:c.provider_origin||"Local_Used",productCondition:n.productCondition||n.condition||c.product_condition||c.condition||"Used"})}const r=o.reduce((n,l)=>n+l.price*l.quantity,0),a=t.customTotal!==void 0?t.customTotal:r,i=D.collection("sales").doc();s.set(i,{...t,status:"completed",fulfillment_status:t.channel&&t.channel.toLowerCase()==="discogs"?"preparing":"fulfilled",total:a,date:new Date().toISOString().split("T")[0],timestamp:firebase.firestore.FieldValue.serverTimestamp(),items:o.map(n=>({productId:n.ref.id,artist:n.data.artist,album:n.data.album,sku:n.data.sku,unitPrice:n.price,costAtSale:n.cost,qty:n.quantity,providerOrigin:n.providerOrigin||"Local_Used",productCondition:n.productCondition||"Used"}))});for(const n of o){s.update(n.ref,{stock:firebase.firestore.FieldValue.increment(-n.quantity)});const l=D.collection("inventory_logs").doc();s.set(l,{type:"SOLD",sku:n.data.sku||"Unknown",album:n.data.album||"Unknown",artist:n.data.artist||"Unknown",timestamp:firebase.firestore.FieldValue.serverTimestamp(),details:`Venta registrada (Admin) - Canal: ${t.channel||"Tienda"}`})}e=o.map(n=>({discogs_listing_id:n.data.discogs_listing_id,artist:n.data.artist,album:n.data.album}))}),t.channel&&t.channel.toLowerCase()==="discogs"){for(const s of e)if(s.discogs_listing_id)try{const o=await fetch(`${R}/discogs/delete-listing/${s.discogs_listing_id}`,{method:"DELETE"});o.ok?console.log(`✅ Discogs listing ${s.discogs_listing_id} deleted for ${s.artist} - ${s.album}`):console.warn(`⚠️ Could not delete Discogs listing ${s.discogs_listing_id}:`,await o.text())}catch(o){console.error(`❌ Error deleting Discogs listing ${s.discogs_listing_id}:`,o)}}},async notifyPreparing(t){const e=await ee.currentUser.getIdToken(),s=await fetch(`${R}/sales/${t}/notify-preparing`,{method:"POST",headers:{Authorization:`Bearer ${e}`}});if(!s.ok)throw new Error(await s.text());return s.json()},async cancelOrder(t){const e=await ee.currentUser.getIdToken(),s=await fetch(`${R}/sales/${t}/cancel-order`,{method:"POST",headers:{Authorization:`Bearer ${e}`}});if(!s.ok)throw new Error(await s.text());return s.json()},async updateTracking(t,e){const s=await ee.currentUser.getIdToken(),o=await fetch(`${R}/sales/${t}/update-tracking`,{method:"POST",headers:{Authorization:`Bearer ${s}`,"Content-Type":"application/json"},body:JSON.stringify({trackingNumber:e})});if(!o.ok)throw new Error(await o.text());return o.json()},async notifyShipped(t,e,s=null){const o=await ee.currentUser.getIdToken(),r={trackingNumber:e};s&&(r.trackingLink=s);const a=await fetch(`${R}/sales/${t}/notify-shipped`,{method:"POST",headers:{Authorization:`Bearer ${o}`,"Content-Type":"application/json"},body:JSON.stringify(r)});if(!a.ok)throw new Error(await a.text());return a.json()},async markDispatched(t){const e=await ee.currentUser.getIdToken(),s=await fetch(`${R}/sales/${t}/mark-dispatched`,{method:"POST",headers:{Authorization:`Bearer ${e}`}});if(!s.ok)throw new Error(await s.text());return s.json()},async notifyPickupReady(t){const e=await ee.currentUser.getIdToken(),s=await fetch(`${R}/sales/${t}/notify-pickup-ready`,{method:"POST",headers:{Authorization:`Bearer ${e}`}});if(!s.ok)throw new Error(await s.text());return s.json()},async markPickedUp(t){const e=await ee.currentUser.getIdToken(),s=await fetch(`${R}/sales/${t}/mark-picked-up`,{method:"POST",headers:{Authorization:`Bearer ${e}`}});if(!s.ok)throw new Error(await s.text());return s.json()}},Se={async runSkuFix_v3(){if(!localStorage.getItem("sku_fix_run_v3")){localStorage.setItem("sku_fix_run_v3","true"),console.log("🔍 Buscando el SKU histórico más alto absoluto...");try{const t=await D.collection("products").get();let e=0;const s=/^SKU-(\d+)$/;t.docs.forEach(n=>{const l=n.data().sku;if(l){const p=l.match(s);p&&(e=Math.max(e,parseInt(p[1])))}});const o=await D.collection("sales").get();o.docs.forEach(n=>{(n.data().items||[]).forEach(p=>{const c=p.sku;if(c){const u=c.match(s);u&&(e=Math.max(e,parseInt(u[1])))}})}),console.log("✅ El SKU histórico más alto encontrado es:",e);const r=new Set;o.docs.forEach(n=>{(n.data().items||[]).forEach(l=>{l.sku&&r.add(l.sku)})});const a=[],i=t.docs.map(n=>({id:n.id,ref:n.ref,data:n.data()})).sort((n,l)=>{var p,c;return(((p=n.data.created_at)==null?void 0:p.seconds)||0)-(((c=l.data.created_at)==null?void 0:c.seconds)||0)});for(const n of i){const l=n.data.sku;l&&(r.has(l)?a.push(n):r.add(l))}if(a.length>0){console.log(`⚠️ Encontrados ${a.length} productos con SKU en conflicto. Arreglando...`);let n=e,l="";await D.runTransaction(async p=>{const c=D.collection("metadata").doc("vinylCounter");for(const u of a){n++;const d=`SKU-${String(n).padStart(3,"0")}`,y=String(n).padStart(4,"0");p.update(u.ref,{sku:d,quickId:y}),l+=`- ${u.data.album} (era ${u.data.sku}) -> ahora es ${d}
`}p.set(c,{current:n},{merge:!0})}),alert(`🛠️ Arreglo histórico completado!
Se reasignaron los SKUs que chocaban con discos viejos vendidos:

`+l+`
Por favor, recarga la página.`)}else{const n=D.collection("metadata").doc("vinylCounter"),l=await n.get(),p=l.exists&&l.data().current||0;e>p&&(await n.set({current:e},{merge:!0}),console.log(`🆙 Contador actualizado a ${e}`)),console.log("✅ No hay conflictos de SKU.")}}catch(t){console.error("❌ Error en script histórico:",t)}}},state:{inventory:[],sales:[],expenses:[],consignors:[],cart:[],viewMode:"list",selectedItems:new Set,currentView:"dashboard",filterMonths:[new Date().getMonth()],filterYear:new Date().getFullYear(),inventorySearch:"",salesHistorySearch:"",expensesSearch:"",events:[],selectedDate:new Date,vatActive:!1,manualSaleSearch:"",posCondition:"Used",posSelectedItemSku:null,orderFeedFilter:"all",filterGenre:"all",filterOwner:"all",filterLabel:"all",filterStorage:"all",filterDiscogs:"all",filterStock:"all",filterCondition:"all",filterStockTime:[],showStats:!1,showAdvancedFilters:!1,privacyMode:!1,rsdExtraDiscount:!1,dashboardAnalysisMode:"genre"},getEffectivePrice(t){return t.is_rsd_discount?Math.round(t.price*.9):t.price},async init(){this.runSkuFix_v3(),!this._initialized&&(this._initialized=!0,ee.onAuthStateChanged(async t=>{if(t)try{document.getElementById("login-view").classList.add("hidden"),document.getElementById("main-app").classList.remove("hidden"),document.getElementById("mobile-nav").classList.remove("hidden"),await this.loadData(),this._pollInterval&&clearInterval(this._pollInterval),this._pollInterval=setInterval(()=>this.loadData(),6e4),this.setupListeners(),this.setupMobileMenu(),this.setupNavigation()}catch(e){console.error("Auth token error:",e),this.logout()}else{document.getElementById("login-view").classList.remove("hidden"),document.getElementById("main-app").classList.add("hidden"),document.getElementById("mobile-nav").classList.add("hidden");const e=document.getElementById("login-btn");e&&(e.disabled=!1,e.innerHTML="<span>Entrar</span>")}}),document.addEventListener("click",t=>{const e=document.getElementById("discogs-results"),s=document.getElementById("discogs-search-input");e&&!e.contains(t.target)&&t.target!==s&&e.classList.add("hidden");const o=document.getElementById("sku-results"),r=document.getElementById("sku-search");o&&!o.contains(t.target)&&t.target!==r&&o.classList.add("hidden")}))},async handleLogin(t){t.preventDefault();const e=t.target.email.value,s=t.target.password.value,o=document.getElementById("login-error"),r=document.getElementById("login-btn");o.classList.add("hidden"),r.disabled=!0,r.innerHTML="<span>Cargando...</span>";try{await ee.signInWithEmailAndPassword(e,s)}catch(a){console.error("Login error:",a),o.innerText="Error: "+a.message,o.classList.remove("hidden"),r.disabled=!1,r.innerHTML='<span>Ingresar</span><i class="ph-bold ph-arrow-right"></i>'}},async updateFulfillmentStatus(t,e,s){var o,r,a;try{const i=((o=t==null?void 0:t.target)==null?void 0:o.closest("button"))||((a=(r=window.event)==null?void 0:r.target)==null?void 0:a.closest("button"));if(i){i.disabled=!0;const n=i.innerHTML;i.innerHTML='<i class="ph ph-circle-notch animate-spin"></i>'}await D.collection("sales").doc(e).update({fulfillment_status:s}),await this.loadData(),document.getElementById("modal-overlay")&&(document.getElementById("modal-overlay").remove(),this.openOnlineSaleDetailModal(e)),this.showToast("Estado de envío actualizado")}catch(i){console.error("Fulfillment update error:",i),this.showToast("Error al actualizar estado: "+i.message,"error")}},async manualShipOrder(t){var e,s,o,r,a,i;try{const n=prompt("Introduce el número de seguimiento:");if(!n)return;const l=((e=event==null?void 0:event.target)==null?void 0:e.closest("button"))||((o=(s=window.event)==null?void 0:s.target)==null?void 0:o.closest("button"));l&&(l.disabled=!0,l.innerHTML='<i class="ph ph-circle-notch animate-spin"></i> Guardando...');const p=await fetch(`${R}/api/manual-ship`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({orderId:t,trackingNumber:n})}),c=await p.json();if(p.ok&&c.success){if(this.showToast("✅ Pedido marcado como enviado"),c.emailSent)this.showToast("📧 Cliente notificado por email","success");else{const d=typeof c.emailError=="object"?JSON.stringify(c.emailError):c.emailError;this.showToast("⚠️ Pedido marcado pero EL EMAIL FALLÓ: "+d,"warning")}await this.loadData();const u=document.getElementById("sale-detail-modal");u&&(u.remove(),this.openUnifiedOrderDetailModal(t))}else throw new Error(c.error||c.message||"Error desconocido")}catch(n){console.error("Error shipping manually:",n),this.showToast("❌ Error: "+(n.message||"No se pudo procesar el envío"),"error");const l=((r=event==null?void 0:event.target)==null?void 0:r.closest("button"))||((i=(a=window.event)==null?void 0:a.target)==null?void 0:i.closest("button"));l&&(l.disabled=!1,l.innerHTML='<i class="ph-bold ph-truck"></i> Ingresar Tracking y Cerrar')}},async logout(){try{await ee.signOut(),location.reload()}catch(t){console.error("Sign out error:",t),location.reload()}},setupListeners(){this._unsubscribeProducts&&this._unsubscribeProducts(),this._unsubscribeProducts=D.collection("products").onSnapshot(t=>{this.state.inventory=t.docs.map(e=>{const s=e.data();return{id:e.id,...s,condition:s.condition||"VG",owner:s.owner||"El Cuartito",label:s.label||"Desconocido",storageLocation:s.storageLocation||"Tienda",cover_image:s.cover_image||s.coverImage||null}}),(this.state.currentTab==="inventory"||this.state.currentTab==="dashboard")&&this.renderCurrentTab()},t=>{console.error("Inventory listener error:",t)})},async loadData(){try{const[t,e,s,o,r,a]=await Promise.all([D.collection("products").get(),D.collection("sales").get(),D.collection("expenses").get(),D.collection("events").orderBy("date","desc").get(),D.collection("consignors").get(),D.collection("extra_income").get()]);this.state.inventory=t.docs.map(i=>{const n=i.data();return{id:i.id,...n,condition:n.condition||"VG",owner:n.owner||"El Cuartito",label:n.label||"Desconocido",storageLocation:n.storageLocation||"Tienda",cover_image:n.cover_image||n.coverImage||null}}),this.state.sales=e.docs.map(i=>{var p,c;const n=i.data(),l={id:i.id,...n,date:n.date||((p=n.timestamp)!=null&&p.toDate?n.timestamp.toDate().toISOString().split("T")[0]:(c=n.created_at)!=null&&c.toDate?n.created_at.toDate().toISOString().split("T")[0]:new Date().toISOString().split("T")[0])};return n.total_amount!==void 0&&n.total===void 0&&(l.total=n.total_amount),n.payment_method&&!n.paymentMethod&&(l.paymentMethod=n.payment_method),l.items&&Array.isArray(l.items)&&(l.items=l.items.map(u=>({...u,priceAtSale:u.priceAtSale!==void 0?u.priceAtSale:u.unitPrice||0,qty:u.qty!==void 0?u.qty:u.quantity||1,costAtSale:u.costAtSale!==void 0?u.costAtSale:u.cost||0}))),l}).filter(i=>i.status!=="PENDING"&&i.status!=="failed").sort((i,n)=>{const l=new Date(i.date);return new Date(n.date)-l}),this.state.expenses=s.docs.map(i=>{var l;const n=i.data();return{id:i.id,...n,date:n.fecha_factura||n.date||((l=n.timestamp)==null?void 0:l.split("T")[0])||new Date().toISOString().split("T")[0]}}).sort((i,n)=>new Date(n.date)-new Date(i.date)),this.state.events=o.docs.map(i=>({id:i.id,...i.data()})),this.state.consignors=r.docs.map(i=>{const n=i.data();return{id:i.id,...n,agreementSplit:n.split||n.agreementSplit||(n.percentage?Math.round(n.percentage*100):70)}}),await this.loadInvestments(),this.state.extraIncome=a.docs.map(i=>({id:i.id,...i.data()})).sort((i,n)=>new Date(n.date)-new Date(i.date)),this.initFuse(),this.refreshCurrentView()}catch(t){console.error("Failed to load data:",t),this.showToast("❌ Error de conexión: "+t.message,"error")}},refreshCurrentView(){const t=document.getElementById("app-content");if(t)switch(this.state.currentView){case"dashboard":this.renderDashboard(t);break;case"inventory":this.renderInventory(t);break;case"sales":this.renderSales(t);break;case"onlineSales":this.renderOnlineSales(t);break;case"discogsSales":this.renderDiscogsSales(t);break;case"expenses":this.renderExpenses(t);break;case"consignments":this.renderConsignments(t);break;case"backup":this.renderBackup(t);break;case"settings":this.renderSettings(t);break;case"calendar":this.renderCalendar(t);break;case"shipping":this.renderShipping(t);break;case"pickups":this.renderPickups(t);break;case"investments":this.renderInvestments(t);break;case"vatReport":this.renderVATReport(t);break;case"datosLegales":this.renderDatosLegales(t);break;case"contabilidad":this.renderContabilidad(t);break;case"facturasManual":this.renderFacturasManual(t);break;case"extraIncome":this.renderExtraIncome(t);break;case"newsletter":this.renderNewsletter(t);break}},async renderNewsletter(t){this.state.newsletterSelectedIds||(this.state.newsletterSelectedIds=[]);let e=0,s=[];try{const n=await D.collection("subscribers").where("active","==",!0).get();e=n.size,s=n.docs.map(l=>({id:l.id,...l.data()}))}catch(n){console.warn("Could not fetch subscribers count:",n)}this.state.subscribersList=s;let o=this.state.inventory&&this.state.inventory.length>0?[...this.state.inventory]:[...this.state.products||[]];if(o.length===0)try{o=(await D.collection("products").get()).docs.map(l=>({id:l.id,...l.data()})),this.state.inventory=o}catch(n){console.warn("Could not fetch products for newsletter:",n)}const r=n=>{if(n.created_at){const l=new Date(n.created_at).getTime();if(!isNaN(l))return l}if(n.createdAt){const l=new Date(n.createdAt).getTime();if(!isNaN(l))return l}if(n.timestamp){if(typeof n.timestamp.seconds=="number")return n.timestamp.seconds*1e3;const l=new Date(n.timestamp).getTime();if(!isNaN(l))return l}if(n.quickId){const l=parseInt(n.quickId,10);if(!isNaN(l))return l}if(n.sku&&n.sku.startsWith("SKU-")){const l=parseInt(n.sku.replace("SKU-",""),10);if(!isNaN(l))return l}return 0};o.sort((n,l)=>r(l)-r(n));const a=o.filter(n=>this.state.newsletterSelectedIds.includes(n.id)),i=`
                    ...
                    <div class="flex items-center gap-3">
                        <div onclick="app.showSubscribersModal()" class="bg-white px-5 py-3 rounded-2xl border border-orange-100 shadow-sm flex items-center gap-3 cursor-pointer hover:border-brand-orange hover:shadow-md transition-all group">
                            ...
                            <div>
                                ...
                                <div class="text-xl font-bold text-brand-dark">${e} <span class="text-xs font-normal text-slate-400 underline">(ver lista)</span></div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="lg:col-span-7 space-y-6">
                        ...
                        <div class="bg-white rounded-3xl p-6 border border-orange-100 shadow-sm space-y-4">
                                ...
                                <div class="divide-y divide-slate-100 max-h-80 overflow-y-auto pr-1">
                                    ${a.map(n=>`
                                        <div class="py-3 flex items-center justify-between gap-3">
                                            <div class="flex items-center gap-3">
                                                <img src="${n.cover_image||n.image||"logo.jpg"}" class="w-12 h-12 rounded-lg object-cover border border-slate-200 shrink-0">
                                                <div>
                                                    <div class="text-sm font-bold text-brand-dark leading-tight">${n.title||n.album||"Sin Título"}</div>
                                                    <div class="text-xs text-slate-500 uppercase">${n.artist||"Desconocido"} · DKK ${n.price||0}</div>
                                                </div>
                                            </div>
                                            <button onclick="app.toggleNewsletterProduct('${n.id}')" class="w-8 h-8 rounded-xl bg-slate-100 text-slate-500 hover:bg-red-50 hover:text-red-500 flex items-center justify-center transition-colors">
                            ...
                            <button onclick="app.sendNewsletterDrop()" id="send-drop-btn"
                                ...
                                <span>Enviar Weekly Drop a ${e} Suscriptores</span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-5">
                        <div class="bg-white rounded-3xl p-6 border border-orange-100 shadow-sm space-y-4">
                            <h2 class="text-lg font-bold text-brand-dark flex items-center gap-2">
                                ...
                                Buscar e Incluir Discos (${o.length})
                            </h2>
                            ...
                            <div id="newsletter-product-list" class="space-y-2 max-h-[500px] overflow-y-auto pr-1">
                                ${this.renderNewsletterProductList(o,"")}
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        `;t.innerHTML=i},renderNewsletterProductList(t,e){let s=t;if(e&&e.trim()){const r=e.toLowerCase().trim();s=t.filter(a=>a.title&&a.title.toLowerCase().includes(r)||a.album&&a.album.toLowerCase().includes(r)||a.artist&&a.artist.toLowerCase().includes(r)||a.label&&a.label.toLowerCase().includes(r))}const o=this.state.newsletterSelectedIds||[];return s.slice(0,50).map(r=>{const a=o.includes(r.id);return`
                <div onclick="app.toggleNewsletterProduct('${r.id}')" 
                    class="p-3 rounded-2xl border ${a?"border-brand-orange bg-orange-50/50":"border-slate-100 hover:border-slate-200"} cursor-pointer flex items-center justify-between gap-3 transition-all">
                    <div class="flex items-center gap-3 overflow-hidden">
                        <img src="${r.cover_image||r.image||"logo.jpg"}" class="w-10 h-10 rounded-lg object-cover border border-slate-200 shrink-0">
                        <div class="truncate">
                            <div class="text-sm font-bold text-brand-dark truncate">${r.title||r.album||"Sin Título"}</div>
                            <div class="text-xs text-slate-500 uppercase truncate">${r.artist||"Desconocido"} · DKK ${r.price||0}</div>
                        </div>
                    </div>
                    <div class="w-6 h-6 rounded-lg ${a?"bg-brand-orange text-white":"bg-slate-100 text-slate-400"} flex items-center justify-center shrink-0">
                        <i class="ph-bold ${a?"ph-check":"ph-plus"} text-xs"></i>
                    </div>
                </div>
            `}).join("")},filterNewsletterProducts(t){const e=document.getElementById("newsletter-product-list"),s=this.state.inventory&&this.state.inventory.length>0?this.state.inventory:this.state.products||[];e&&(e.innerHTML=this.renderNewsletterProductList(s,t))},async showSubscribersModal(){let t=this.state.subscribersList||[];try{t=(await D.collection("subscribers").get()).docs.map(r=>({id:r.id,...r.data()}))}catch(o){console.error("Error fetching subscribers list:",o)}const e=document.createElement("div");e.id="subscribers-modal",e.className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-fadeIn";const s=`
                ...
                <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-400 font-medium">${t.length} correos registrados</p>
                        </div>
                    </div>
                    ...
                    `:t.map(o=>`
                        <div class="py-3 flex items-center justify-between gap-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="text-sm font-bold text-brand-dark">${o.email}</div>
                                    <div class="text-xs text-slate-400">Suscrito: ${o.subscribedAt?new Date(o.subscribedAt).toLocaleDateString():"Reciente"}</div>
                                </div>
                            </div>
                            <span class="px-3 py-1 rounded-full text-xs font-bold ${o.active!==!1?"bg-green-100 text-green-700":"bg-slate-100 text-slate-500"}">
                                ${o.active!==!1?"ACTIVO":"INACTIVO"}
                            </span>
                        </div>
                    `).join("")}
                </div>
        ...
        `;e.innerHTML=s,document.body.appendChild(e)},toggleNewsletterProduct(t){this.state.newsletterSelectedIds||(this.state.newsletterSelectedIds=[]);const e=this.state.newsletterSelectedIds.indexOf(t);e>-1?this.state.newsletterSelectedIds.splice(e,1):this.state.newsletterSelectedIds.push(t),this.renderNewsletter(document.getElementById("app-content"))},async sendNewsletterDrop(){var i,n;const t=this.state.newsletterSelectedIds||[];if(t.length===0){alert("Debes seleccionar al menos un disco");return}const e=((i=document.getElementById("newsletter-subject"))==null?void 0:i.value)||"New This Week — El Cuartito Records",s=((n=document.getElementById("newsletter-intro"))==null?void 0:n.value)||"Fresh drops just landed at El Cuartito Records.";if(!confirm("¿Estás seguro de enviar este Weekly Drop a todos los suscriptores?"))return;const o=document.getElementById("send-drop-btn");o&&(o.disabled=!0,o.innerHTML='<i class="ph-bold ph-spinner animate-spin"></i> Enviando newsletter...');const a=window.location.hostname==="localhost"?"http://localhost:3001":"https://el-cuartito-shop.up.railway.app";try{const p=await(await fetch(`${a}/api/newsletter/send-drop`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({productIds:t,subject:e,intro:s})})).json();if(p.success)this.showToast(`🚀 Drop enviado exitosamente a ${p.sentCount} suscriptores`,"success"),this.state.newsletterSelectedIds=[],this.renderNewsletter(document.getElementById("app-content"));else throw new Error(p.error||"Error enviando newsletter")}catch(l){console.error("Error sending drop:",l),alert("Error al enviar el newsletter: "+l.message)}finally{o&&(o.disabled=!1)}},renderDatosLegales(t){const e=`
        ...
        `;t.innerHTML=e},renderContabilidad(t){const e=new Date().getFullYear(),s=Math.floor(new Date().getMonth()/3)+1;this.state.contabilidadYear||(this.state.contabilidadYear=e),this.state.contabilidadQuarter||(this.state.contabilidadQuarter=s),this.state.contabilidadInvoices||(this.state.contabilidadInvoices=[]),this.state.contabilidadLoading||(this.state.contabilidadLoading=!1);const o=this.state.contabilidadYear,r=this.state.contabilidadQuarter,a=this.state.contabilidadInvoices,i=this.state.contabilidadLoading,n=p=>{const c={local:"bg-emerald-100 text-emerald-700",online:"bg-blue-100 text-blue-700",discogs:"bg-purple-100 text-purple-700"},u={local:"Tienda",online:"Webshop",discogs:"Discogs"};return`<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold ${c[p]||"bg-slate-100 text-slate-600"}">${u[p]||p}</span>`},l=`
            <div class="max-w-6xl mx-auto px-4 md:px-8 pb-24 md:pb-8 pt-6">
                <!-- Header -->
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                                ...
                                <tbody>
                                    ${a.map((p,c)=>`
                                        <tr class="inv-row border-b border-slate-50 ${c%2===0?"bg-white":"bg-slate-50/30"}">
                                            ...
                                            <td class="px-5 py-3">${n(p.channel)}</td>
...
${t.message}`),this.showToast("Error en backfill","error")}},renderFacturasManual(t){const e=(this.state.contabilidadInvoices||[]).filter(o=>o.channel==="manual"||o.isManual),s=`
        ...
        `;t.innerHTML=s,this.state.manualInvoicesLoaded||this.loadManualInvoices()},async loadManualInvoices(){try{const t=await ee.currentUser.getIdToken(),e=await fetch(`${R}/invoices?year=${new Date().getFullYear()}`,{headers:{Authorization:`Bearer ${t}`}});if(!e.ok)throw new Error("Error cargando facturas");const s=await e.json();this.state.contabilidadInvoices=s.invoices||[],this.state.manualInvoicesLoaded=!0,this.state.currentView==="facturasManual"&&this.refreshCurrentView()}catch(t){console.error("Error loading manual invoices:",t)}},async submitManualInvoice(t){t.preventDefault();const e=document.getElementById("manual-invoice-form"),s=document.getElementById("manual-invoice-btn"),o=new FormData(e),r={customerName:o.get("customerName"),customerVAT:o.get("customerVAT")||void 0,customerAddress:o.get("customerAddress")||void 0,description:o.get("description"),amount:parseFloat(o.get("amount")),vatAmount:o.get("vatAmount")?parseFloat(o.get("vatAmount")):void 0,date:o.get("date"),paymentMethod:o.get("paymentMethod")};if(!r.customerName||!r.description||!r.amount||!r.date){this.showToast("Completa todos los campos obligatorios","error");return}s.disabled=!0,s.innerHTML='<i class="ph ph-circle-notch animate-spin"></i> Generando...';try{const a=await ee.currentUser.getIdToken(),i=await fetch(`${R}/invoices/manual`,{method:"POST",headers:{Authorization:`Bearer ${a}`,"Content-Type":"application/json"},body:JSON.stringify(r)});if(!i.ok){const p=await i.json();throw new Error(p.error||"Error generando factura")}const n=await i.json(),l=document.getElementById("manual-invoice-result");document.getElementById("result-invoice-number").textContent=`Factura ${n.invoiceNumber} generada`,document.getElementById("result-download-link").href=n.downloadUrl,l.classList.remove("hidden"),this.showToast(`✅ Factura ${n.invoiceNumber} generada correctamente`),e.reset(),document.querySelector('[name="date"]').value=new Date().toISOString().split("T")[0],this.state.manualInvoicesLoaded=!1,this.loadManualInvoices()}catch(a){console.error("Error generating manual invoice:",a),this.showToast("❌ Error: "+a.message,"error"),alert("Error generando factura: "+a.message)}s.disabled=!1,s.innerHTML='<i class="ph-bold ph-file-pdf"></i> Generar Factura PDF'},renderExtraIncome(t){const e=this.state.extraIncome||[],s=e.reduce((i,n)=>i+(Number(n.amount)||0),0),o=e.reduce((i,n)=>i+(Number(n.vatAmount)||0),0),r=i=>({event:"🎵 Evento",service:"🔧 Servicio",other:"📦 Otro"})[i]||i,a=e.map(i=>`
            <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                <td class="py-3 px-4 text-sm text-slate-600">${i.date||"-"}</td>
                <td class="py-3 px-4 text-sm font-medium text-brand-dark">${i.description||"-"}</td>
                <td class="py-3 px-4"><span class="text-xs font-bold px-2 py-1 rounded-full bg-orange-100 text-orange-700">${r(i.category)}</span></td>
                <td class="py-3 px-4 text-sm font-bold text-brand-dark text-right">${Number(i.amount).toFixed(2)} DKK</td>
                <td class="py-3 px-4 text-sm text-slate-500 text-right">${Number(i.vatAmount||0).toFixed(2)} DKK</td>
                <td class="py-3 px-4 text-sm text-slate-400">${i.paymentMethod||"Transfer"}</td>
                <td class="py-3 px-4 text-center">
                    <button onclick="app.deleteExtraIncome('${i.id}')" class="text-red-400 hover:text-red-600 transition-colors" title="Eliminar">
                ...
                <div class="flex flex-col lg:flex-row gap-8 h-[calc(100vh-140px)]">
                    ...
                    <div class="flex-1 bg-white rounded-2xl shadow-sm border border-orange-100 p-6 flex flex-col">
                        ...
                        <div class="grid grid-cols-7 gap-2 flex-1 auto-rows-fr">
                            ...
                            ${Array.from({length:i},(u,d)=>{const y=d+1,m=`${s}-${String(o+1).padStart(2,"0")}-${String(y).padStart(2,"0")}`,E=e.getDate()===y,$=p(y),b=new Date().toDateString()===new Date(s,o,y).toDateString();return`
...
¿Estás seguro?`))return;if(prompt("Para confirmar, ingresa la contraseña de administrador:")!=="alejo13"){alert("Contraseña incorrecta. Operación cancelada.");return}this.showToast("Borrando todas las ventas..."),D.collection("sales").get().then(e=>{const s=D.batch();return e.docs.forEach(o=>{s.delete(o.ref)}),s.commit()}).then(()=>{this.showToast("✅ Todas las ventas han sido eliminadas"),setTimeout(()=>location.reload(),1500)}).catch(e=>{console.error(e),alert("Error al borrar ventas: "+e.message)})},async findProductBySku(t){try{const e=await D.collection("products").where("sku","==",t).get();if(e.empty)return null;const s=e.docs[0];return{id:s.id,ref:s.ref,data:s.data()}}catch(e){return console.error("Error finding product by SKU:",e),null}},logInventoryMovement(t,e){let s="";t==="EDIT"?s="Producto actualizado":t==="ADD"?s="Ingreso de inventario":t==="DELETE"?s="Egreso manual":t==="SOLD"&&(s="Venta registrada"),D.collection("inventory_logs").add({type:t,sku:e.sku||"Unknown",album:e.album||"Unknown",artist:e.artist||"Unknown",timestamp:firebase.firestore.FieldValue.serverTimestamp(),details:s}).catch(o=>console.error("Error logging movement:",o))},openInventoryLogModal(){D.collection("inventory_logs").orderBy("timestamp","desc").limit(50).get().then(t=>{const s=`
        ...
        `;document.body.insertAdjacentHTML("beforeend",r)},async downloadFinancialReport(){const t=document.getElementById("reportStartDate").value,e=document.getElementById("reportEndDate").value,s=document.getElementById("btnDownloadReport");if(!t||!e){Se.showToast("Por favor, selecciona ambas fechas","error");return}if(t>e){Se.showToast('La fecha "Desde" no puede ser mayor a "Hasta"',"error");return}const o=s.innerHTML;s.innerHTML='<i class="ph-bold ph-spinner animate-spin"></i> Procesando...',s.disabled=!0,s.classList.add("opacity-70","cursor-not-allowed");try{const r=ee.currentUser?await ee.currentUser.getIdToken():"",a=`${R}/api/reports/financial?startDate=${t}&endDate=${e}`,i=await fetch(a,{method:"GET",headers:{Authorization:`Bearer ${r}`}});if(!i.ok)throw new Error(`Error al generar el reporte: ${i.statusText}`);const n=await i.blob(),l=window.URL.createObjectURL(n),p=document.createElement("a");p.href=l,p.download=`Reporte_Financiero_${t}_al_${e}.xlsx`,document.body.appendChild(p),p.click(),document.body.removeChild(p),window.URL.revokeObjectURL(l),document.getElementById("financialReportModal").remove(),Se.showToast("Reporte descargado con éxito")}catch(r){console.error("Error downloading report:",r),Se.showToast(r.message,"error")}finally{document.getElementById("btnDownloadReport")&&(s.innerHTML=o,s.disabled=!1,s.classList.remove("opacity-70","cursor-not-allowed"))}},toggleMonthFilter(t){const e=this.state.filterMonths.indexOf(t);e===-1?this.state.filterMonths.push(t):this.state.filterMonths.length>1&&this.state.filterMonths.splice(e,1),this.state.filterMonths.sort((s,o)=>s-o),this.refreshCurrentView()},renderDashboard(t){var e,s;try{const o=this.state.filterMonths,r=this.state.filterYear,a=["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"],i=this.state.sales.filter(h=>{var J;const w=(J=h.timestamp)!=null&&J.toDate?h.timestamp.toDate():new Date(h.timestamp||h.date);return w.getFullYear()===r&&o.includes(w.getMonth())}),n=[...i].sort((h,w)=>{var F,j;const J=(F=h.timestamp)!=null&&F.toDate?h.timestamp.toDate():new Date(h.timestamp||h.date);return((j=w.timestamp)!=null&&j.toDate?w.timestamp.toDate():new Date(w.timestamp||w.date))-J}),l=[...this.state.sales.map(h=>({...h,type:"sale",sortDate:new Date(h.date)})),...this.state.expenses.map(h=>({...h,type:"expense",sortDate:new Date(h.date||h.fecha_factura)}))].sort((h,w)=>w.sortDate-h.sortDate).slice(0,5),p=[],c=[];for(let h=29;h>=0;h--){const w=new Date;w.setDate(w.getDate()-h);const J=w.toISOString().split("T")[0];p.push(w.getDate());const B=this.state.sales.filter(F=>F.date===J).reduce((F,j)=>F+(Number(j.total||j.total_amount)||0),0);c.push(B)}const u=new Date,d=u.getMonth(),y=u.getFullYear(),m=d===0?11:d-1,E=d===0?y-1:y,$=this.state.sales.filter(h=>{const w=new Date(h.date);return w.getMonth()===d&&w.getFullYear()===y}).reduce((h,w)=>h+(Number(w.originalTotal||w.total_amount||w.total)||0),0)+(this.state.extraIncome||[]).filter(h=>{const w=new Date(h.date);return w.getMonth()===d&&w.getFullYear()===y}).reduce((h,w)=>h+(Number(w.amount)||0),0),b=this.state.sales.filter(h=>{const w=new Date(h.date);return w.getMonth()===m&&w.getFullYear()===E}).reduce((h,w)=>h+(Number(w.originalTotal||w.total_amount||w.total)||0),0)+(this.state.extraIncome||[]).filter(h=>{const w=new Date(h.date);return w.getMonth()===m&&w.getFullYear()===E}).reduce((h,w)=>h+(Number(w.amount)||0),0),v=b>0?($-b)/b*100:0,k=`${v>=0?"+":""}${v.toFixed(1)}% vs ${this.getMonthName(m)}`;let I=0,T=0,g=0,f=0,S=0,C=0,_=0,L=0;i.forEach(h=>{var we;const w=((we=h.channel)==null?void 0:we.toLowerCase())==="discogs",J=Number(h.originalTotal)||Number(h.total_amount)||Number(h.total)||0,B=Number(h.total)||Number(h.total_amount)||0,F=w?J-B:0,j=Number(h.shipping_cost)||0;I+=J,g+=j;let re=0;const be=h.items||[];be.length>0?be.forEach(se=>{const ue=Number(se.priceAtSale||se.unitPrice||se.price)||0,Ee=Number(se.qty||se.quantity)||1;let he=Number(se.costAtSale||se.cost)||0;const je=(se.owner||"").toLowerCase();let ke=se.providerOrigin||se.provider_origin;const Fe=ue*Ee;if(he===0||!ke){const ge=se.productId||se.recordId,Te=se.album,Ie=this.state.inventory.find(Ae=>ge&&(Ae.id===ge||Ae.sku===ge)||Te&&Ae.album===Te);Ie&&(he===0&&(he=Ie.cost||0),ke||(ke=Ie.provider_origin))}if(ke||(ke="Local_Used"),ke==="EU_B2B"||ke==="DK_B2B")S+=Fe*.2;else{const ge=Fe-he*Ee;C+=ge>0?ge*.2:0}if(je==="el cuartito"||je==="")he=Number(se.costAtSale||se.cost)||0;else{if(he===0||isNaN(he)){const ge=this.state.consignors?this.state.consignors.find(Ie=>(Ie.name||"").toLowerCase()===je):null,Te=ge&&(ge.agreementSplit||ge.split)||70;he=ue*(Number(Te)||70)/100}f+=he*Ee}re+=(ue-he)*Ee}):(re=J,S+=J*.2);const pe=parseFloat(h.shipping_income||h.shipping||h.shipping_cost||0);pe>0&&(_+=pe*.2,L+=pe),T+=re-F});const q=(this.state.extraIncome||[]).filter(h=>{const w=new Date(h.date);return w.getFullYear()===r&&o.includes(w.getMonth())});let P=0;q.forEach(h=>{const w=Number(h.amount)||0,J=Number(h.vatAmount)||0;P+=w,I+=w,T+=w,S+=J});const K=this.state.expenses.filter(h=>{var B;const w=h.fecha_factura?new Date(h.fecha_factura):(B=h.timestamp)!=null&&B.toDate?h.timestamp.toDate():new Date(h.timestamp||h.date);return(h.categoria_tipo==="operativo"||h.categoria_tipo==="stock_nuevo"||h.is_vat_deductible)&&w.getFullYear()===r&&o.includes(w.getMonth())}).reduce((h,w)=>h+(parseFloat(w.monto_iva)||0),0),W=(this.state.inventory||[]).filter(h=>{if(!h.item_phantom_vat||h.item_phantom_vat<=0||h.provider_origin!=="EU_B2B")return!1;const w=h.acquisition_date?new Date(h.acquisition_date):null;return w?w.getFullYear()===r&&o.includes(w.getMonth()):!1}).reduce((h,w)=>h+(w.item_phantom_vat||0),0),Z=(this.state.inventory||[]).filter(h=>{if(!h.item_real_vat||h.item_real_vat<=0||h.provider_origin!=="DK_B2B")return!1;const w=h.acquisition_date?new Date(h.acquisition_date):null;return w?w.getFullYear()===r&&o.includes(w.getMonth()):!1}).reduce((h,w)=>h+(w.item_real_vat||0),0),ae=S+C+_+W,x=K+W+Z,V=ae-x,oe=this.state.expenses.filter(h=>{const w=new Date(h.date||h.fecha_factura);return w.getFullYear()===r&&o.includes(w.getMonth())}).reduce((h,w)=>h+(Number(w.monto_total||w.amount)||0),0),te=T-V-oe,N=T-V,le=this.state.inventory.reduce((h,w)=>h+w.price*w.stock,0),ie=this.state.inventory.reduce((h,w)=>h+w.stock,0),X=this.state.inventory.filter(h=>h.stock>0&&h.stock<1),Y=this.state.sales.filter(h=>{var w;return h.fulfillment_status==="preparing"||h.status==="paid"||((w=h.channel)==null?void 0:w.toLowerCase())==="discogs"&&h.status!=="shipped"&&h.fulfillment_status!=="shipped"}),O=V,G=o.length===12?`Año ${r} `:`${o.map(h=>this.getMonthName(h)).join(", ")} ${r} `,z=this.state.dashboardAnalysisMode||"genre",Q={},H={};let U=0,me=0,fe=0;i.forEach(h=>{const w=h.items||[],J=(B,F)=>{const j=this.state.inventory.find(ue=>B&&(ue.id===B||ue.sku===B)||F&&ue.album===F);if(!j)return null;if(z==="storage")return j.storageLocation||null;const re=[j.genre,j.genre2,j.genre3,j.genre4,j.genre5].filter(Boolean),be=[];re.forEach(ue=>{be.push(...ue.split(",").map(Ee=>Ee.trim()).filter(Boolean))});const pe=[...new Set(be)],we=pe.filter(ue=>ue.toLowerCase()!=="electronic");return(we.length>0?we:pe.length>0?pe:["Otros"])[0]||null};if(w.length>0)w.forEach(B=>{const F=B.productId||B.recordId,j=J(F,B.album)||(z==="storage"?"Sin ubicación":h.genre||"Otros"),re=Number(B.qty||B.quantity)||1,be=Number(B.priceAtSale||B.unitPrice||B.price)||0;Q[j]=(Q[j]||0)+re,H[j]=(H[j]||0)+be*re,U+=re,(B.productCondition||B.condition||"Used")==="New"?me+=re:fe+=re});else{const B=Number(h.quantity)||1,F=Number(h.originalTotal||h.total_amount||h.total)||0,j=z==="storage"?"Sin ubicación":h.genre||"Otros";Q[j]=(Q[j]||0)+B,H[j]=(H[j]||0)+F,U+=B,fe+=B}});const ne=Object.entries(Q).sort((h,w)=>w[1]-h[1]),ye=Object.entries(H).sort((h,w)=>w[1]-h[1]),De=ye.length>0?{name:ye[0][0],revenue:ye[0][1]}:{name:"N/A",revenue:0},_e=i.length>0?I/i.length:0,de=U>0?Math.round(me/U*100):0,xe=U>0?Math.round(fe/U*100):0,ce=["#FF6B4A","#F59E0B","#14B8A6","#8B5CF6","#F43F5E","#0EA5E9","#84CC16","#D946EF","#64748B"],$e=z==="storage"?"Análisis por Ubicación":"Análisis por Género Musical",Me=z==="storage"?"ph-map-pin":"ph-music-notes-simple",Be=z==="storage"?"Ubicación Más Rentable":"Género Más Rentable",Le=`
            <div class="max-w-7xl mx-auto space-y-8 pb-24 md:pb-8 px-4 md:px-8 pt-6">
                    ...
                    <div class="relative group bg-white p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div class="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-max min-w-[250px] bg-brand-dark text-white text-xs rounded-xl p-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 shadow-xl pointer-events-none">
                            <div class="flex justify-between gap-4 mb-1">
                                ...
                                <span>${this.formatCurrency(T-P)}</span>
                            </div>
                            <div class="flex justify-between gap-4 mb-1">
                                <span class="text-slate-400">Ingresos Extra:</span>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <p class="text-3xl font-display font-bold text-brand-dark mb-1">${this.formatCurrency(_e)}</p>
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-left">
                                    <tbody class="divide-y divide-slate-50">
                                        ${l.map(h=>{const w=h.type==="sale",J=w?h.album||"Venta de Items":h.proveedor||h.description||"Gasto registrado",B=w?h.channel||"Tienda Local":h.categoria||"Operativo";let F="ph-receipt";if(w){const j=(h.channel||"").toLowerCase();j.includes("web")&&(F="ph-globe-simple"),j.includes("discogs")&&(F="ph-vinyl-record")}else F="ph-credit-card";return`
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                ${s.map(a=>`
                                    <div onclick="app.navigateInventoryFolder('genre', '${a}')" class="bg-white p-4 rounded-xl border border-slate-100 shadow-sm hover:shadow-md hover:border-brand-orange cursor-pointer transition-all group text-center">
                                        ...
                                        <p class="text-xs text-slate-500">${this.state.inventory.filter(i=>i.genre===a).length} items</p>
                                    </div>
                                `).join("")}
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                ${o.map(a=>`
                                    <div onclick="app.navigateInventoryFolder('owner', '${a}')" class="bg-white p-4 rounded-xl border border-slate-100 shadow-sm hover:shadow-md hover:border-brand-orange cursor-pointer transition-all group text-center">
                                        ...
                                        <h4 class="font-bold text-brand-dark text-sm truncate">${a}</h4>
                                        <p class="text-xs text-slate-500">${this.state.inventory.filter(i=>i.owner===a).length} items</p>
                                    </div>
                                `).join("")}
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                ${r.map(a=>`
                                    <div onclick="app.navigateInventoryFolder('storage', '${a.replace(/'/g,"\\'")}')" class="bg-white p-4 rounded-xl border border-slate-100 shadow-sm hover:shadow-md hover:border-brand-orange cursor-pointer transition-all group text-center">
                                        ...
                                        <h4 class="font-bold text-brand-dark text-sm truncate">${a}</h4>
                                        <p class="text-xs text-slate-500">${this.state.inventory.filter(i=>i.storageLocation===a).length} items</p>
                                    </div>
                                `).join("")}
                            </div>
                        </div>
                    ...
                    <div class="sticky top-0 bg-slate-50 z-20 pb-4 pt-4 -mx-4 px-4 md:mx-0 md:px-0">
                         <div class="flex justify-between items-center mb-5">
                            <div>
                                ...
                                <p class="text-xs text-slate-400 mt-1" id="inventory-subtitle">${n} discos registrados</p>
                            </div>
                             <div class="flex gap-2">
    ...
    `);const m=document.getElementById("inventory-kpi-container");if(m){const g=y?'<span class="ml-1.5 text-[9px] bg-orange-100 text-brand-orange px-1.5 py-0.5 rounded-full font-bold"><i class="ph-bold ph-funnel text-[8px]"></i> Filtrado</span>':"";m.innerHTML=`
                <div class="kpi-card">
                    ...
                    <p class="text-xl font-bold text-brand-dark font-display mt-1">${l}${y?` <span class="text-xs text-slate-400 font-normal">/ ${n}</span>`:""}</p>
                </div>
                <div class="kpi-card">
                ...
                <div class="kpi-card">
                    ...
                    <p class="text-xl font-bold text-emerald-600 font-display mt-1">${c} <span class="text-xs text-slate-400 font-normal">/ ${l}</span></p>
                </div>
                <div class="kpi-card">
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">En Discogs ${g}</p>
                    <p class="text-xl font-bold text-purple-600 font-display mt-1">${u} <span class="text-xs text-slate-400 font-normal">/ ${l}</span></p>
                </div>
            ...
            `);const v=document.getElementById("inventory-stats-section");if(v)if(this.state.showStats){const g={};a.forEach(P=>{const A=[P.genre,P.genre2,P.genre3,P.genre4,P.genre5].filter(Boolean),K=[];A.forEach(x=>{K.push(...x.split(",").map(M=>M.trim()).filter(Boolean))});const W=[...new Set(K)],Z=W.filter(x=>x.toLowerCase()!=="electronic");(Z.length>0?Z:W.length>0?W:["Otros"]).forEach(x=>{g[x]=(g[x]||0)+1})});const f=Object.entries(g).sort((P,A)=>A[1]-P[1]).slice(0,10),S=f.length>0?f[0][1]:1,C=["#F05A28","#e04d1c","#f97316","#fb923c","#fdba74","#8b5cf6","#a78bfa","#3b82f6","#60a5fa","#22c55e"],_=a.reduce((P,A)=>{const K=Number(A.stock)||0;return P+(K>0?(parseFloat(A.price)||0)*K:0)},0),L=a.reduce((P,A)=>{const K=Number(A.stock)||0;return P+(K<=0&&parseFloat(A.price)||0)},0),q=Math.max(_,L,1);v.innerHTML=`
                                ...
                                ${f.map(([P,A],K)=>`
                                    <div>
                                        ...
                                        <div class="stat-bar-track">
                                            <div class="stat-bar-fill" style="width: ${Math.max(A/S*100,8)}%; background: ${C[K%C.length]};"></div>
                                        </div>
                                    </div>
                                `).join("")}
                            ...
                            <div class="space-y-4">
                                <div>
                                    <div class="flex justify-between items-center mb-1.5">
                                        ...
                                        <span class="text-sm font-bold text-brand-dark font-display">${this.formatCurrency(_)}</span>
                                    </div>
                                    <div class="stat-bar-track">
                                        <div class="stat-bar-fill" style="width: ${Math.max(_/q*100,5)}%; background: linear-gradient(90deg, #22c55e, #4ade80);"></div>
                                    </div>
                                </div>
                                <div>
                                    <div class="flex justify-between items-center mb-1.5">
                    ...
                    <div class="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                        <div class="flex bg-slate-50 p-1 rounded-xl border border-slate-200">
                            <select id="sales-year" onchange="app.updateFilter('year', this.value)" class="bg-transparent px-3 py-1.5 text-sm font-bold text-slate-600 outline-none cursor-pointer">
                                <option value="2026" ${i===2026?"selected":""}>2026</option>
                                <option value="2025" ${i===2025?"selected":""}>2025</option>
                            </select>
                        </div>
                        <div class="flex flex-wrap gap-1">
                            ${["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"].map((b,v)=>`
                                <button onclick="app.toggleMonthFilter(${v})" 
                                    class="px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all ${n.includes(v)?"bg-brand-dark text-white":"bg-slate-50 text-slate-400 hover:bg-slate-100"}">
                                    ${b}
                                </button>
                            `).join("")}
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-3 max-h-[80vh] overflow-y-auto pr-2 custom-scrollbar pb-10">
                            ${d.map(b=>{const v=b.status==="shipped",k=b.status==="paid"||b.source==="STORE"||b.paymentMethod!=="Pending",I=b.channel==="Discogs",T=b.source==="STORE",g=b.items&&b.items.length>0?b.items[0]:{album:b.album||"Venta Manual",artist:b.artist||"Desconocido"},f=b.items&&b.items.length>1?b.items.length-1:0;return`
                                <div class="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm hover:border-slate-200 transition-all cursor-pointer group flex items-center gap-4 relative" onclick="app.openUnifiedOrderDetailModal('${b.id}')">
                                    <!-- Source Icon -->
                                    <div class="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${I?"bg-slate-900 text-white":T?"bg-orange-100 text-brand-orange":"bg-blue-100 text-blue-600"}">
                                        <i class="ph-bold ${I?"ph-disc":T?"ph-storefront":"ph-globe"} text-xl"></i>
                                    </div>
    ...
    `).join(""),e.classList.remove("hidden")):e.classList.add("hidden")},selectSku(t){const e=this.state.inventory.find(s=>s.id===t||s.sku===t);e&&(this.state.posSelectedItemSku=e.sku,this.renderSales(document.getElementById("app-content")),setTimeout(()=>{const s=document.getElementById("input-price"),o=document.getElementById("input-sku"),r=document.getElementById("input-cost"),a=document.getElementById("input-artist"),i=document.getElementById("input-album"),n=document.getElementById("input-genre"),l=document.getElementById("input-owner"),p=document.getElementById("sku-search");s&&(s.value=e.price),o&&(o.value=e.sku),r&&(r.value=e.cost||0),a&&(a.value=e.artist),i&&(i.value=e.album),n&&(n.value=e.genre),l&&(l.value=e.owner),p&&(p.value=`${e.artist} - ${e.album}`,this.state.manualSaleSearch=p.value);const c=document.getElementById("sku-results");c&&c.classList.add("hidden")},50),e.stock<=0&&this.showToast("⚠️ Este producto no tiene stock disponible","warning"))},updateTotal(){const t=parseFloat(document.getElementById("input-price").value)||0,e=parseInt(document.getElementById("input-qty").value)||1,s=t*e;document.getElementById("form-total").innerText=this.formatCurrency(s)},openAddVinylModal(t=null){let e={sku:"",artist:"",album:"",genre:"Minimal",condition:"NM",provider_origin:"EU_B2B",acquisition_date:"",item_phantom_vat:0,item_real_vat:0,price:"",cost:"",stock:1,owner:"El Cuartito"},s=!1;if(t){const a=this.state.inventory.find(i=>i.id===t||i.sku===t);a&&(e=a,s=!0)}if(!s){const a=this.state.inventory.map(n=>{const l=n.sku.match(/^SKU\s*-\s*(\d+)/);return l?parseInt(l[1]):0}),i=Math.max(0,...a);e.sku=`SKU-${String(i+1).padStart(3,"0")}`}const o=["Minimal","Techno","House","Deep House","Electro"];[...new Set([...o,...this.state.customGenres||[]])];const r=`
    <div id="modal-overlay" class="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
                ...
                <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 shrink-0">
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            <span class="px-2 py-0.5 rounded-full ${i(e.status).color} text-[9px] font-bold uppercase">${i(e.status).label}</span>
                        </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
                                    <table class="w-full text-sm">
                                        ...
                                        <tbody class="divide-y divide-slate-50">
                                            ${(e.items||[]).map(b=>{var v,k,I,T,g;return`
                                                <tr>
                                                    <td class="px-4 py-4">
                                                        <div class="flex items-center gap-3">
                                                    ...
                                                    <td class="px-4 py-4 text-center font-mono text-xs text-slate-400">${b.sku||((T=b.record)==null?void 0:T.sku)||"-"}</td>
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="relative pl-4 border-l-2 border-slate-100 space-y-6">
                                    ${a.map((b,v)=>{const k=i(b.status);return`
                                            <div class="relative">
        ...
        `;document.body.insertAdjacentHTML("beforeend",d)},openInvoiceModal(t){var v;const e=this.state.sales.find(k=>k.id===t);if(!e){this.showToast("Sale not found","error");return}const s=e.items||[],o=(v=e.date)!=null&&v.toDate?e.date.toDate():new Date(e.date||e.timestamp),r=o.toISOString().slice(0,10).replace(/-/g,""),a=e.invoiceNumber||`ECR-${r}-${t.slice(-4).toUpperCase()}`,i=s.filter(k=>k.productCondition==="New"),n=s.filter(k=>k.productCondition!=="New");let l=0,p=0;const c=(k,I)=>k.map(T=>{const g=T.priceAtSale||T.price||0,f=T.qty||T.quantity||1,S=g*f;if(p+=S,I){const C=S*.2;return l+=C,`
                        <tr>
                            <td style="padding: 12px 0; border-bottom: 1px solid #eee;">
                                <div style="font-weight: bold;">${T.album||"Product"}</div>
                                <div style="font-size: 11px; color: #666;">${T.artist||""}</div>
                                <div style="font-size: 11px; color: #2563eb; margin-top: 4px;">✓ Moms (25%): DKK ${C.toFixed(2)}</div>
                            </td>
                    ...
                    `}).join("");let u="";i.length>0&&n.length>0?u=`
                ...
                ${c(n,!1)}
            `:u=c(i,!0)+c(n,!1);const d=parseFloat(e.shipping_income||e.shipping||e.shipping_cost||0),y=d*.2,m=p+d,E=e.customer?`${e.customer.firstName||""} ${e.customer.lastName||""}`.trim():e.customerName||"Customer",$=e.customer?`${e.customer.address||""}<br>${e.customer.postalCode||""} ${e.customer.city||""}<br>${e.customer.country||""}`:"",b=`
        ...
        `),e.document.close()},navigateInventoryFolder(t,e){t==="genre"&&(this.state.filterGenre=e),t==="owner"&&(this.state.filterOwner=e),t==="label"&&(this.state.filterLabel=e),t==="storage"&&(this.state.filterStorage=e),this.refreshCurrentView()},toggleSelection(t){this.state.selectedItems.has(t)?this.state.selectedItems.delete(t):this.state.selectedItems.add(t),this.refreshCurrentView()},async openPrintLabelModal(t){const e=this.state.inventory.find(i=>i.id===t||i.sku===t);if(!e)return;let s={...e};try{const i=await D.collection("products").doc(e.id).get();i.exists&&(s={...e,...i.data()})}catch(i){console.warn("[printLabel] Could not fetch fresh product data, using state copy",i)}const o=s.year&&Number(s.year)!==0?String(s.year):"—",r=s.price?Number(s.price).toLocaleString("da-DK"):"—",a=`
...
`;document.body.insertAdjacentHTML("beforeend",a),setTimeout(()=>{const i=document.getElementById("qr-container");i&&typeof QRCode<"u"&&(i.innerHTML="",new QRCode(i,{text:s.sku,width:57,height:57,colorDark:"#000000",colorLight:"#ffffff",correctLevel:QRCode.CorrectLevel.M}))},50)},closePrintLabelModal(){const t=document.getElementById("print-label-modal");if(t){const e=document.getElementById("label-comment");e&&(e.value=""),t.remove()}},setLabelOrientation(t){const e=document.getElementById("print-label-modal");if(!e)return;e.dataset.orientation=t;const s=document.getElementById("orient-h"),o=document.getElementById("orient-v"),r="flex-1 py-2 bg-white text-brand-dark font-bold rounded-lg text-sm shadow-sm flex items-center justify-center gap-1.5 transition-all",a="flex-1 py-2 text-slate-500 font-bold rounded-lg text-sm flex items-center justify-center gap-1.5 transition-all";s&&(s.className=t==="landscape"?r:a),o&&(o.className=t==="portrait"?r:a);const i=document.getElementById("printable-label");i&&i.classList.toggle("label-b--portrait",t==="portrait")},async confirmPrintLabel(){const t=document.getElementById("label-comment"),e=document.getElementById("preview-comment"),s=t?t.value:"";t&&e&&(e.innerText=s);const o=document.querySelector('#print-label-modal button[onclick="app.confirmPrintLabel()"]');o&&(o.disabled=!0,o.innerHTML='<i class="ph-bold ph-circle-notch animate-spin"></i> Imprimiendo…');try{const r=document.getElementById("print-label-modal"),a=r&&r.dataset.orientation||"landscape",n=(await this._drawLabelCanvas(s,a)).toDataURL("image/png").split(",")[1],l=await fetch(`${R}/api/print-label`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({image:n})}),p=await l.json();if(!l.ok)throw new Error(p.error||"Error desconocido");o&&(o.innerHTML='<i class="ph-bold ph-check"></i> ¡Enviado!',o.classList.replace("bg-brand-dark","bg-green-600")),this.showToast("✅ Etiqueta enviada a la impresora"),setTimeout(()=>this.closePrintLabelModal(),1500)}catch(r){o&&(o.disabled=!1,o.innerHTML='<i class="ph-bold ph-printer"></i> Imprimir'),this.showToast("Error al imprimir: "+r.message,"error")}},async _drawLabelCanvas(t,e="landscape"){const s=document.getElementById("print-label-modal"),o=s?s.dataset.sku:null,r=o?this.state.inventory.find($=>$.id===o||$.sku===o)||{}:{},a=($,b)=>{const v=document.getElementById($);return v&&v.value.trim()?v.value.trim():b},i={...r,album:a("label-edit-title",r.album),artist:a("label-edit-artist",r.artist),year:a("label-edit-year",r.year),price:a("label-edit-price",r.price),genre:a("label-edit-genre1",r.genre),genre2:a("label-edit-genre2",r.genre2),condition:a("label-edit-cond",r.condition),storageLocation:a("label-edit-loc",r.storageLocation)},n=300/25.4,l=e==="portrait",p=Math.round((l?40:62)*n),c=Math.round((l?62:40)*n),u=document.createElement("canvas");u.width=p,u.height=c;const d=u.getContext("2d");"wordSpacing"in d&&(d.wordSpacing="0px"),"letterSpacing"in d&&(d.letterSpacing="0px"),d.fillStyle="#ffffff",d.fillRect(0,0,p,c);const y=Math.round(5.5*n);d.fillStyle="#000000",d.fillRect(0,0,p,y);const m=Math.round(2.2*n);d.fillStyle="#ffffff",d.font=`800 ${m}px "DM Sans", Arial, sans-serif`,d.textBaseline="middle",d.textAlign="left";const E=((i.genre||"VINYL")+(i.genre2?" / "+i.genre2:"")).toUpperCase();d.fillText(E,Math.round(1.6*n),y/2);try{const $=await new Promise(b=>{const v=new Image;v.crossOrigin="anonymous",v.onload=()=>b(v),v.onerror=()=>b(null),v.src="logo-broadsheet.png"});if($){const b=Math.round(3*n),v=Math.round($.naturalWidth*(b/$.naturalHeight)),k=document.createElement("canvas");k.width=$.naturalWidth,k.height=$.naturalHeight;const I=k.getContext("2d");I.drawImage($,0,0);const T=I.getImageData(0,0,k.width,k.height),g=T.data;for(let L=0;L<g.length;L+=4)g[L+3]>0&&(g[L]=255,g[L+1]=255,g[L+2]=255);I.putImageData(T,0,0);const S=Math.round(18*n),C=p-S+Math.round((S-v)/2),_=(y-b)/2;d.drawImage(k,C,_,v,b)}}catch{}if(l){const $=Math.round(1.6*n),b=p-$*2;d.textAlign="left",d.textBaseline="top";const v=Math.round(3.8*n);d.font=`800 ${v}px "DM Sans", Arial, sans-serif`,d.fillStyle="#000000";const k=this._wrapText(d,i.album||"",b,2);let I=y+Math.round(1.1*n);k.forEach((Q,H)=>{d.fillText(Q,$,I+H*Math.round(v*1.1))}),I+=k.length*Math.round(v*1.1);const T=Math.round(2.4*n);if(d.font=`600 ${T}px "DM Sans", Arial, sans-serif`,d.fillStyle="rgba(0,0,0,0.5)",I+=Math.round(.5*n),d.fillText(this._truncateText(d,i.artist||"",b),$,I),I+=T,i.label&&i.label!=="Desconocido"){const Q=Math.round(2.2*n);I+=Math.round(.6*n),d.font=`700 ${Math.round(1.9*n)}px "DM Sans", Arial, sans-serif`,d.fillStyle="rgba(0,0,0,0.35)",d.fillText("LABEL",$,I);const H=d.measureText("LABEL ").width;d.font=`600 ${Q}px "DM Sans", Arial, sans-serif`,d.fillStyle="#333333",d.fillText(this._truncateText(d,i.label,b-H),$+H,I),I+=Q}if(t){const Q=Math.round(2.1*n);d.font=`italic 600 ${Q}px "DM Sans", Arial, sans-serif`,d.fillStyle="rgba(0,0,0,0.4)",d.textBaseline="top";const H=b-Math.round(3*n),U=this._wrapText(d,t,H,5),me=Math.round(Q*1.35),fe=I+Math.round(1.8*n);U.forEach((ne,ye)=>{d.fillText(ne,$+Math.round(1.5*n),fe+ye*me)})}const g=Math.round(12*n),f=Math.round(20*n),S=Math.round((p-f)/2),C=c-g-Math.round(1.6*n),_=C-Math.round(4.5*n),L=_-Math.round(2.2*n);d.strokeStyle="rgba(0,0,0,0.15)",d.lineWidth=1,d.beginPath(),d.moveTo($,_),d.lineTo(p-$,_),d.stroke();const q=Math.round(2*n),P=Math.round(1.8*n),A=b/3;d.textBaseline="middle";const K=(Q,H,U)=>{d.textAlign="left",d.font=`700 ${P}px "DM Sans", Arial, sans-serif`,d.fillStyle="rgba(0,0,0,0.45)",d.fillText(Q+" ",U,L);const me=d.measureText(Q+" ").width;d.font=`700 ${q}px "DM Mono", "Courier New", monospace`,d.fillStyle="rgba(0,0,0,0.7)",d.fillText(H,U+me,L)};K("Loc",i.storageLocation||"—",$),K("Cond",i.condition||"—",$+A),K("Year",i.year&&Number(i.year)!==0?String(i.year):"—",$+A*2);const W=Math.round(13*n),Z=Math.round((p-W)/2),ae=Math.round(1.9*n),x=I+Math.round(2.5*n),M=L-Math.round(ae+Math.round(.4*n)+4),V=x+Math.max(0,Math.round((M-x-W)/2)),oe=document.getElementById("qr-container"),te=oe?oe.querySelector("canvas"):null;te?d.drawImage(te,Z,V,W,W):(d.strokeStyle="#ccc",d.lineWidth=1,d.strokeRect(Z,V,W,W)),d.font=`600 ${ae}px "DM Mono", "Courier New", monospace`,d.fillStyle="rgba(0,0,0,0.45)",d.textAlign="center",d.textBaseline="top",d.fillText(i.sku||"",p/2,V+W+Math.round(.4*n)),d.fillStyle="#000000";const N=Math.round(.8*n);d.beginPath(),d.moveTo(S+N,C),d.lineTo(S+f-N,C),d.quadraticCurveTo(S+f,C,S+f,C+N),d.lineTo(S+f,C+g-N),d.quadraticCurveTo(S+f,C+g,S+f-N,C+g),d.lineTo(S+N,C+g),d.quadraticCurveTo(S,C+g,S,C+g-N),d.lineTo(S,C+N),d.quadraticCurveTo(S,C,S+N,C),d.closePath(),d.fill();const le=i.price?Number(i.price).toLocaleString("da-DK"):"—",ie=Math.round(4.8*n),X=Math.round(2.2*n),Y=ie*1.1,O=X*1.1,G=C+(g-Y-O)/2+Y/2,z=G+Y/2+O/2;d.textAlign="center",d.textBaseline="middle",d.font=`800 ${ie}px "DM Mono", "Courier New", monospace`,d.fillStyle="#ffffff",d.fillText(le,S+f/2,G),d.font=`700 ${X}px "DM Sans", Arial, sans-serif`,d.fillStyle="#ffffff",d.fillText("DKK",S+f/2,z)}else{const $=y,b=Math.round(18*n),v=p-b,k=Math.round(1.6*n),I=Math.round(1.1*n);d.textAlign="left",d.textBaseline="top";const T=Math.round(3.8*n);d.font=`800 ${T}px "DM Sans", Arial, sans-serif`,d.fillStyle="#000000";const g=v-k-Math.round(.95*n),f=this._wrapText(d,i.album||"",g,2);f.forEach((de,xe)=>{d.fillText(de,k,$+I+xe*Math.round(T*1.1))});const S=Math.round(2.4*n);d.font=`600 ${S}px "DM Sans", Arial, sans-serif`,d.fillStyle="rgba(0,0,0,0.5)";const C=$+I+f.length*Math.round(T*1.1)+Math.round(.5*n),_=this._truncateText(d,i.artist||"",g);d.fillText(_,k,C);let L=C+S;if(i.label&&i.label!=="Desconocido"){const de=Math.round(2.2*n),xe=L+Math.round(.6*n);d.font=`700 ${Math.round(1.9*n)}px "DM Sans", Arial, sans-serif`,d.fillStyle="rgba(0,0,0,0.35)",d.fillText("LABEL",k,xe),d.font=`600 ${de}px "DM Sans", Arial, sans-serif`,d.fillStyle="#333333";const ce=k+d.measureText("LABEL ").width;d.fillText(this._truncateText(d,i.label,g-ce+k),ce,xe),L=xe+de}const q=c-Math.round(5*n);if(t){const de=Math.round(2.1*n);d.font=`italic 600 ${de}px "DM Sans", Arial, sans-serif`,d.fillStyle="rgba(0,0,0,0.4)",d.textBaseline="top";const xe=g-Math.round(2*n),ce=this._wrapText(d,t,xe,5),$e=Math.round(de*1.35),Me=ce.length*$e,Be=L+(q-L-Me)/2;ce.forEach((Le,Ce)=>{d.fillText(Le,k+Math.round(1.5*n),Be+Ce*$e)})}const P=Math.round(2*n),A=Math.round(1.8*n),K=c-Math.round(2.5*n),Z=(v-k-Math.round(.95*n))/3;d.strokeStyle="rgba(0,0,0,0.15)",d.lineWidth=1,d.beginPath(),d.moveTo(k,q),d.lineTo(v-Math.round(.95*n),q),d.stroke(),d.textBaseline="middle",d.textAlign="left";const ae=(de,xe,ce)=>{d.font=`700 ${A}px "DM Sans", Arial, sans-serif`,d.fillStyle="rgba(0,0,0,0.45)",d.fillText(de+" ",ce,K);const $e=d.measureText(de+" ").width;d.font=`700 ${P}px "DM Mono", "Courier New", monospace`,d.fillStyle="rgba(0,0,0,0.7)",d.fillText(xe,ce+$e,K)};ae("Loc",i.storageLocation||"—",k),ae("Cond",i.condition||"—",k+Z),ae("Year",i.year&&Number(i.year)!==0?String(i.year):"—",k+Z*2);const x=v;d.strokeStyle="rgba(0,0,0,0.12)",d.lineWidth=1,d.beginPath(),d.moveTo(x,$),d.lineTo(x,c),d.stroke();const M=document.getElementById("qr-container"),V=M?M.querySelector("canvas"):null,te=Math.round(15*n),N=x+Math.round((b-te)/2),le=$+Math.round(1.1*n);V?d.drawImage(V,N,le,te,te):(d.strokeStyle="#ccc",d.strokeRect(N,le,te,te));const ie=Math.round(1.9*n);d.font=`600 ${ie}px "DM Mono", "Courier New", monospace`,d.fillStyle="rgba(0,0,0,0.45)",d.textAlign="center",d.textBaseline="top",d.fillText(i.sku||"",x+b/2,le+te+Math.round(.4*n));const X=Math.round(12*n),Y=Math.round(15*n),O=x+Math.round((b-Y)/2),G=c-X-Math.round(1.6*n);d.fillStyle="#000000";const z=Math.round(.8*n);d.beginPath(),d.moveTo(O+z,G),d.lineTo(O+Y-z,G),d.quadraticCurveTo(O+Y,G,O+Y,G+z),d.lineTo(O+Y,G+X-z),d.quadraticCurveTo(O+Y,G+X,O+Y-z,G+X),d.lineTo(O+z,G+X),d.quadraticCurveTo(O,G+X,O,G+X-z),d.lineTo(O,G+z),d.quadraticCurveTo(O,G,O+z,G),d.closePath(),d.fill();const Q=i.price?Number(i.price).toLocaleString("da-DK"):"—",H=Math.round(4.8*n),U=Math.round(2*n),me=H*1.1,fe=U*1.1,ne=me+fe,ye=G+(X-ne)/2+me/2,De=ye+me/2+fe/2;d.textAlign="center",d.textBaseline="middle",d.font=`800 ${H}px "DM Mono", "Courier New", monospace`,d.fillStyle="#ffffff",d.fillText(Q,O+Y/2,ye);const _e=Math.round(2.2*n);d.font=`700 ${_e}px "DM Sans", Arial, sans-serif`,d.fillStyle="#ffffff",d.fillText("DKK",O+Y/2,De)}return u},_wrapText(t,e,s,o){const r=e.split(" "),a=[];let i="";for(const n of r){const l=i?i+" "+n:n;if(t.measureText(l).width>s&&i){if(a.push(i),i=n,a.length>=o)break}else i=l}if(i&&a.length<o&&a.push(i),a.length>0){const n=a[a.length-1];a[a.length-1]=this._truncateText(t,n,s)}return a},_truncateText(t,e,s){if(t.measureText(e).width<=s)return e;let o=e;for(;o.length>1&&t.measureText(o+"…").width>s;)o=o.slice(0,-1);return o+"…"},initFuse(){if(typeof Fuse>"u"){console.warn("Fuse.js not loaded yet");return}const t={keys:[{name:"artist",weight:.35},{name:"album",weight:.25},{name:"label",weight:.15},{name:"storageLocation",weight:.15},{name:"sku",weight:.1},{name:"quickId",weight:.1},{name:"genre",weight:.03},{name:"notes",weight:.02}],threshold:.4,distance:100,ignoreLocation:!0,minMatchCharLength:2};this.fuse=new Fuse(this.state.inventory,t)},getFilteredInventory(){const t=(this.state.inventorySearch||"").trim().toLowerCase(),e=this.state.filterGenre||"all",s=this.state.filterOwner||"all",o=this.state.filterLabel||"all",r=this.state.filterStorage||"all",a=this.state.filterDiscogs||"all",i=this.state.filterHero||"all",n=this.state.filterStock||"all",l=this.state.filterCondition||"all";let p=this.state.inventory;if(t.length>=2)if(this.fuse)p=this.fuse.search(t).map(c=>c.item);else{const c=t.split(" ").filter(u=>u.length>0);p=p.filter(u=>c.every(d=>(u.artist||"").toLowerCase().includes(d)||(u.album||"").toLowerCase().includes(d)||(u.label||"").toLowerCase().includes(d)||(u.storageLocation||"").toLowerCase().includes(d)||(u.genre||"").toLowerCase().includes(d)||(u.notes||"").toLowerCase().includes(d)||(u.sku||"").toLowerCase().includes(d)))}return p.filter(c=>{const u=[c.genre,c.genre2,c.genre3,c.genre4,c.genre5].filter(Boolean),d=[];u.forEach(A=>{d.push(...A.split(",").map(K=>K.trim()).filter(Boolean))});const y=[...new Set(d)],m=y.filter(A=>A.toLowerCase()!=="electronic"),E=m.length>0?m:y.length>0?y:["Otros"],$=e==="all"||E.includes(e),b=s==="all"||c.owner===s,v=o==="all"||c.label===o,k=r==="all"||c.storageLocation===r,I=!!c.discogs_listing_id,T=a==="all"||a==="yes"&&I||a==="no"&&!I,g=c.tags&&(Array.isArray(c.tags),c.tags.includes("hero")),f=i==="all"||i==="yes"&&g||i==="no"&&!g,S=this.getTimeInStockCategory(c.created_at||null),C=this.state.filterStockTime.length===0||this.state.filterStockTime.includes(S),_=Number(c.stock)||0,L=n==="all"||n==="inStock"&&_>0||n==="outOfStock"&&_<=0,q=c.product_condition||"Second-hand";return $&&b&&v&&k&&T&&f&&C&&L&&(l==="all"||l==="used"&&q==="Second-hand"||l==="new"&&q!=="Second-hand")})},toggleSelectAll(){const t=this.getFilteredInventory();t.length>0&&t.every(e=>this.state.selectedItems.has(e.sku))?t.forEach(e=>this.state.selectedItems.delete(e.sku)):t.forEach(e=>this.state.selectedItems.add(e.sku)),this.refreshCurrentView()},addSelectionToCart(){this.state.selectedItems.forEach(t=>{const e=this.state.inventory.find(s=>s.id===t||s.sku===t);e&&e.stock>0&&(this.state.cart.find(s=>s.sku===t)||this.state.cart.push(e))}),this.state.selectedItems.clear(),this.showToast(`${this.state.cart.length} items agregados al carrito`),this.refreshCurrentView()},deleteSelection(){if(!confirm(`¿Estás seguro de eliminar ${this.state.selectedItems.size} productos ? `))return;const t=D.batch(),e=[];this.state.selectedItems.forEach(s=>{const o=D.collection("products").doc(s),r=this.state.inventory.find(a=>a.id===s||a.sku===s);r&&e.push(r),t.delete(o)}),t.commit().then(()=>{this.showToast("Productos eliminados"),e.forEach(s=>this.logInventoryMovement("DELETE",s)),this.state.selectedItems.clear()}).catch(s=>{console.error("Error logging movement:",s),alert("Error al eliminar")})},openAddExpenseModal(){const t=["Alquiler","Servicios","Marketing","Suministros","Honorarios"],s=`
    <div id="modal-overlay" class="fixed inset-0 bg-brand-dark/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" >
    ...
    `;document.body.insertAdjacentHTML("beforeend",s)},async handleAddVinyl(t,e){t.preventDefault();const s=new FormData(t.target);let o=s.get("genre"),r=s.get("collection");r==="other"&&(r=s.get("custom_collection"));const a=s.get("sku"),i=s.get("is_online")==="on",n=s.get("publish_discogs")==="on",l=s.get("publish_local")==="on",p={sku:a,artist:s.get("artist"),album:s.get("album"),genre:o,genre2:s.get("genre2")||null,genre3:s.get("genre3")||null,genre4:s.get("genre4")||null,genre5:s.get("genre5")||null,label:s.get("label"),collection:r||null,collectionNote:s.get("collectionNote")||null,year:s.get("year")?parseInt(s.get("year")):null,condition:s.get("condition"),provider_origin:s.get("provider_origin")||"Local_Used",sleeveCondition:s.get("sleeveCondition")||"",comments:s.get("comments")||"",price:parseFloat(s.get("price")),cost:parseFloat(s.get("cost"))||0,stock:parseInt(s.get("stock")),storageLocation:s.get("storageLocation"),owner:s.get("owner"),is_online:i,publish_webshop:i,publish_discogs:n,publish_local:l,cover_image:s.get("cover_image")||null,updated_at:firebase.firestore.FieldValue.serverTimestamp(),tags:[s.get("tag_hero")?"hero":null,s.get("tag_new")?"new_arrival":null,s.get("collection_tag")?s.get("collection_tag").trim():null].filter(Boolean),is_rsd_discount:s.get("is_rsd_discount")==="on",discogsUrl:s.get("discogsUrl"),discogsId:s.get("discogsId"),discogs_release_id:s.get("discogs_release_id")||s.get("discogsId"),tracks:(()=>{try{return JSON.parse(s.get("tracks")||"[]")}catch{return[]}})()};p.provider_origin==="EU_B2B"?(p.item_phantom_vat=Math.round(p.cost*.25*100)/100,p.item_real_vat=0,p.acquisition_date=s.get("acquisition_date")||new Date().toISOString().split("T")[0]):p.provider_origin==="DK_B2B"?(p.item_phantom_vat=0,p.item_real_vat=Math.round(p.cost*.25*100)/100,p.acquisition_date=s.get("acquisition_date")||new Date().toISOString().split("T")[0]):(p.item_phantom_vat=0,p.item_real_vat=0,p.acquisition_date=null),console.log(`[handleAddVinyl] editSku: ${e}, recordData:`,p);try{let c=null,u=null;if(e){const d=await this.findProductBySku(e);if(!d){this.showToast("❌ Producto no encontrado","error");return}u=d.data,c=d.id,await d.ref.update(p),this.showToast("✅ Disco actualizado")}else{const d=this.state.inventory.map(m=>{const E=m.sku&&typeof m.sku=="string"?m.sku.match(/^SKU\s*-\s*(\d+)/):null;return E?parseInt(E[1]):0}),y=Math.max(0,...d);p.created_at=firebase.firestore.FieldValue.serverTimestamp(),c=await D.runTransaction(async m=>{const E=D.collection("metadata").doc("vinylCounter"),$=await m.get(E);let b=0;$.exists&&(b=$.data().current||0);const v=Math.max(b,y)+1,k=String(v).padStart(4,"0");m.set(E,{current:v},{merge:!0}),p.quickId=k,p.sku=`SKU-${String(v).padStart(3,"0")}`;const I=D.collection("products").doc();return m.set(I,p),I.id}),this.showToast(`✅ Disco agregado (ID: ${p.quickId})`)}if(n){const d=s.get("discogs_release_id")||s.get("discogsId");if(u&&u.discogs_listing_id)try{const m=await(await fetch(`${R}/discogs/update-listing/${u.discogs_listing_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({product:p})})).json();if(m.success)this.showToast("💿 Listing de Discogs actualizado");else throw new Error(m.error||"Error desconocido")}catch(y){console.error("Error updating Discogs listing:",y),this.showToast(`⚠️ Error Discogs: ${y.message}`,"error")}else if(d)try{const m=await(await fetch(`${R}/discogs/create-listing`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({releaseId:parseInt(d),product:p})})).json();if(m.success&&m.listingId)await D.collection("products").doc(c).update({discogs_listing_id:String(m.listingId),discogs_release_id:parseInt(d)}),this.showToast("💿 Publicado en Discogs correctamente");else throw new Error(m.error||"Error desconocido")}catch(y){console.error("Error creating Discogs listing:",y);let m=y.message;(m.toLowerCase().includes("mp3")||m.toLowerCase().includes("digital")||m.toLowerCase().includes("format"))&&(m="Discogs solo permite formatos físicos (Vinyl, CD, Cassette). Este release es digital o MP3."),this.showToast(`⚠️ Error Discogs: ${m}`,"error")}else this.showToast("⚠️ Necesitas buscar el disco en Discogs primero para publicarlo","warning")}document.getElementById("modal-overlay").remove(),this.loadData()}catch(c){console.error(c),this.showToast("❌ Error: "+(c.message||"desconocido"),"error")}},async toggleProductTag(t,e){try{const s=this.state.inventory.find(i=>i.id===t||i.sku===t);if(!s){this.showToast("❌ Producto no encontrado","error");return}let o=s.tags||[];o.includes(e)?o=o.filter(i=>i!==e):o.push(e);const r=D.collection("products").doc(s.id);if(!(await r.get()).exists){this.showToast("❌ Error: Documento no encontrado","error");return}await r.update({tags:o,updated_at:firebase.firestore.FieldValue.serverTimestamp()}),this.showToast(`✅ ${e==="hero"?"Héroe":"Novedad"} actualizado`),s.tags=o,this.refreshCurrentView()}catch(s){console.error("Error toggling product tag:",s),this.showToast("❌ Error al actualizar tag","error")}},async toggleRsdDiscount(t){try{const e=this.state.inventory.find(a=>a.id===t||a.sku===t);if(!e){this.showToast("❌ Producto no encontrado","error");return}const s=!e.is_rsd_discount,o=D.collection("products").doc(e.id);if(!(await o.get()).exists){this.showToast("❌ Error: Documento no encontrado","error");return}await o.update({is_rsd_discount:s,updated_at:firebase.firestore.FieldValue.serverTimestamp()}),this.showToast(`✅ RSD ${s?"activado":"desactivado"} — ${e.album}`),e.is_rsd_discount=s,this.refreshCurrentView()}catch(e){console.error("Error toggling RSD discount:",e),this.showToast("❌ Error al actualizar RSD","error")}},deleteVinyl(t){const e=this.state.inventory.find(o=>o.id===t||o.sku===t);if(!e){alert("Error: Item not found");return}const s=`
                                                    ...
                                                    `;document.body.insertAdjacentHTML("beforeend",s)},async confirmDelete(t){const e=document.getElementById("delete-confirm-modal");e&&e.remove();const s=document.getElementById("modal-overlay");s&&s.remove();try{const o=this.state.inventory.find(n=>n.id===t||n.sku===t),r=o?o.id:t,a=await D.collection("products").doc(r).get();if(!a.exists){this.showToast("❌ Producto no encontrado","error");return}const i={id:a.id,ref:a.ref,data:a.data()};if(console.log("Product to delete:",i.data),console.log("Has discogs_listing_id?",i.data.discogs_listing_id),i.data.discogs_listing_id){console.log("Attempting to delete from Discogs:",i.data.discogs_listing_id);try{const n=await fetch(`${R}/discogs/delete-listing/${i.data.discogs_listing_id}`,{method:"DELETE"});console.log("Discogs delete response status:",n.status);const l=await n.json();console.log("Discogs delete result:",l),l.success?(console.log("Discogs listing deleted successfully"),this.showToast("💿 Eliminado de Discogs")):this.showToast("⚠️ "+(l.error||"Error en Discogs"),"warning")}catch(n){console.error("Error deleting from Discogs:",n),this.showToast("⚠️ Error eliminando de Discogs, pero continuando...","warning")}}else console.log("No discogs_listing_id found, skipping Discogs deletion");await i.ref.delete(),this.showToast("✅ Disco eliminado"),await this.loadData()}catch(o){console.error("Error removing document: ",o),this.showToast("❌ Error al eliminar: "+o.message,"error")}},handleSaleSubmit(t){var k,I,T,g,f,S,C;t.preventDefault();const e=new FormData(t.target);let s=e.get("sku");s||(s=(k=document.getElementById("input-sku"))==null?void 0:k.value);const o=this.state.inventory.find(_=>_.sku===s);if(!o){this.showToast("⚠️ Debes seleccionar un producto válido del listado","error");const _=document.getElementById("sku-search");_&&(_.focus(),_.classList.add("border-red-500","animate-pulse"),setTimeout(()=>_.classList.remove("border-red-500","animate-pulse"),2e3));return}let r=parseInt(e.get("quantity"));if(isNaN(r)&&(r=parseInt((I=document.getElementById("input-qty"))==null?void 0:I.value)||1),o.stock<r){this.showToast(`❌ Stock insuficiente. Disponible: ${o.stock}`,"error");return}let a=parseFloat(e.get("price"));isNaN(a)&&(a=parseFloat((T=document.getElementById("input-price"))==null?void 0:T.value)||0);const i=parseFloat(e.get("cost"))||0,n=parseFloat(e.get("shipping_income"))||0,l=a*r+n;e.get("date")||new Date().toISOString();const p=e.get("paymentMethod"),c=e.get("soldAt");e.get("comment");let u=e.get("artist");u||(u=(g=document.getElementById("input-artist"))==null?void 0:g.value);let d=e.get("album");d||(d=(f=document.getElementById("input-album"))==null?void 0:f.value);let y=e.get("genre");y||(y=(S=document.getElementById("input-genre"))==null?void 0:S.value);let m=e.get("owner");m||(m=(C=document.getElementById("input-owner"))==null?void 0:C.value);const E=e.get("customerName"),$=e.get("customerEmail"),b=e.get("requestInvoice")==="on",v={items:[{recordId:o.id,quantity:r,unitPrice:a,costAtSale:i}],paymentMethod:p||"CASH",customerName:E||"Venta Manual",customerEmail:$||null,shipping_income:n,total_amount:l,source:"STORE",channel:(c==null?void 0:c.toLowerCase())||"store"};ve.createSale(v).then(()=>{this.showToast(b?"Venta registrada (Factura Solicitada)":"Venta registrada");const _=document.getElementById("modal-overlay");_&&_.remove();const L=t.target;L&&L.reset();const q=document.getElementById("form-total");q&&(q.innerText="$0.00");const P=document.getElementById("sku-search");P&&(P.value=""),this.state.manualSaleSearch="",this.loadData()}).catch(_=>{console.error("Error adding sale: ",_),this.showToast("❌ Error al registrar venta: "+(_.message||""),"error")})},addToCart(t,e){e&&e.stopPropagation();const s=this.state.inventory.find(r=>r.id===t||r.sku===t);if(!s)return;if(this.state.cart.filter(r=>r.sku===t).length>=s.stock){this.showToast("⚠️ No hay más stock disponible");return}this.state.cart.push(s),document.getElementById("inventory-cart-container")?this.renderInventoryCart():this.renderCartWidget(),this.showToast("Agregado al carrito")},removeFromCart(t){this.state.cart.splice(t,1),this.renderCartWidget()},clearCart(){this.state.cart=[],this.renderCartWidget()},renderOnlineSales(t){const e=this.state.sales.filter(a=>a.channel==="online"),s=e.filter(a=>a.status==="completed"),o=e.filter(a=>a.status==="PENDING"),r=s.reduce((a,i)=>a+(parseFloat(i.total_amount||i.total)||0),0);t.innerHTML=`
        <div class="p-6">
            <!-- Header -->
            <div class="flex items-center justify-between mb-8">
                <div>
                            ...
                            <tbody>
                                ${e.map(a=>{var n;const i=(n=a.timestamp)!=null&&n.toDate?a.timestamp.toDate():new Date(a.date||0);return{...a,_sortDate:i.getTime()}}).sort((a,i)=>i._sortDate-a._sortDate).map(a=>{var y,m,E,$,b,v,k;const i=a.customer||{},n=a.orderNumber||"N/A",l=(y=a.timestamp)!=null&&y.toDate?a.timestamp.toDate():new Date(a.date),c=((m=a.completed_at)!=null&&m.toDate?a.completed_at.toDate():null)||l,u={completed:"bg-green-50 text-green-700 border-green-200",PENDING:"bg-yellow-50 text-yellow-700 border-yellow-200",failed:"bg-red-50 text-red-700 border-red-200"},d={completed:"✅ Completado",PENDING:"⏳ Pendiente",failed:"❌ Fallido"};return`
                                        <tr class="border-b border-slate-50 hover:bg-slate-50 transition-colors cursor-pointer" onclick="app.openUnifiedOrderDetailModal('${a.id}')">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="font-semibold text-brand-dark">${i.name||(i.firstName?`${i.firstName} ${i.lastName||""}`:"")||((E=i.stripe_info)==null?void 0:E.name)||"Cliente"}</div>
                                                <div class="text-xs text-slate-500">${i.email||(($=i.stripe_info)==null?void 0:$.email)||"No email"}</div>
                                            </td>
                                            <td class="px-6 py-4">
                                                <div class="text-sm text-slate-600 truncate max-w-[200px]">
                                                    ${((b=i.shipping)==null?void 0:b.line1)||i.address||((k=(v=i.stripe_info)==null?void 0:v.shipping)==null?void 0:k.line1)||"Sin dirección"}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4">
                                                <div class="text-sm">
    ...
    `},openOnlineSaleDetailModal(t){var l,p,c;const e=this.state.sales.find(u=>u.id===t);if(!e)return;const s=e.customer||{},o=s.stripe_info||{},r=s.shipping||o.shipping||{},a={line1:r.line1||s.address||"Sin dirección",line2:r.line2||"",city:r.city||s.city||"",postal:r.postal_code||s.postalCode||"",country:r.country||s.country||"Denmark"},i=`
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-left">
                                    ...
                                    <tbody class="divide-y divide-orange-50">
                                        ${o.length>0?o.map(a=>{var i;return`
                                            <tr class="hover:bg-orange-50/30 transition-colors group">
                                                ...
                                                <td class="p-4">
                                                    <span class="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded-full">
                                                        ${((i=e.find(n=>n.value===(a.categoria||a.category)))==null?void 0:i.label)||a.categoria||a.category||"-"}
                                                    </span>
                                    ...
                                    <div class="flex justify-between items-center mb-3">
                                        ...
                                        <div class="text-right">
                                            ...
                                            <p class="text-lg font-bold text-green-600">
                                                ${this.formatCurrency(o.reduce((a,i)=>a+(i.monto_iva||0),0))}
                                            </p>
                                        </div>
                                    </div>
                                    <!-- Export Button -->
...
¿CONFIRMAS que deseas eliminar permanentemente este registro y su recibo?`)){this.showToast("ℹ️ Eliminación cancelada");return}}else if(!confirm("¿Eliminar esta compra?"))return;D.collection("expenses").doc(t).delete().then(()=>{this.showToast("✅ Compra eliminada"),this.loadData()}).catch(s=>console.error(s))},async downloadReceiptsZip(){const t=new Date,e=t.getFullYear(),s=t.getMonth(),o=this.state.expenses.filter(r=>{const a=new Date(r.fecha_factura||r.date);return a.getFullYear()===e&&a.getMonth()===s&&r.receiptUrl});if(o.length===0){this.showToast("ℹ️ No hay comprobantes con recibo este mes");return}this.showToast(`📦 Preparando ZIP con ${o.length} comprobantes...`);try{const r=new JSZip,a=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"],i=`Comprobantes_${e}_${String(s+1).padStart(2,"0")}_${a[s]}`,n=r.folder(i);let l=`RESUMEN DE COMPROBANTES - ${a[s]} ${e}
`;l+=`${"=".repeat(50)}

`,l+=`Generado: ${t.toLocaleString("es-ES")}
`,l+=`Total comprobantes: ${o.length}
`,l+=`Total gastos: ${this.formatCurrency(o.reduce((y,m)=>y+(m.monto_total||m.amount||0),0))}
`,l+=`Total IVA: ${this.formatCurrency(o.reduce((y,m)=>y+(m.monto_iva||0),0))}

`,l+=`${"=".repeat(50)}

`,l+=`DETALLE:

`;let p=0,c=0;for(let y=0;y<o.length;y++){const m=o[y],$=new Date(m.fecha_factura||m.date).toISOString().split("T")[0],b=(m.proveedor||"SinNombre").replace(/[^a-zA-Z0-9áéíóúÁÉÍÓÚñÑ\s]/g,"").replace(/\s+/g,"-").substring(0,20).trim(),v=Math.round(m.monto_total||m.amount||0);let k="jpg";m.receiptUrl.includes(".pdf")?k="pdf":m.receiptUrl.includes(".png")&&(k="png");const I=`${String(y+1).padStart(3,"0")}_${$}_${b}_${v}DKK.${k}`;try{const T=await fetch(m.receiptUrl);if(!T.ok)throw new Error("Fetch failed");const g=await T.blob();n.file(I,g),p++,l+=`${String(y+1).padStart(3,"0")}. ${$} | ${b}
`,l+=`    Total: ${this.formatCurrency(m.monto_total||m.amount||0)} | IVA: ${this.formatCurrency(m.monto_iva||0)}
`,l+=`    Archivo: ${I}

`}catch(T){console.warn(`Could not fetch receipt for ${m.proveedor}:`,T),c++,l+=`${String(y+1).padStart(3,"0")}. ${$} | ${b} - ⚠️ ERROR: No se pudo descargar

`}}n.file("_INDICE.txt",l);const u=await r.generateAsync({type:"blob",compression:"DEFLATE",compressionOptions:{level:6}}),d=`${i}.zip`;saveAs(u,d),c>0?this.showToast(`⚠️ ZIP generado: ${p} OK, ${c} con error`):this.showToast(`✅ ZIP descargado: ${p} comprobantes`)}catch(r){console.error("ZIP generation error:",r),this.showToast("❌ Error al generar ZIP")}},async handleReceiptUpload(t){const e=t.files[0];if(!e)return;const s=document.getElementById("upload-placeholder"),o=document.getElementById("upload-preview"),r=document.getElementById("receipt-preview-img"),a=document.getElementById("receipt-filename"),i=document.getElementById("receipt-url");s.innerHTML='<i class="ph-duotone ph-spinner text-4xl text-brand-orange animate-spin mb-2"></i><p class="text-sm text-slate-500">Subiendo...</p>';try{const n=e.name.split(".").pop().toLowerCase(),{structuredPath:l,structuredFilename:p}=this.generateReceiptPath(n),u=firebase.storage().ref().child(l);await u.put(e);const d=await u.getDownloadURL();if(i.value=d,document.getElementById("receipt-url").dataset.structuredPath=l,document.getElementById("receipt-url").dataset.structuredFilename=p,e.type.startsWith("image/"))r.src=URL.createObjectURL(e),r.classList.remove("hidden");else if(e.type==="application/pdf"){r.src="",r.classList.add("hidden");const y=r.parentNode.querySelector(".ph-file-pdf");y&&y.remove();const m=document.createElement("i");m.className="ph-duotone ph-file-pdf text-6xl text-red-500 mb-2 block mx-auto",r.parentNode.insertBefore(m,r)}a.textContent=p,s.classList.add("hidden"),o.classList.remove("hidden"),s.innerHTML=`
                <i class="ph-duotone ph-upload-simple text-4xl text-slate-300 group-hover:text-brand-orange transition-colors mb-2"></i>
                <p class="text-sm text-slate-500 group-hover:text-brand-orange transition-colors font-medium">
                    Subir Factura/Recibo
                </p>
...
`).replace(/\s+/g," "),o=t.split(/\r?\n/).map(p=>p.trim()).filter(p=>p),r=[/(\d{1,2}[-\/\.]\d{1,2}[-\/\.]\d{2,4})/,/(\d{4}[-\/\.]\d{1,2}[-\/\.]\d{1,2})/,/(\d{1,2}\.\s?\w+\.?\s?\d{2,4})/i];for(const p of r){const c=s.match(p);if(c){e.fecha=this.normalizeDate(c[1]);break}}const a=[/(?:i\s*alt|total|sum|totalt|att\s*betala)[:\s]*(\d+[.,]\d{2})/i,/(?:total|sum)[:\s]*(?:kr\.?|dkk)?\s*(\d+[.,]\d{2})/i,/(\d+[.,]\d{2})\s*(?:dkk|kr)/i];for(const p of a){const c=s.match(p);if(c){e.monto_total=parseFloat(c[1].replace(",","."));break}}const i=[/(?:moms|25%|heraf\s*moms)[:\s]*(\d+[.,]\d{2})/i,/(?:vat|iva|tax)[:\s]*(\d+[.,]\d{2})/i,/moms\s*(?:kr\.?|dkk)?\s*(\d+[.,]\d{2})/i];for(const p of i){const c=s.match(p);if(c){e.monto_iva=parseFloat(c[1].replace(",","."));break}}e.monto_total&&!e.monto_iva&&(e.monto_iva=Math.round(e.monto_total*.2*100)/100);const n=["kvittering","receipt","bon","faktura","invoice","kopi","copy"];for(const p of o.slice(0,5)){const c=p.trim();if(c.length>2&&c.length<50&&!n.some(u=>c.toLowerCase().includes(u))&&!/^\d+$/.test(c)&&!/^[\d\s\-\/\.]+$/.test(c)){e.proveedor=c;break}}const l=s.match(/(?:cvr|org\.?\s*nr)[:\s]*(\d{8})/i);if(l&&o.length>0){const p=o.findIndex(c=>c.includes(l[0]));p>0&&!e.proveedor&&(e.proveedor=o[p-1])}return console.log("Parsed Receipt Data:",e),e},normalizeDate(t){try{const s=t.replace(/\s/g,"").replace(/[\.\/]/g,"-").split("-");if(s.length>=3){let o,r,a;return s[0].length===4?[a,r,o]=s:([o,r,a]=s,a.length===2&&(a="20"+a)),o=o.padStart(2,"0"),r=r.padStart(2,"0"),`${a}-${r}-${o}`}}catch{console.warn("Date normalization failed:",t)}return null},autoFillExpenseForm(t){if(t.fecha){const e=document.getElementById("expense-fecha");e&&(e.value=t.fecha,this.highlightAutoFilled(e))}if(t.proveedor){const e=document.getElementById("expense-proveedor");e&&(e.value=t.proveedor,this.highlightAutoFilled(e))}if(t.monto_total){const e=document.getElementById("expense-monto");e&&(e.value=t.monto_total.toFixed(2),this.highlightAutoFilled(e))}if(t.monto_iva){const e=document.getElementById("expense-iva");e&&!e.disabled&&(e.value=t.monto_iva.toFixed(2),this.highlightAutoFilled(e))}},highlightAutoFilled(t){t.classList.add("ring-2","ring-green-400","bg-green-50");const e=()=>{t.classList.remove("ring-2","ring-green-400","bg-green-50"),t.removeEventListener("focus",e)};t.addEventListener("focus",e),setTimeout(e,5e3)},clearReceiptUpload(){document.getElementById("receipt-file").value="",document.getElementById("receipt-url").value="",document.getElementById("upload-placeholder").classList.remove("hidden"),document.getElementById("upload-preview").classList.add("hidden"),document.getElementById("receipt-preview-img").src="",document.getElementById("receipt-filename").textContent=""},renderConsignments(t){if(!t)return;const e=`
    <div class="max-w-7xl mx-auto px-4 md:px-8 pb-24 md:pb-8 pt-6 animate-fadeIn" >
                                                                    ...
                                                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                                                        ${this.state.consignors.map(s=>{const o=s.name,a=this.state.inventory.filter(c=>c.owner===o).reduce((c,u)=>c+u.stock,0),i=[];this.state.sales.forEach(c=>{(c.items||[]).filter(d=>{if((d.owner||"").toLowerCase()===o.toLowerCase())return!0;const y=this.state.inventory.find(m=>m.id===(d.productId||d.recordId));return y&&(y.owner||"").toLowerCase()===o.toLowerCase()}).forEach(d=>{const y=Number(d.priceAtSale||d.unitPrice||0),m=s.agreementSplit||s.split||70,E=y*m/100;i.push({...d,id:c.id,date:c.date,cost:d.costAtSale||d.cost||E,payoutStatus:c.payoutStatus||"pending",payoutDate:c.payoutDate||null})}),(!c.items||c.items.length===0)&&(c.owner||"").toLowerCase()===o.toLowerCase()&&i.push({...c,album:c.album||c.sku||"Record",cost:c.cost||(Number(c.total)||0)*(s.agreementSplit||70)/100})}),i.sort((c,u)=>new Date(u.date)-new Date(c.date)),i.reduce((c,u)=>c+(Number(u.qty||u.quantity)||1),0);const n=i.reduce((c,u)=>c+(Number(u.cost)||0),0),l=i.filter(c=>c.payoutStatus==="paid").reduce((c,u)=>c+(Number(u.cost)||0),0),p=n-l;return`
...
`)},handleAddConsignor(t){t.preventDefault();const e=new FormData(t.target),s={name:e.get("name"),agreementSplit:parseFloat(e.get("split")),email:e.get("email"),phone:e.get("phone")};D.collection("consignors").add(s).then(()=>{this.showToast("✅ Socio registrado correctamente"),document.getElementById("modal-overlay").remove(),this.loadData()}).catch(o=>{console.error(o),this.showToast("❌ Error al crear socio: "+o.message,"error")})},deleteConsignor(t){confirm("¿Eliminar este socio?")&&D.collection("consignors").doc(t).delete().then(()=>{this.showToast("✅ Socio eliminado"),this.loadData()}).catch(e=>{console.error(e),this.showToast("❌ Error al eliminar socio: "+e.message,"error")})},saveData(){try{const t={};localStorage.setItem("el-cuartito-settings",JSON.stringify(t))}catch(t){console.error("Error saving settings:",t)}},searchDiscogs(){const t=document.getElementById("discogs-search-input").value,e=document.getElementById("discogs-results");if(t){if(e.innerHTML='<p class="text-xs text-slate-400 animate-pulse p-2">Buscando en Discogs...</p>',e.classList.remove("hidden"),/^\d+$/.test(t.trim())){this.fetchDiscogsById(t.trim());return}fetch(`${R}/discogs/search?q=${encodeURIComponent(t)}`).then(s=>{if(!s.ok)throw new Error(`Error ${s.status}`);return s.json()}).then(s=>{const o=s.results||[];o.length>0?e.innerHTML=o.slice(0,10).map(r=>`
                ...
                `})}},resyncMusic(){["input-discogs-id","input-discogs-release-id","input-discogs-url","input-cover-image"].forEach(o=>{const r=document.getElementById(o);r&&(r.value="")});const t=document.querySelector('input[name="artist"]').value,e=document.querySelector('input[name="album"]').value,s=document.getElementById("discogs-search-input");s&&t&&e?(s.value=`${t} - ${e}`,this.searchDiscogs(),this.showToast("✅ Música desvinculada. Selecciona una nueva edición.","success")):this.showToast("⚠️ Falta Artista o Álbum para buscar.","error")},handleDiscogsSelection(t){const e=document.getElementById("discogs-results");e&&e.classList.add("hidden");const s=t.title.split(" - "),o=s[0]||"",r=s.slice(1).join(" - ")||t.title,a=document.querySelector("#modal-overlay form");if(!a)return;if(a.artist&&(a.artist.value=o),a.album&&(a.album.value=r),a.year&&t.year&&(a.year.value=t.year),t.thumb||t.cover_image){const n=t.cover_image||t.thumb,l=document.getElementById("input-cover-image"),p=document.getElementById("cover-preview");if(l&&(l.value=n),p){const c=p.querySelector("img"),u=document.getElementById("cover-placeholder");c&&(c.src=n,c.classList.remove("hidden")),u&&u.classList.add("hidden")}}const i=document.getElementById("input-discogs-id");if(i&&t.id&&(i.value=t.id),t.uri||t.resource_url){const n=t.uri||t.resource_url,l=n.startsWith("http")?n:"https://www.discogs.com"+n,p=document.getElementById("input-discogs-url");p&&(p.value=l)}if(t.id){const n=document.getElementById("discogs-metadata-area"),l=document.getElementById("metadata-tracks"),p=document.getElementById("metadata-tags"),c=document.getElementById("discogs-link");console.log("Metadata Area Found:",!!n),n&&(n.classList.remove("hidden"),n.style.display="grid"),l&&(l.innerHTML='<p class="text-[10px] text-slate-400 animate-pulse">Loading tracks...</p>'),this.showToast("⏳ Cargando detalles...","info"),fetch(`${R}/discogs/release/${t.id}`).then(u=>u.json()).then(u=>{const d=u.release||u;if(console.log("Full Release Data:",d),n&&(n.classList.remove("hidden"),n.style.display="grid"),c&&d.uri){const E=d.uri.startsWith("http")?d.uri:"https://www.discogs.com"+d.uri;c.href=E,c.classList.remove("hidden"),c.style.display="flex"}const y=d.styles||[],m=[...new Set(y)];p&&(p.innerHTML=m.map(E=>`<span class="meta-chip border border-slate-200">${E}</span>`).join(""));for(let E=0;E<Math.min(m.length,3);E++){const $=document.getElementById(`genre-${E+1}`);$&&($.value=m[E])}if(l)if(d.tracklist&&d.tracklist.length>0){const E=document.getElementById("input-tracks");E&&(E.value=JSON.stringify(d.tracklist)),l.innerHTML=d.tracklist.map($=>`
                            ...
                            `).join("")}else l.innerHTML='<p class="text-[10px] text-slate-400 italic">No tracks found.</p>';a.label&&d.labels&&d.labels.length>0&&(a.label.value=d.labels[0].name)}).catch(u=>{console.error("Error fetching full release:",u),l&&(l.innerHTML='<p class="text-[10px] text-red-400">Error loading tracklist.</p>')})}},openTracklistModal(t){const e=this.state.inventory.find(a=>a.id===t||a.sku===t);if(!e)return;let s=e.discogsId;document.body.insertAdjacentHTML("beforeend",`
                                                                ...
                                                                `);const r=a=>{fetch(`${R}/discogs/release/${a}`).then(i=>{if(!i.ok)throw new Error("Release not found");return i.json()}).then(i=>{const l=(i.release||i).tracklist||[],p=l.map(u=>`
                                                                ...
                                                                <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl relative animate-fadeIn max-h-[85vh] flex flex-col overflow-hidden">
                                                                    ...
                                                                    <div class="p-4 overflow-y-auto custom-scrollbar flex-1">
                                                                        ${l.length>0?p:'<p class="text-center text-slate-500 py-8">No se encontraron temas para esta edición.</p>'}
                                                                    </div>
                                                                ...
                                                                `;document.getElementById("tracklist-overlay").innerHTML=c}).catch(i=>{console.error(i),document.getElementById("tracklist-overlay").innerHTML=`
                                                                ...
                                                                `})};if(s)r(s);else{const a=`${e.artist} - ${e.album}`;fetch(`${R}/discogs/search?q=${encodeURIComponent(a)}`).then(i=>i.json()).then(i=>{if(i.results&&i.results.length>0)r(i.results[0].id);else throw new Error("No results found in fallback search")}).catch(()=>{document.getElementById("tracklist-overlay").innerHTML=`
                         <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl relative">
                            <div class="text-center py-6">
                    ...
                    `})}},renderDiscogsSales(t){const e=this.state.sales.filter(l=>l.channel==="discogs"),s=l=>parseFloat(l.total)||0,o=l=>parseFloat(l.originalTotal)||parseFloat(l.total)+(parseFloat(l.discogsFee||0)+parseFloat(l.paypalFee||0)),r=l=>o(l)-s(l),a=e.reduce((l,p)=>l+s(p),0),i=e.reduce((l,p)=>l+r(p),0),n=e.reduce((l,p)=>{const c=s(p);let u=0;return p.items&&Array.isArray(p.items)&&(u=p.items.reduce((d,y)=>{const m=parseFloat(y.costAtSale||0),E=parseInt(y.qty||y.quantity)||1;return d+m*E},0)),l+(c-u)},0);t.innerHTML=`
        <div class="p-6">
            <!-- Header -->
            <div class="flex items-center justify-between mb-8">
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div class="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="text-2xl font-bold text-red-600">${this.formatCurrency(i)}</div>
                ...
                <div class="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="text-2xl font-bold text-green-600">${this.formatCurrency(n)}</div>
                ...
                `:`
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody>
                                ${e.map(l=>{var c;const p=(c=l.timestamp)!=null&&c.toDate?l.timestamp.toDate():l.date?new Date(l.date):new Date(0);return{...l,_sortDate:p.getTime()}}).sort((l,p)=>p._sortDate-l._sortDate).map(l=>{var m;const p=(m=l.timestamp)!=null&&m.toDate?l.timestamp.toDate():new Date(l.date),c=l.items&&l.items[0],u=l.originalTotal||l.total+(l.discogsFee||0)+(l.paypalFee||0);l.discogsFee,l.paypalFee;const d=l.total,y=l.status==="pending_review"||l.needsReview;return`
                                        <tr class="border-b border-slate-50 hover:bg-purple-50/30 transition-colors cursor-pointer ${y?"bg-orange-50/50":""}" onclick="app.openUnifiedOrderDetailModal('${l.id}')">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="flex flex-col gap-2">
                                                    ...
                                                    `}
                                                    <button onclick="app.openUpdateSaleValueModal('${l.id}', ${u}, ${d})" class="w-full py-1.5 bg-slate-100 text-slate-600 hover:bg-slate-200 text-[10px] font-bold rounded-lg transition-colors border border-slate-200 flex items-center justify-center gap-1">
        ...
        `;document.body.insertAdjacentHTML("beforeend",s)},calculateModalFee(t,e){const s=parseFloat(t)||0,o=e-s,r=e>0?o/e*100:0,a=document.getElementById("modal-fee-display"),i=document.getElementById("modal-fee-value");if(o>0){a.classList.remove("hidden"),i.innerText=`- kr. ${o.toFixed(2)}`;const n=document.getElementById("modal-fee-percent");n&&(n.innerText=`${r.toFixed(1)}%`)}else a.classList.add("hidden")},async handleSaleValueUpdate(t,e,s){t.preventDefault();const r=new FormData(t.target).get("netReceived"),a=document.getElementById("update-sale-submit-btn");if(r){a.disabled=!0,a.innerHTML='<i class="ph-bold ph-circle-notch animate-spin"></i> Guardando...';try{const i=R,n=await ee.currentUser.getIdToken(),l=await fetch(`${i}/firebase/sales/${e}/value`,{method:"PATCH",headers:{"Content-Type":"application/json",Authorization:`Bearer ${n}`},body:JSON.stringify({netReceived:r})}),p=l.headers.get("content-type");if(!p||!p.includes("application/json")){const u=await l.text();throw console.error("Non-JSON response received:",u),new Error(`Server returned non-JSON response (${l.status})`)}const c=await l.json();if(c.success)this.showToast("✅ Venta actualizada y fee registrado"),document.getElementById("update-sale-modal").remove(),await this.loadData(),this.refreshCurrentView();else throw new Error(c.error||"Error al actualizar")}catch(i){console.error("Update sale error:",i),this.showToast(`❌ Error: ${i.message}`),a.disabled=!1,a.innerText="Confirmar Ajuste"}}},renderPickups(t){const e=this.state.sales.filter(i=>{var n;return i.channel==="online"&&(((n=i.shipping_method)==null?void 0:n.id)==="local_pickup"||i.shipping_cost===0&&i.status!=="failed")}),s=e.filter(i=>i.status==="completed"||i.status==="paid"||i.status==="paid_pending"),o=e.filter(i=>i.status==="ready_for_pickup"),r=e.filter(i=>i.status==="shipped"||i.status==="delivered"||i.status==="picked_up"),a=`
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                ...
                                `:s.map(i=>{var n,l;return`
                                    <tr class="hover:bg-slate-50 transition-colors cursor-pointer" onclick="app.openUnifiedOrderDetailModal('${i.id}')">
                                        <td class="p-4 text-sm font-bold text-brand-orange">#${i.id.slice(0,8)}</td>
                                        <td class="p-4 text-sm font-bold text-brand-dark">${((n=i.customer)==null?void 0:n.name)||i.customerName||"Cliente"}</td>
                                        <td class="p-4 text-xs text-slate-500">${((l=i.items)==null?void 0:l.length)||0} items</td>
                                        <td class="p-4 text-xs text-slate-500 font-medium">${this.formatDate(i.date)}</td>
                                        <td class="p-4 text-center" onclick="event.stopPropagation()">
                                            <button onclick="app.setReadyForPickup('${i.id}', event)" class="bg-brand-dark text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-slate-800 transition-colors flex items-center gap-2 mx-auto">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                ...
                                `:o.map(i=>{var n,l;return`
                                    <tr class="hover:bg-slate-50 transition-colors cursor-pointer" onclick="app.openUnifiedOrderDetailModal('${i.id}')">
                                        <td class="p-4 text-sm font-bold text-brand-orange">#${i.id.slice(0,8)}</td>
                                        <td class="p-4 text-sm font-bold text-brand-dark">${((n=i.customer)==null?void 0:n.name)||i.customerName||"Cliente"}</td>
                                        <td class="p-4 text-xs text-slate-500 font-medium">${this.formatDate((l=i.updated_at)!=null&&l.toDate?i.updated_at.toDate():i.updated_at||i.date)}</td>
                                        <td class="p-4 text-center" onclick="event.stopPropagation()">
                                            <button onclick="app.markAsDelivered('${i.id}', event)" class="bg-green-600 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-green-700 transition-colors flex items-center gap-2 mx-auto">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            <tbody class="divide-y divide-slate-50">
                                ${r.slice(0,10).map(i=>`
                                    <tr>
                                        <td class="p-4 text-sm font-medium text-slate-400">#${i.id.slice(0,8)}</td>
                                        <td class="p-4 text-sm text-slate-500">${i.customerName||"Cliente"}</td>
...
¿CONFIRMAS que deseas eliminar permanentemente este registro y su recibo?`)){this.showToast("ℹ️ Eliminación cancelada");return}}else if(!confirm("¿Estás seguro de que quieres eliminar este gasto?"))return;try{await D.collection("expenses").doc(t).delete(),this.showToast("✅ Gasto eliminado"),this.loadData()}catch(s){console.error("Error deleting expense:",s),this.showToast("❌ Error al eliminar gasto")}},renderVATReport(t){const e=new Date,s=Math.floor(e.getMonth()/3)+1,o=e.getFullYear(),r=this.state.vatReportQuarter!==void 0?this.state.vatReportQuarter:s,a=this.state.vatReportYear||o;let i,n;if(r===0)i=new Date(a,0,1),n=new Date(a,11,31,23,59,59);else{const x=(r-1)*3;i=new Date(a,x,1),n=new Date(a,x+3,0,23,59,59)}const l=this.state.sales.filter(x=>{var V;const M=(V=x.timestamp)!=null&&V.toDate?x.timestamp.toDate():new Date(x.timestamp||x.date);return M>=i&&M<=n});let p=[],c=[],u=[],d=0,y=0,m=0,E=0;l.forEach(x=>{var te;const M=(te=x.timestamp)!=null&&te.toDate?x.timestamp.toDate():new Date(x.timestamp||x.date);(x.items||[]).forEach(N=>{const le=N.priceAtSale||N.price||0;let ie=N.costAtSale||N.cost||0;const X=N.productId||N.recordId,Y=N.album;let O=N.providerOrigin||N.provider_origin;if(ie===0||!O){const H=this.state.inventory.find(U=>X&&(U.id===X||U.sku===X)||Y&&U.album===Y);H&&(ie===0&&(ie=H.cost||0),O||(O=H.provider_origin||"Local_Used"))}O||(O="Local_Used");const G=N.qty||N.quantity||1,z=le*G,Q=ie*G;if(O==="EU_B2B"||O==="DK_B2B"){const H=z*.2;d+=H,p.push({date:M,productId:N.productId||N.album||"N/A",album:N.album||"N/A",salePrice:z,vat:H})}else{const H=z-Q,U=H>0?H*.2:0;y+=U,c.push({date:M,productId:N.productId||N.album||"N/A",album:N.album||"N/A",cost:Q,salePrice:z,margin:H,vat:U})}});const oe=parseFloat(x.shipping_income||x.shipping||x.shipping_cost||0);if(oe>0){const N=oe*.2;m+=N,E+=oe,u.push({date:M,orderId:x.orderNumber||(x.id&&typeof x.id=="string"?x.id.slice(-8):"N/A"),income:oe,vat:N})}}),(this.state.extraIncome||[]).filter(x=>{const M=new Date(x.date);return M>=i&&M<=n}).forEach(x=>{const M=Number(x.amount)||0,V=Number(x.vatAmount)||0;d+=V,p.push({date:new Date(x.date),productId:"EXTRA",album:`💰 ${x.description||"Ingreso Extra"} (${x.category||"other"})`,salePrice:M,vat:V})});const b=d+y+m,v=this.state.expenses.filter(x=>{var oe;const M=x.fecha_factura?new Date(x.fecha_factura):(oe=x.timestamp)!=null&&oe.toDate?x.timestamp.toDate():new Date(x.timestamp||x.date);return(x.categoria_tipo==="operativo"||x.categoria_tipo==="stock_nuevo"||x.is_vat_deductible)&&M>=i&&M<=n}),k=v.filter(x=>x.categoria!=="envios"),I=v.filter(x=>x.categoria==="envios"),T=k.reduce((x,M)=>x+(parseFloat(M.monto_iva)||0),0),g=I.reduce((x,M)=>x+(parseFloat(M.monto_iva)||0),0);I.reduce((x,M)=>x+(parseFloat(M.monto_total)||0),0);const f=(this.state.inventory||[]).filter(x=>{if(!x.item_phantom_vat||x.item_phantom_vat<=0||x.provider_origin!=="EU_B2B")return!1;const M=x.acquisition_date?new Date(x.acquisition_date):null;return M?M>=i&&M<=n:!1}),S=f.reduce((x,M)=>x+(M.item_phantom_vat||0),0),C=(this.state.inventory||[]).filter(x=>{if(!x.item_real_vat||x.item_real_vat<=0||x.provider_origin!=="DK_B2B")return!1;const M=x.acquisition_date?new Date(x.acquisition_date):null;return M?M>=i&&M<=n:!1}),_=C.reduce((x,M)=>x+(M.item_real_vat||0),0),L=b+S,q=T+g+S+_,P=L-q,K={0:`Resumen anual ${a}`,1:`1 de junio, ${a}`,2:`1 de septiembre, ${a}`,3:`1 de diciembre, ${a}`,4:`1 de marzo, ${a+1}`}[r],ae=`
            <div class="max-w-7xl mx-auto px-4 md:px-8 pb-24 md:pb-8 pt-6">
                <!-- Header Section -->
                <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-6 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <div class="flex items-center gap-4">
                                ...
                                <div class="p-5 flex items-center justify-between border-b border-emerald-100">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-[10px] text-emerald-500 uppercase font-bold tracking-tighter">${C.length} producto${C.length>1?"s":""} DK B2B</p>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-xl font-bold text-emerald-700">${this.formatCurrency(_)}</p>
        ...
        `;t.innerHTML=ae},updateVATQuarter(){const t=parseInt(document.getElementById("vat-quarter-select").value),e=parseInt(document.getElementById("vat-year-select").value);this.state.vatReportQuarter=t,this.state.vatReportYear=e,this.renderVATReport(document.getElementById("app-content"))},downloadVATAuditReport(){const t=new Date,e=Math.floor(t.getMonth()/3)+1,s=t.getFullYear(),o=this.state.vatReportQuarter||e,r=this.state.vatReportYear||s,a=(o-1)*3,i=new Date(r,a,1),n=new Date(r,a+3,0,23,59,59),l=this.state.sales.filter(f=>{var C;const S=(C=f.timestamp)!=null&&C.toDate?f.timestamp.toDate():new Date(f.timestamp||f.date);return S>=i&&S<=n}),p=[];let c=1;l.forEach(f=>{var P;const S=(P=f.timestamp)!=null&&P.toDate?f.timestamp.toDate():new Date(f.timestamp||f.date),C=S.toISOString().slice(0,10).replace(/-/g,""),_=f.channel||"N/A";(f.items||[]).forEach(A=>{const K=A.priceAtSale||A.price||0;let W=A.costAtSale||A.cost||0;const Z=A.productId||A.recordId,ae=A.album;let x="Local_Used",M="N/A";const V=this.state.inventory.find(U=>Z&&(U.id===Z||U.sku===Z)||ae&&U.album===ae);V&&(W=W===0?V.cost||0:W,x=V.provider_origin||"Local_Used",V.acquisition_date&&(M=new Date(V.acquisition_date).toISOString().slice(0,10)));const oe=A.qty||A.quantity||1,te=A.providerOrigin||x,N=te==="EU_B2B"||te==="DK_B2B",le=K*oe,ie=W*oe;let X,Y,O;if(N)X=le,Y=le*.2,O="Standard Rate";else{const U=le-ie;X=U>0?U:0,Y=U>0?U*.2:0,O="Margin Scheme"}const G=V&&V.acquisition_date?new Date(V.acquisition_date):null,z=G&&G>=i&&G<=n,Q=z&&x==="EU_B2B"&&V.item_phantom_vat||0,H=z?x==="DK_B2B"?V.item_real_vat||0:Q:0;p.push({transactionId:`ECR-${C}-${String(c).padStart(4,"0")}`,date:S.toISOString().slice(0,10),channel:_,productName:`${A.album||"N/A"} - ${A.artist||"N/A"}`,sku:A.sku||Z||"N/A",providerOrigin:x,acquisitionDate:M,condition,costPrice:ie.toFixed(2),salesPrice:le.toFixed(2),calculationBasis:X.toFixed(2),schemeApplied:O,outputVat:Y.toFixed(2),euPhantomVat:Q.toFixed(2),inputVat:H.toFixed(2)}),c++});const q=parseFloat(f.shipping_income||f.shipping||f.shipping_cost||0);q>0&&(p.push({transactionId:`ECR-SHIP-${C}-${String(c).padStart(4,"0")}`,date:S.toISOString().slice(0,10),channel:_,productName:`Envío Cobrado - Orden: ${f.orderNumber||"N/A"}`,sku:"SHIPPING",providerOrigin:"N/A",acquisitionDate:"N/A",condition:"Service",costPrice:"0.00",salesPrice:q.toFixed(2),calculationBasis:q.toFixed(2),schemeApplied:"Standard Rate",outputVat:(q*.2).toFixed(2),euPhantomVat:"0.00",inputVat:"0.00"}),c++)});const u=[];let d=1;this.state.expenses.filter(f=>{var _;const S=f.fecha_factura?new Date(f.fecha_factura):(_=f.timestamp)!=null&&_.toDate?f.timestamp.toDate():new Date(f.timestamp||f.date);return(f.categoria_tipo==="operativo"||f.categoria_tipo==="stock_nuevo"||f.is_vat_deductible)&&S>=i&&S<=n}).forEach(f=>{var _;const S=f.fecha_factura?new Date(f.fecha_factura):(_=f.timestamp)!=null&&_.toDate?f.timestamp.toDate():new Date(f.timestamp||f.date),C=S.toISOString().slice(0,10).replace(/-/g,"");u.push({transactionId:`ECP-EXP-${C}-${String(d).padStart(4,"0")}`,invoiceDate:S.toISOString().slice(0,10),category:f.categoria==="envios"?"Shipping Expense":"Operational Expense",vendor:f.proveedor||f.nombre||"N/A",description:f.descripcion||f.categoria||"N/A",sku:"N/A",grossAmount:parseFloat(f.monto_total||0).toFixed(2),euPhantomVat:"0.00",inputVat:parseFloat(f.monto_iva||0).toFixed(2)}),d++}),(this.state.inventory||[]).filter(f=>{if(!(f.provider_origin==="EU_B2B"||f.provider_origin==="DK_B2B"))return!1;const C=f.acquisition_date?new Date(f.acquisition_date):null;return C?C>=i&&C<=n:!1}).forEach(f=>{const S=new Date(f.acquisition_date),C=S.toISOString().slice(0,10).replace(/-/g,""),_=parseFloat(f.cost||0),L=f.provider_origin==="EU_B2B"&&f.item_phantom_vat||0,q=f.provider_origin==="DK_B2B"?f.item_real_vat||0:L;u.push({transactionId:`ECP-INV-${C}-${String(d).padStart(4,"0")}`,invoiceDate:S.toISOString().slice(0,10),category:`Stock Import (${f.provider_origin})`,vendor:f.provider_origin,description:`${f.album||"N/A"} - ${f.artist||"N/A"}`,sku:f.sku||"N/A",grossAmount:_.toFixed(2),euPhantomVat:L.toFixed(2),inputVat:q.toFixed(2)}),d++});const E="\uFEFF",$=(f,S)=>[f.join(","),...S.map(C=>f.map(_=>{const L=Object.keys(C)[f.indexOf(_)];return`"${String(C[L]||"").replace(/"/g,'""')}"`}).join(","))].join(`
`),b=["Transaction ID","Transaction Date","Sales Channel","Product Name","SKU / Item ID","Provider Origin","Acquisition Date","Condition","Cost Price (DKK)","Sales Price (DKK)","Calculation Basis (DKK)","VAT Scheme Applied","Output VAT / Salgsmoms (DKK)","EU Phantom VAT / Rubrik A (DKK)","Input VAT / Købsmoms (DKK)"],v=["Transaction ID","Invoice Date","Category","Vendor / Origin","Description","SKU / Item ID","Gross Amount / Cost (DKK)","EU Phantom VAT / Rubrik A (DKK)","Input VAT / Købsmoms (DKK)"],k=$(b,p),I=$(v,u),T=new Blob([E+k],{type:"text/csv;charset=utf-8;"}),g=document.createElement("a");g.href=URL.createObjectURL(T),g.download=`Sales_VAT_Ledger_Q${o}_${r}.csv`,g.style.display="none",document.body.appendChild(g),g.click(),setTimeout(()=>{const f=new Blob([E+I],{type:"text/csv;charset=utf-8;"}),S=document.createElement("a");S.href=URL.createObjectURL(f),S.download=`Purchases_VAT_Ledger_Q${o}_${r}.csv`,S.style.display="none",document.body.appendChild(S),S.click(),document.body.removeChild(g),document.body.removeChild(S),URL.revokeObjectURL(g.href),URL.revokeObjectURL(S.href)},300),this.showToast(`✅ Exported ${p.length} sales & ${u.length} purchase records.`)},renderInvestments(t){const e=["Alejo","Facundo","Rafael"],s=this.state.investments||[],o=e.reduce((i,n)=>(i[n]=s.filter(l=>l.partner===n).reduce((l,p)=>l+(parseFloat(p.amount)||0),0),i),{}),r=Object.values(o).reduce((i,n)=>i+n,0),a=`
            <div class="max-w-7xl mx-auto px-4 md:px-8 pb-24 pt-6">
                <div class="flex justify-between items-center mb-8">
                    <div>
                    ...
                    ${e.map(i=>`
                        <div class="bg-white rounded-2xl shadow-sm border border-orange-100 p-5 hover:shadow-md transition-shadow">
                            <div class="flex items-center gap-3 mb-3">
                                ...
                                <h3 class="font-bold text-brand-dark">${i}</h3>
                            </div>
                            <p class="text-2xl font-display font-bold text-brand-dark">${this.formatCurrency(o[i])}</p>
                            <p class="text-xs text-slate-400">${s.filter(n=>n.partner===i).length} inversiones</p>
                        </div>
                    `).join("")}
                ...
                ${e.map(i=>{const n=s.filter(l=>l.partner===i).sort((l,p)=>new Date(p.date)-new Date(l.date));return`
                    <div class="bg-white rounded-2xl shadow-sm border border-orange-100 overflow-hidden mb-6">
                        <div class="p-5 border-b border-orange-50 bg-orange-50/30 flex justify-between items-center">
                            <h3 class="font-bold text-brand-dark flex items-center gap-2">
                                <span class="w-8 h-8 rounded-lg bg-brand-orange/10 flex items-center justify-center text-brand-orange font-bold">${i.charAt(0)}</span>
                                ${i}
                            </h3>
                            <span class="text-lg font-display font-bold text-brand-orange">${this.formatCurrency(o[i])}</span>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full text-left">
                                <thead class="bg-slate-50 text-[10px] uppercase text-slate-500 font-bold">
        ...
        `;document.body.insertAdjacentHTML("beforeend",s)},async saveInvestment(t){t.preventDefault();const e=t.target,s={partner:e.partner.value,amount:parseFloat(e.amount.value),description:e.description.value,date:e.date.value,created_at:firebase.firestore.FieldValue.serverTimestamp()};try{await D.collection("investments").add(s),document.getElementById("add-investment-modal").remove(),this.showToast("✅ Inversión registrada"),await this.loadInvestments(),this.refreshCurrentView()}catch(o){this.showToast("❌ Error: "+o.message,"error")}},async deleteInvestment(t){if(confirm("¿Eliminar esta inversión?"))try{await D.collection("investments").doc(t).delete(),this.showToast("🗑️ Inversión eliminada"),await this.loadInvestments(),this.refreshCurrentView()}catch(e){this.showToast("❌ Error: "+e.message,"error")}},async loadInvestments(){const t=await D.collection("investments").get();this.state.investments=t.docs.map(e=>({id:e.id,...e.data()}))},renderShipping(t){const e=l=>{var p;return((p=l.shipping_method)==null?void 0:p.id)==="local_pickup"||l.shipping_method&&typeof l.shipping_method=="string"&&l.shipping_method.toLowerCase().includes("pickup")||l.shippingMethod&&l.shippingMethod.toLowerCase().includes("pickup")||Number(l.shipping)===0||Number(l.shipping_cost)===0||Number(l.shipping_income)===0},s=l=>!e(l),o=l=>!["shipped","picked_up","delivered","fulfilled","canceled"].includes(l.fulfillment_status),r=this.state.sales.filter(l=>{var p;return(l.channel==="online"||((p=l.channel)==null?void 0:p.toLowerCase())==="discogs")&&e(l)&&o(l)}).sort((l,p)=>new Date(l.date)-new Date(p.date)),a=this.state.sales.filter(l=>{var p;return(l.channel==="online"||((p=l.channel)==null?void 0:p.toLowerCase())==="discogs")&&s(l)&&o(l)}).sort((l,p)=>new Date(l.date)-new Date(p.date)),i=this.state.sales.filter(l=>{var p;return(l.channel==="online"||((p=l.channel)==null?void 0:p.toLowerCase())==="discogs")&&!o(l)}).sort((l,p)=>{var c,u;return new Date((c=p.updated_at)!=null&&c.toDate?p.updated_at.toDate():p.updated_at||p.date)-new Date((u=l.updated_at)!=null&&u.toDate?l.updated_at.toDate():l.updated_at||l.date)}).slice(0,20),n=`
            <div class="max-w-7xl mx-auto px-4 md:px-8 pb-24 pt-6 animate-fadeIn">
                   ...
                   <div class="flex gap-4">
                        <div class="bg-indigo-500 text-white px-5 py-3 rounded-2xl shadow-lg shadow-indigo-500/20 flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-2xl font-display font-bold">${this.formatCurrency(this.state.sales.reduce((l,p)=>l+parseFloat(p.shipping||p.shipping_cost||0),0))}</p>
                            </div>
                        </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-blue-50">
                                ${r.length>0?r.map(l=>{var v;const p=this.getCustomerInfo(l),c=l.fulfillment_status||"unfulfilled";let u=c==="unfulfilled"?`onclick="app.notifyPreparingDiscogs('${l.id}')"`:"disabled",d=c==="unfulfilled"?"bg-blue-500 text-white shadow-sm hover:bg-blue-600":c==="preparing"||c==="ready_for_pickup"?"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-75":"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-50",y=c==="preparing"?`onclick="app.notifyPickupReadyDiscogs('${l.id}')"`:"disabled",m=c==="preparing"?"bg-brand-orange text-white shadow-sm hover:bg-orange-600":c==="ready_for_pickup"?"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-75":"bg-slate-50 border border-slate-100 text-slate-300 cursor-not-allowed opacity-50",E=c==="ready_for_pickup"?`onclick="app.markPickedUpDiscogs('${l.id}')"`:"disabled",b=`
                <div class="flex flex-col gap-2 relative pl-2">
                    ...
                    <button onclick="app.cancelOrderDiscogs('${l.id}')" class="w-full text-left px-3 py-1.5 mt-1 rounded-lg text-[10px] font-bold text-red-500 hover:bg-red-50 hover:text-red-600 transition-colors flex items-center gap-2 border border-transparent hover:border-red-100">
                                    ...
                                    <tr class="hover:bg-blue-50/20 transition-colors">
                                        ...
                                        <td class="p-4">
                                            <div class="flex -space-x-2 overflow-hidden">
                                                ${(l.items||[]).slice(0,3).map(k=>`<img src="${k.image||k.cover_image||"https://elcuartito.dk/default-vinyl.png"}" 
                                            ...
                                            <div class="text-[10px] text-slate-400 mt-1">${((v=l.items)==null?void 0:v.length)||0} items</div>
                                        </td>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-orange-100 overflow-hidden mb-8">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-orange-50">
                                ${a.length>0?a.map(l=>{var b;const p=this.getCustomerInfo(l),c=l.fulfillment_status||"unfulfilled";let u=c==="unfulfilled"?`onclick="app.notifyPreparingDiscogs('${l.id}')"`:"disabled",d=c==="unfulfilled"?"bg-brand-orange text-white shadow-sm hover:bg-orange-600":c==="preparing"||c==="in_transit"?"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-75":"bg-slate-50 border border-slate-100 text-slate-400 cursor-not-allowed opacity-50",y=c==="in_transit"?`onclick="app.markDispatchedDiscogs('${l.id}')"`:"disabled",m=c==="in_transit"?"bg-brand-dark text-white shadow-sm hover:bg-black":"bg-slate-50 border border-slate-100 text-slate-300 cursor-not-allowed opacity-50",E="";c==="preparing"?E=`
            ...
            `;return`
                                    <tr class="hover:bg-orange-50/20 transition-colors">
                                        ...
                                        <td class="p-4">
                                            <div class="flex -space-x-2 overflow-hidden">
                                                ${(l.items||[]).slice(0,3).map(v=>`<img src="${v.image||v.cover_image||"https://elcuartito.dk/default-vinyl.png"}" 
                                                         class="inline-block h-8 w-8 rounded-full ring-2 ring-white object-cover" 
                                                         title="${v.album}">`).join("")}
                                                ${(l.items||[]).length>3?`<span class="h-8 w-8 rounded-full bg-gray-100 flex items-center justify-center text-[10px] ring-2 ring-white text-slate-500 font-bold">+${l.items.length-3}</span>`:""}
                                            </div>
                                            <div class="text-[10px] text-slate-400 mt-1">${((b=l.items)==null?void 0:b.length)||0} items</div>
                                        </td>
                                ...
                                ${i.map(l=>{var p;return`
                                    <tr class="hover:bg-slate-50 transition-colors cursor-pointer" onclick="app.openUnifiedOrderDetailModal('${l.id}')" title="Ver historial">
                                        ...
                                        <td class="p-4 text-xs text-slate-400">
                                            ${this.formatDate((p=l.updated_at)!=null&&p.toDate?l.updated_at.toDate():l.updated_at||l.date)}
                                        </td>
                                        <td class="p-4 text-right">
        ...
        `;t.innerHTML=n},openOrderHistoryModal(t){var l,p,c,u,d,y;const e=this.state.sales.find(m=>m.id===t);if(!e)return;const s=e.history||[],o=(l=e.timestamp)!=null&&l.toDate?e.timestamp.toDate():new Date(e.date);let r=[];s.length>0?r=s.map(m=>({status:m.status,timestamp:new Date(m.timestamp),note:m.note})).sort((m,E)=>E.timestamp-m.timestamp):r.push({status:e.fulfillment_status,timestamp:(p=e.updated_at)!=null&&p.toDate?e.updated_at.toDate():new Date,note:"Última actualización"}),r.push({status:"created",timestamp:o,note:`Orden recibida via ${e.channel||"Online"}`});const a=m=>m==="created"?"bg-slate-100 text-slate-500":m==="preparing"?"bg-blue-100 text-blue-600":m==="ready_for_pickup"?"bg-emerald-100 text-emerald-600":m==="in_transit"?"bg-orange-100 text-orange-600":m==="shipped"||m==="picked_up"?"bg-green-100 text-green-600":"bg-slate-100",i=m=>({created:"Orden Creada",preparing:"En Preparación",ready_for_pickup:"Listo para Retiro",in_transit:"En Tránsito",shipped:"Despachado",picked_up:"Retirado"})[m]||m,n=document.createElement("div");n.className="fixed inset-0 bg-brand-dark/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4",n.onclick=m=>{m.target===n&&n.remove()},n.innerHTML=`
        ...
        `,document.body.appendChild(t)},async handleBulkImportBatch(){const t=document.getElementById("bulk-csv-data").value.trim();if(!t){this.showToast("Por favor, pega el contenido del CSV.","error");return}const e=document.getElementById("start-bulk-import-btn");e.innerHTML,e.disabled=!0,e.innerHTML='<i class="ph-bold ph-spinner animate-spin"></i> Importando...';try{const s=await fetch(`${R}/discogs/bulk-import`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({csvData:t})}),o=await s.json();s.ok&&(this.showToast(`✅ ${o.summary}`),document.getElementById("bulk-import-modal").remove(),await this.loadData(),this.refreshCurrentView())}catch(s){console.error("Bulk import error:",s),this.showToast("❌ "+s.message,"error");const o=document.getElementById("start-bulk-import-btn");o&&(o.disabled=!1,o.innerHTML='<i class="ph-bold ph-rocket-launch"></i> Comenzar Importación')}},async refreshProductMetadata(t){const e=document.getElementById("refresh-metadata-btn");if(!e)return;const s=e.innerHTML;e.disabled=!0,e.innerHTML='<i class="ph-bold ph-spinner animate-spin"></i> ...';try{let o=t;const r=this.state.inventory.find(n=>n.sku===t||n.id===t);r&&r.id&&(o=r.id);const a=await fetch(`${R}/discogs/refresh-metadata/${o}`,{method:"POST",headers:{"Content-Type":"application/json"}}),i=await a.json();if(a.ok){this.showToast("✅ Metadata actualizada correctamente");const n=document.getElementById("modal-overlay");n&&n.remove(),await this.loadData(),this.refreshCurrentView(),r&&this.openProductModal(r.sku)}else throw new Error(i.error||"Error al actualizar metadata")}catch(o){console.error("Refresh metadata error:",o),this.showToast("❌ "+o.message,"error"),e.disabled=!1,e.innerHTML=s}}};window.app=Se;document.addEventListener("DOMContentLoaded",()=>{Se.init()});window.migrateAllData=async function(t=!0){const e=firebase.firestore();console.log(`🚀 Iniciando Migración Total... (Dry Run: ${t})`);try{const s=await e.collection("products").get(),o=await e.collection("sales").get();let r=[],a=[];const i=new Map;for(const n of s.docs){const l=n.data();let p=!1,c={};l.sku&&i.set(l.sku,l),i.set(n.id,l),(!l.provider_origin||l.provider_origin==="Local_Used")&&(l.product_condition==="New"||l.condition==="New"?l.provider_origin!=="EU_B2B"&&(c.provider_origin="EU_B2B",p=!0):l.provider_origin||(c.provider_origin="Local_Used",p=!0)),p&&(r.push({id:n.id,changes:c,name:l.album}),Object.assign(l,c))}for(const n of o.docs){const l=n.data();let p=!1;if(!l.items||!Array.isArray(l.items))continue;let c=l.items.map(u=>{let d={...u},y=null;if(u.productCondition==="New"||u.condition==="New")y="EU_B2B";else if(u.productCondition==="Second-hand"||u.productCondition==="Used")y="Local_Used";else{const m=u.productId||u.recordId,E=i.get(u.sku)||i.get(m);E?y=E.provider_origin||"Local_Used":y="Local_Used"}return d.providerOrigin!==y&&(d.providerOrigin=y,p=!0),d});p&&a.push({id:n.id,items:c})}if(console.log(`📦 Discos de inventario a corregir: ${r.length}`),r.length>0&&console.log("Ejemplo de disco:",r[0]),console.log(`🧾 Ventas a corregir: ${a.length}`),t)console.log("⚠️ MODO DRY RUN. Escribí window.migrateAllData(false) para aplicar los cambios.");else{console.log("💾 Guardando en Firestore...");const n=e.batch();let l=0;const p=async()=>{l>0&&(await n.commit(),console.log(`✅ Batch commited (${l} operaciones)`),l=0)};for(const c of r)n.update(e.collection("products").doc(c.id),c.changes),l++,l>=450&&await p();for(const c of a)n.update(e.collection("sales").doc(c.id),{items:c.items}),l++,l>=450&&await p();await p(),console.log("🎉 Migración Total Finalizada!")}}catch(s){console.error("❌ Error en la migración:",s)}};window.migrateAllData(!0);
