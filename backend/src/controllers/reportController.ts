...
export const generateFinancialReport = async (req: Request, res: Response) => {
    try {
        ...
        const expensesByTimestampSnapshot = await db.collection('expenses')
            .where('timestamp', '>=', start)
            .where('timestamp', '<=', end)
            .get();

        const extraIncomeSnapshot = await db.collection('extraIncome')
            .where('date', '>=', start.toISOString().split('T')[0])
            .where('date', '<=', end.toISOString().split('T')[0])
            .get();
        ...
        const consignorsSnapshot = await db.collection('consignors').get();
        const consignors = consignorsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            ...
            if (items.length > 0) {
                items.forEach((item: any) => {
                    ...
                    if (itemCost === 0 || !origin) {
                        ...
                        const inventoryProduct: any = inventory.find((p: any) => 
                    ...
                    if (owner === 'el cuartito' || owner === '') {
                        ...
                        if (itemCost === 0 || isNaN(itemCost)) {
                            const partner: any = consignors.find((c: any) => (c.name || '').toLowerCase() === owner);
                    ...
                    itemBreakdown.push({
                        date: sale.timestamp?.toDate ? sale.timestamp.toDate() : new Date(sale.timestamp || sale.date),
                        orderId: doc.id,
