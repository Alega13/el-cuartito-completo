 ...
 */
export function calculateDenmarkVAT(
    salesPrice: number,
    costPrice: number,
...
export function calculateSaleVATLiability(
    ...
    for (const item of items) {
        const result = calculateDenmarkVAT(
            item.priceAtSale,
            item.costAtSale,
            item.productCondition || 'Second-hand' // Default to used if not specified
        );
