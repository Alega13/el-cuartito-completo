...
const isKeyMissing = (key: string | undefined) =>
    !key || key === 're_placeholder' || key === 're_your_api_key_here';
    ...
    <tr>
      ...
      <td align="right" style="padding:16px 0;border-top:1px solid #ececec;vertical-align:middle;font-size:14px;color:#555555;white-space:nowrap;">DKK ${price.toFixed(2)}</td>
        ...
        if (hasNew) {
            ...
            if (!hasUsed && shipping_cost > 0) finalVatAmount += shipping_cost * 0.20;
            vatBreakdownHtml = `
    <tr>
...
export const sendWelcomeEmail = async (email: string) => {
    try {
        ...
        const unsubscribeUrl = `${config.FRONTEND_URL || SHOP_URL}/api/newsletter/unsubscribe?email=${encodeURIComponent(email)}`;
...
export const sendWeeklyDropEmail = async (
    ...
    try {
        ...
        const SHOP_URL = 'https://elcuartito.dk';
        const unsubscribeUrl = `${config.FRONTEND_URL || SHOP_URL}/api/newsletter/unsubscribe?email=${encodeURIComponent(recipientEmail)}`;
