...
const app = {
    ...
    renderInventory(container) {
        const filteredInventory = this.state.inventory.filter(item => {
            ...
            return item.artist.toLowerCase().includes(search) ||
            ...
            <div class="max-w-7xl mx-auto pb-24 md:pb-8 px-4 md:px-8 pt-10">
                ...
                <div class="space-y-3 md:hidden mt-2">
                    ${filteredInventory.map(item => `
                        <div class="bg-white p-4 rounded-xl border border-slate-200 flex gap-4 items-center" onclick="app.openAddVinylModal('${item.sku}')">
                            <div class="w-12 h-12 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400 shrink-0">
                                <i class="ph-fill ph-music-note text-xl"></i>
                            </div>
        ...
        const modalHtml = `
            <div id="modal-overlay" class="fixed inset-0 bg-brand-dark/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl transform scale-100 transition-all border border-orange-100 max-h-[90vh] overflow-y-auto">
                    <h3 class="font-display text-xl font-bold mb-4 text-brand-dark">${isEdit ? 'Editar Vinilo' : 'Agregar Nuevo Vinilo'}</h3>
                    <form onsubmit="app.handleAddVinyl(event, '${isEdit ? item.sku : ''}')" class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="genre" onchange="app.checkCustomInput(this, 'custom-genre-container')" class="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 focus:border-brand-orange outline-none">
                        ...
                        <div class="p-3 bg-orange-50/50 rounded-xl border border-orange-100">
                            <div class="grid grid-cols-3 gap-3 mb-2">
                                <div>
                                    <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Precio Venta</label>
                                    <input name="price" id="modal-price" type="number" step="0.5" value="${item.price}" required onchange="app.updateCostFromOwner()" class="w-full bg-white border border-slate-200 rounded-lg p-2 focus:border-brand-orange outline-none font-bold text-brand-dark">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Costo / A Pagar</label>
                                    <input name="cost" id="modal-cost" type="number" step="0.5" value="${item.cost}" required class="w-full bg-white border border-slate-200 rounded-lg p-2 focus:border-brand-orange outline-none">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Margen %</label>
                                    <input name="margin" id="modal-margin" type="number" value="40" onchange="app.calculatePrice()" class="w-full bg-white border border-slate-200 rounded-lg p-2 focus:border-brand-orange outline-none text-brand-orange font-bold">
                                </div>
                            </div>
                            <p id="cost-helper" class="text-[10px] text-slate-400 italic text-center mt-1">
                                ${item.owner === 'El Cuartito' ? 'Calculado manualmente o por margen.' : 'Calculado automáticamente según acuerdo.'}
                            </p>
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Stock</label>
                            <input name="stock" type="number" value="${item.stock}" required class="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 focus:border-brand-orange outline-none">
                        </div>
